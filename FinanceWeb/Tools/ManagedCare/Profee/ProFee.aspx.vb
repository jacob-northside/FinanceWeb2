...
Partial Class ProFee
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
          ...
          ",[CONTRACTID_SUM_NAME]" & _
               " FROM DWH.Reference.ContractID_Profee_LU " & _
            ...
            x = "notfilled"
            stupidsql = "SELECT case when COUNT(*) > 100 then 100 else COUNT(*) end as cnt FROM DWH.Reference.ContractID_Profee_LU  where STARINSPLANCODE is null" & _
    ...
    Private Sub CheckUserPermission()
        ...
        If GetScalar("select count(*) from WebFD.dbo.aspnet_Users u " & _
            ...
            tabProFeeTins.Visible = True
            GalenLU.Visible = True
            cbEditInsplans.Visible = True
            tpGalenUpdates.HeaderText = "Galen STARINSPLAN Update"
        End If
    ...
    Protected Sub ddlSelectedPayor_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles ddlSelectedPayor.SelectedIndexChanged
        ...
        Try
            ...
            PopulateTINs()
            If ddlSelectedTINs.Items.Count > 0 Then
                ddlSelectedTINs.SelectedIndex = 0
            End If

            'Dim Sql2 As String
            Dim NewSQL2 As String
            'Dim ds As DataSet
            Dim da As SqlDataAdapter
            Dim dr As DataRow
            ...
            Else
                ...
                Try
                    ...
                    'Flipped 10/30/2017 CRW for ease of use

                    NewSQL2 = "select FeeSchedIDs as [IDs], PayorName as [Payor Name], FeeSchedName as [FeeSched Name], convert(varchar, EffectiveDate, 107) as [Effective Date], convert(varchar, EndDate, 107) as [End Date], [Default], ManagersNotes as [Manager's Notes]  " & _
                        "from ( " & _
                        "Select distinct  " & _
                        "    substring( " & _
                        "        ( " & _
                        "            Select ', '+ convert(varchar, ST1.FeeSchedID)  AS [text()] " & _
                        "            From (select distinct FeeSchedID, [FeeSchedName] ,[PayorName] ,[EffectiveDate] ,[EndDate] ,[Default] ,[LoadDate] ,[ManagersNotes]   " & _
                        "			from DWH.ProFee.FeeSchedule ) ST1 " & _
                        "            Where isnull(ST1.FeeSchedName, '') = isnull(ST2.FeeSchedName, '') " & _
                        "				and isnull(ST1.PayorName, '') = isnull(ST2.PayorName, '') " & _
                        "				and isnull(ST1.EffectiveDate, '1/1/1800') = isnull(ST2.EffectiveDate, '1/1/1800') " & _
                        "				and isnull(ST1.EndDate, '12/31/9999') = isnull(ST2.EndDate, '12/31/9999') " & _
                        "				and isnull(ST1.[Default], '') = isnull(ST2.[Default], '') " & _
                        "				and isnull(ST1.LoadDate, '1/1/1800') = isnull(ST2.LoadDate, '1/1/1800') " & _
                        "				and isnull(ST1.ManagersNotes, '') = isnull(ST2.ManagersNotes, '')			 " & _
                        "           ORDER BY ST1.FeeSchedName " & _
                        "            For XML PATH ('') " & _
                        "        ),3, 5000) [FeeSchedIDs], " & _
                        "		FeeSchedID, [FeeSchedName] ,[PayorName] ,[EffectiveDate] ,[EndDate] ,[Default] ,[LoadDate] ,[ManagersNotes]   " & _
                        "From DWH.ProFee.FeeSchedule ST2 " & _
                        "where PayorName = '" & Replace(ddlSelectedPayor.SelectedValue.ToString, "'", "''") & "'  and (FeeSchedName = '" & _
                            Replace(ddlSelectedFS.SelectedValue.ToString, "'", "''") & "' or '" & Replace(ddlSelectedFS.SelectedValue.ToString, "'", "''") & "' = '  -- None --  ') " & _
                        ") x " & _
                        "group by FeeSchedIDs, [FeeSchedName] ,[PayorName] ,[EffectiveDate] ,[EndDate] ,[Default] ,[LoadDate] ,[ManagersNotes]   " & _
                        "	order by FeeSchedName, PayorName, EffectiveDate desc, EndDate desc, LoadDate desc, " & _
                        "    [Default], [ManagersNotes] "
                    ...
                    Using conn2 As New System.Data.SqlClient.SqlConnection(ConfigurationManager.ConnectionStrings("PRDconn").ConnectionString)
                        da = New SqlDataAdapter(NewSQL2, conn2)
                        da.Fill(tennis, "DEMO")
                        ...
                        txtCurrentMN.Text = dr("Manager's Notes").ToString
                        ...
                        FakelblManagersNotes.Text = dr("Manager's Notes").ToString
    ...
    Private Sub PopulateFeeSchedules()

        ddlSelectedFS.DataSource = GetData("declare @Value varchar(max) " & _
            "set @Value = '  -- None --  ' " & _
           "" & _
           "  Select Distinct FeeSchedName from (select @Value as FeeSchedName from DWH.ProFee.FeeSchedule ) s " & _
           "  " & _
           " Union " & _
           "  " & _
           " select Distinct FeeSchedName from DWH.ProFee.FeeSchedule " & _
           " where PayorName = '" & Replace(ddlSelectedPayor.SelectedValue, "'", "''") & "' or '" & Replace(ddlSelectedPayor.SelectedValue, "'", "''") & "' = 'novalueselected' " & _
           " order by FeeSchedName asc")
        ddlSelectedFS.DataTextField = "FeeSchedName"
        ddlSelectedFS.DataValueField = "FeeSchedName"
        ddlSelectedFS.DataBind()

    End Sub

    Private Sub PopulateTINs()

        ddlSelectedTINs.DataSource = GetData(" " & _
                    "if OBJECT_ID('tempdb..#tempCat') is not null " & _
    "begin  " & _
    "drop table #tempCat " & _
    "end  " & _
                "if OBJECT_ID('tempdb..#tempCat2') is not null " & _
    "begin  " & _
    "drop table #tempCat2 " & _
    "end  " & _
" " & _
"create table #tempCat (rownumber int, TIN varchar(max)) " & _
        "declare @cat2 NVARCHAR(max) " & _
        "create table #tempCat2 (rowlist varchar(max), TIN varchar(max)) " & _
        "insert into #tempCat2 (rowlist, TIN ) values (-1, '  -- None --  ') " & _
" " & _
        "DECLARE @concatenated NVARCHAR(max) " & _
        "declare @counter integer = (select MAX(rankin) from ( " & _
        "select FeeSchedID,  " & _
        "DENSE_RANK() over (partition by PayorName order by FeeSchedName, [EffectiveDate] desc ,[EndDate] desc, [Default] ,[LoadDate] desc ,[ManagersNotes])  " & _
        "as rankin   " & _
        "from DWH.ProFee.FeeSchedule where PayorName = '" & Replace(ddlSelectedPayor.SelectedValue, "'", "''") & "' and " & _
        "(FeeSchedName = '" & Replace(ddlSelectedFS.SelectedValue, "'", "''") & "' or '" & Replace(ddlSelectedFS.SelectedValue, "'", "''") & "' = '  -- None --  ')  " & _
        ") x ) " & _
" " & _
        "declare @i integer = 1 " & _
"		 " & _
"		while @i < @counter + 1 " & _
        "begin " & _
        "set @concatenated = '' " & _
"         " & _
        "SELECT @concatenated = @concatenated + case when TIN = '' then '' when TIN = ' ' then '' else TIN + ', ' end " & _
        "FROM DWH.ProFee.FeeSchedule where FeeSchedID in (select FeeSchedID from (select FeeSchedID,  " & _
        "DENSE_RANK() over (partition by PayorName order by FeeSchedName, [EffectiveDate] desc ,[EndDate] desc ,[LoadDate] desc, [Default] ,[ManagersNotes])  " & _
        "as rankin  from DWH.ProFee.FeeSchedule where PayorName = '" & Replace(ddlSelectedPayor.SelectedValue, "'", "''") & "' and " & _
        "(FeeSchedName = '" & Replace(ddlSelectedFS.SelectedValue, "'", "''") & "' or '" & Replace(ddlSelectedFS.SelectedValue, "'", "''") & "' = '  -- None --  ') )  " & _
        "x where rankin = @i) order by TIN, FeeSchedName, [EffectiveDate] desc ,[EndDate] desc ,[LoadDate], [Default] ,[ManagersNotes] " & _
"               " & _
        "insert into #tempCat (rownumber, TIN)  " & _
        "Select top 1 @i , left(@concatenated, LEN(@concatenated) - 1) as TIN  " & _
        "FROM DWH.ProFee.FeeSchedule where  " & _
        "FeeSchedID in (select FeeSchedID from (select FeeSchedID, DENSE_RANK() over (partition by PayorName  " & _
        "order by FeeSchedName, [EffectiveDate] desc ,[EndDate] desc ,[LoadDate] desc, [Default] ,[ManagersNotes])  " & _
        "as rankin  from DWH.ProFee.FeeSchedule where PayorName = '" & Replace(ddlSelectedPayor.SelectedValue, "'", "''") & "' and  " & _
        "(FeeSchedName = '" & Replace(ddlSelectedFS.SelectedValue, "'", "''") & "' or '" & Replace(ddlSelectedFS.SelectedValue, "'", "''") & "' = '  -- None --  ') ) x where rankin = @i)  " & _
        "order by FeeSchedName, [EffectiveDate] desc ,[EndDate] desc ,[LoadDate], [Default] ,[ManagersNotes] " & _
"         " & _
"         " & _
        "set @i = @i + 1 " & _
        "end " & _
"         " & _
        "declare @counter2 int = (select COUNT(distinct TIN) from #tempCat) " & _
        "declare @j int = 0 " & _
"         " & _
        "while @j < @counter2 + 1 " & _
        "begin " & _
        "set @cat2 = '(' " & _
        "select @cat2 = @cat2 + convert(varchar, rownumber) + ', ' " & _
        "from (select *, Dense_Rank() over (order by TIN) as rnk from #tempCat) x where rnk = @j " & _
"         " & _
        "insert into #tempCat2 ( rowlist ,TIN) Select top 1 left(@cat2, LEN(@cat2) - 1) + ')' as rowlist , TIN " & _
        "from (select *, Dense_Rank() over (order by TIN) as rnk from #tempCat) x where rnk = @j " & _
"         " & _
        "set @j	= @j + 1 " & _
"         " & _
...
"         " & _
        ...
        ddlSelectedTINs.DataTextField = "TIN"
        ddlSelectedTINs.DataValueField = "rowlist"
        ddlSelectedTINs.DataBind()
    ...
    Sub ddlSelectedFSChangedFunction()
        ...
        Try
            ...
            'Flipped 10/30/2017 for EaseOfUse -Chelsea
    ...
    Protected Sub GridView1_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles GridView1.SelectedIndexChanged
        ...
        Try
            ...
            If ddlSelectedTINs.SelectedIndex <> 0 Then
                ...
                txtCurrentEndD.DataBind()
                txtCurrentMN.Text = dr3("Manager's Notes").ToString
                txtCurrentMN.DataBind()
                txtPayorName.Text = dr3("Payor Name").ToString
                txtPayorName.DataBind()
                txtFSName.Text = dr3("FeeSched Name").ToString
                txtFSName.DataBind()
                UpdateButton.Enabled = True
                ...
                FakelblEndDate.DataBind()
                FakelblManagersNotes.Text = dr3("Manager's Notes").ToString
                FakelblManagersNotes.DataBind()
                FakelblPayorName.Text = dr3("Payor Name").ToString
                FakelblPayorName.DataBind()
                FakelblFSName.Text = dr3("FeeSched Name").ToString
                FakelblFSName.DataBind()
            ...
            txtCurrentMN.Text = dr("Manager's Notes").ToString
            txtCurrentMN.DataBind()
            txtPayorName.Text = dr("Payor Name").ToString
            txtPayorName.DataBind()
            txtFSName.Text = dr("FeeSched Name").ToString
            txtFSName.DataBind()
            UpdateButton.Enabled = True
            ...
            FakelblManagersNotes.Text = dr("Manager's Notes").ToString
            FakelblManagersNotes.DataBind()
            FakelblPayorName.Text = dr("Payor Name").ToString
            FakelblPayorName.DataBind()
            FakelblFSName.Text = dr("FeeSched Name").ToString
            FakelblFSName.DataBind()
    ...
    Private Sub lblPreviousTIN_DataBinding(sender As Object, e As System.EventArgs) Handles lblPreviousTIN.DataBinding
        ...
        Try
            ...
            If InStr(Right(Left(leftovers, Len(leftovers) - 1), Len(leftovers) - 2), "|", CompareMethod.Text) > 0 Then
                ...
                If InStr(leftovers, "|NAEC/TEG|", CompareMethod.Text) > 0 Then
                    cbNAEC.Checked = True
                    leftovers = leftovers.Replace("|NAEC/TEG|", "")
                ElseIf InStr(leftovers, "|NAEC|", CompareMethod.Text) > 0 Then
                    cbNAEC.Checked = True
                    leftovers = leftovers.Replace("|NAEC|", "")
                ElseIf InStr(leftovers, "|TEG|", CompareMethod.Text) > 0 Then
                    cbNAEC.Checked = True
                    leftovers = leftovers.Replace("|TEG|", "")
                Else
                    cbNAEC.Checked = False
                End If

                If InStr(leftovers, "|SEPC|", CompareMethod.Text) > 0 Then
                    cbSEPC.Checked = True
                    leftovers = leftovers.Replace("|SEPC|", "")
                Else
                    cbSEPC.Checked = False
                End If

                If InStr(leftovers, "|NGDC|", CompareMethod.Text) > 0 Then
                    cbNGDC.Checked = True
                    leftovers = leftovers.Replace("|NGDC|", "")
                Else
                    cbNGDC.Checked = False
                End If
                ...
                If InStr(leftovers, "|AJSS|", CompareMethod.Text) > 0 Then
                    cbAJSS.Checked = True
                    leftovers = leftovers.Replace("|AJSS|", " ")
                Else
                    cbAJSS.Checked = False
                End If
                ...
                If InStr(leftovers, "|NGSA|", CompareMethod.Text) > 0 Then
                    cbNGSA.Checked = True
                    leftovers = leftovers.Replace("|NGSA|", " ")
                Else
                    cbNGSA.Checked = False
                End If

                If InStr(leftovers, "|NCI|", CompareMethod.Text) > 0 Then
                    cbNCI.Checked = True
                    leftovers = leftovers.Replace("|NCI|", " ")
                Else
                    cbNCI.Checked = False
                End If
    ...
    "(select * from DWH.ProFee.FeeSchedule where FeeSchedID = " & postFS.ToString & " and TIN = 'AGA') "
            ...
            If cbAJSS.Checked Then
                Tinlist = Tinlist + "'AJSS', "
                insertsql = insertsql + "insert into [DWH].[ProFee].[FeeSchedule] ([FeeSchedID], [FeeSchedName],[PayorName],[EffectiveDate],[EndDate]" & _
    ",[Default],[LoadDate],[TIN],[ManagersNotes]) Select " & postFS.ToString & ", '" & Replace(txtFSName.Text.ToString, "'", "''") & "', '" & Replace(txtPayorName.Text.ToString, "'", "''") & _
    "', '" & Replace(txtCurrentED.Text, "'", "''") & "', '" & Replace(txtCurrentEndD.Text, "'", "''") & "', '" & Replace(txtCurrentDefault.Text, "'", "''") & _
    "', '" & fakelableLoadDate.Text.ToString & "', 'AGA', '" & Replace(txtCurrentMN.Text, "'", "''") & "' where not exists " & _
    "(select * from DWH.ProFee.FeeSchedule where FeeSchedID = " & postFS.ToString & " and TIN = 'AJSS') "
            End If
    ...
    "(select * from DWH.ProFee.FeeSchedule where FeeSchedID = " & postFS.ToString & " and TIN = 'MANG (PCP Only)') "
            ...
            If cbNAEC.Checked Then
                Tinlist = Tinlist + "'NAEC/TEG', "
                insertsql = insertsql + "insert into [DWH].[ProFee].[FeeSchedule] ([FeeSchedID], [FeeSchedName],[PayorName],[EffectiveDate],[EndDate]" & _
    ",[Default],[LoadDate],[TIN],[ManagersNotes]) Select " & postFS.ToString & ", '" & Replace(txtFSName.Text.ToString, "'", "''") & "', '" & Replace(txtPayorName.Text.ToString, "'", "''") & _
    "', '" & Replace(txtCurrentED.Text, "'", "''") & "', '" & Replace(txtCurrentEndD.Text, "'", "''") & "', '" & Replace(txtCurrentDefault.Text, "'", "''") & _
    "', '" & fakelableLoadDate.Text.ToString & "', 'NAEC/TEG', '" & Replace(txtCurrentMN.Text, "'", "''") & "' where not exists " & _
    "(select * from DWH.ProFee.FeeSchedule where FeeSchedID = " & postFS.ToString & " and TIN = 'NAEC/TEG') "
            End If

            If cbNGDC.Checked Then
                Tinlist = Tinlist + "'NGDC', "
                insertsql = insertsql + "insert into [DWH].[ProFee].[FeeSchedule] ([FeeSchedID], [FeeSchedName],[PayorName],[EffectiveDate],[EndDate]" & _
    ",[Default],[LoadDate],[TIN],[ManagersNotes]) Select " & postFS.ToString & ", '" & Replace(txtFSName.Text.ToString, "'", "''") & "', '" & Replace(txtPayorName.Text.ToString, "'", "''") & _
    "', '" & Replace(txtCurrentED.Text, "'", "''") & "', '" & Replace(txtCurrentEndD.Text, "'", "''") & "', '" & Replace(txtCurrentDefault.Text, "'", "''") & _
    "', '" & fakelableLoadDate.Text.ToString & "', 'NGDC', '" & Replace(txtCurrentMN.Text, "'", "''") & "' where not exists " & _
    "(select * from DWH.ProFee.FeeSchedule where FeeSchedID = " & postFS.ToString & " and TIN = 'NGDC') "
            End If

            If cbSEPC.Checked Then
                Tinlist = Tinlist + "'SEPC', "
                insertsql = insertsql + "insert into [DWH].[ProFee].[FeeSchedule] ([FeeSchedID], [FeeSchedName],[PayorName],[EffectiveDate],[EndDate]" & _
    ",[Default],[LoadDate],[TIN],[ManagersNotes]) Select " & postFS.ToString & ", '" & Replace(txtFSName.Text.ToString, "'", "''") & "', '" & Replace(txtPayorName.Text.ToString, "'", "''") & _
    "', '" & Replace(txtCurrentED.Text, "'", "''") & "', '" & Replace(txtCurrentEndD.Text, "'", "''") & "', '" & Replace(txtCurrentDefault.Text, "'", "''") & _
    "', '" & fakelableLoadDate.Text.ToString & "', 'SEPC', '" & Replace(txtCurrentMN.Text, "'", "''") & "' where not exists " & _
    "(select * from DWH.ProFee.FeeSchedule where FeeSchedID = " & postFS.ToString & " and TIN = 'SEPC') "
            End If
    ...
    "(select * from DWH.ProFee.FeeSchedule where FeeSchedID = " & postFS.ToString & " and TIN = 'USA') "
            ...
            ' NGSA and NCI added 8/21/2019 CRW re Caitlin Rodgers
            If cbNGSA.Checked Then
                Tinlist = Tinlist + "'NGSA', "
                insertsql = insertsql + "insert into [DWH].[ProFee].[FeeSchedule] ([FeeSchedID], [FeeSchedName],[PayorName],[EffectiveDate],[EndDate]" & _
    ",[Default],[LoadDate],[TIN],[ManagersNotes]) Select " & postFS.ToString & ", '" & Replace(txtFSName.Text.ToString, "'", "''") & "', '" & Replace(txtPayorName.Text.ToString, "'", "''") & _
    "', '" & Replace(txtCurrentED.Text, "'", "''") & "', '" & Replace(txtCurrentEndD.Text, "'", "''") & "', '" & Replace(txtCurrentDefault.Text, "'", "''") & _
    "', '" & fakelableLoadDate.Text.ToString & "', 'NGSA', '" & Replace(txtCurrentMN.Text, "'", "''") & "' where not exists " & _
    "(select * from DWH.ProFee.FeeSchedule where FeeSchedID = " & postFS.ToString & " and TIN = 'NGSA') "
            End If

            If cbNCI.Checked Then
                Tinlist = Tinlist + "'NCI', "
                insertsql = insertsql + "insert into [DWH].[ProFee].[FeeSchedule] ([FeeSchedID], [FeeSchedName],[PayorName],[EffectiveDate],[EndDate]" & _
    ",[Default],[LoadDate],[TIN],[ManagersNotes]) Select " & postFS.ToString & ", '" & Replace(txtFSName.Text.ToString, "'", "''") & "', '" & Replace(txtPayorName.Text.ToString, "'", "''") & _
    "', '" & Replace(txtCurrentED.Text, "'", "''") & "', '" & Replace(txtCurrentEndD.Text, "'", "''") & "', '" & Replace(txtCurrentDefault.Text, "'", "''") & _
    "', '" & fakelableLoadDate.Text.ToString & "', 'NCI', '" & Replace(txtCurrentMN.Text, "'", "''") & "' where not exists " & _
    "(select * from DWH.ProFee.FeeSchedule where FeeSchedID = " & postFS.ToString & " and TIN = 'NCI') "
            End If
        ...
        Try
            ...
            Else
                ...
                Try
                    ...
                    ' Update 10/30/2017 CRW

                    Sql2 = "select FeeSchedID as [ID], PayorName as [Payor Name], FeeSchedName as [FeeSched Name], convert(varchar, EffectiveDate, 107) as [Effective Date], convert(varchar, EndDate, 107) as [End Date], [Default], ManagersNotes as [Manager's Notes] from DWH.ProFee.FeeSchedule " & _
                            "where TIN = '" & Replace(ddlSelectedTINp2.SelectedValue.ToString, "'", "''") & "'" & _
                            " order by PayorName, FeeSchedName, [Effective Date] desc, [Default], ManagersNotes"
                    ...
                    Using conn2 As New System.Data.SqlClient.SqlConnection(ConfigurationManager.ConnectionStrings("PRDconn").ConnectionString)
                        ...
                        p2txtCurrentMN.Text = dr("Manager's Notes").ToString
                        p2txtCurrentMN.DataBind()
                        p2lblPayorName.Text = dr("Payor Name").ToString
                        p2lblPayorName.DataBind()
                        p2lblFSName.Text = dr("FeeSched Name").ToString
    ...
    Private Sub p2UpdateButton_Click(sender As Object, e As System.EventArgs) Handles p2UpdateButton.Click
        ...
        Try
            ...
            Using conn As New System.Data.SqlClient.SqlConnection(ConfigurationManager.ConnectionStrings("PRDconn").ConnectionString)
                ...
                'temporarily turned off 10/31/2018 CRW for Stephanie's ease of use
    ...
    Private Sub gvGalenBlanks_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles gvGalenBlanks.SelectedIndexChanged
        ...
        Try
            ...
            "SELECT  top 100 *, ROW_NUMBER() over (order by INSPLAN) as rn FROM DWH.Reference.ContractID_Profee_LU  where STARINSPLANCODE is null " & _
            ...
            " and (INSPLAN_NAME = '" & Replace(ddlSelectedINSPLAN.SelectedValue.ToString, "'", "''") & "' or '" & Replace(ddlSelectedINSPLAN.SelectedValue.ToString, "'", "''") & "' = 'novalueselected')" & _
            "order by 2 ) z where rn = @num) " & _
            "Declare @INSPLANNAME varchar(max) = ltrim(rtrim((select INSPLAN_NAME from ( " & _
            "SELECT  top 100 *, ROW_NUMBER() over (order by INSPLAN) as rn FROM DWH.Reference.ContractID_Profee_LU  where STARINSPLANCODE is null " & _
            " and (INSPLAN_NAME = '" & Replace(ddlSelectedINSPLAN.SelectedValue.ToString, "'", "''") & "' or '" & Replace(ddlSelectedINSPLAN.SelectedValue.ToString, "'", "''") & "' = 'novalueselected')" & _
            "order by 2 ) z where rn = @num))) " & _
            "select top 25 " & _
            ...
            "case when @INSPLANNAME = ltrim(rtrim(INSPLAN_NAME)) then 300 when left(ltrim(rtrim(INSPLAN_NAME)), 8) = LEFT(@INSPLANNAME, 8) then 200 " & _
            "when RIGHT(ltrim(rtrim(INSPLAN_NAME)), 8) = RIGHT(@INSPLANNAME, 8) then 180 " & _
            "when INSPLAN_NAME like ('%' + left(@INSPLANNAME, CHARINDEX(' ', @INSPLANNAME)) + '%') and CHARINDEX(' ', @INSPLANNAME) > 0 then 150 " & _
            "when @INSPLANNAME like ('%' + left(ltrim(rtrim(INSPLAN_NAME)),CHARINDEX(' ', ltrim(rtrim(INSPLAN_NAME)))) + '%') and CHARINDEX(' ', ltrim(rtrim(INSPLAN_NAME))) > 0 then 150 else 0 end " & _
            " as ordering " & _
            " from  " & _
            "DWH.Reference.ContractID_Profee_LU " & _
    ...
    Private Sub btnGalenUpdate_Click(sender As Object, e As System.EventArgs) Handles btnGalenUpdate.Click
        ...
        For i As Integer = 0 To gvGalenBlanks.Rows.Count - 1
            ...
            If chk.Checked = True Then
                counter = counter + 1
                galenupdatesql = galenupdatesql & "Update DWH.Reference.ContractID_Profee_LU set STARINSPLANCODE = '" & Replace(txtgalenchosenStarInsplanCode.Text, "'", "''") & _
    ...
    Private Sub btnGalenUpdateAll_Click(sender As Object, e As System.EventArgs) Handles btnGalenUpdateAll.Click
        ...
        Try
            ...
            Else
                ...
                Dim chk As CheckBox
                Dim cmd As SqlCommand
                galenupdatesql = "Update DWH.Reference.ContractID_Profee_LU set STARINSPLANCODE = '" & Replace(txtgalenchosenStarInsplanCode.Text, "'", "''") & _
        ...
        Try
          ...
          ",[CONTRACTID_SUM_NAME]" & _
            ...
            "order by ordering desc"
                    ...
                    'ltrimrtrim added 8/4/2016 CRW.  Because I noticed it, and it bugged me.  Previous versioning below
    ...
    Private Sub ddlSelectedTINs_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles ddlSelectedTINs.SelectedIndexChanged
        ...
        End If
        Try
            Sql2 = "select PayorName as [Payor Name], FeeSchedName as [FeeSched Name], convert(varchar, EffectiveDate, 107) as [Effective Date], " & _
                " convert(varchar, EndDate, 107) as [End Date], [Default], ManagersNotes as [Manager's Notes] from DWH.ProFee.FeeSchedule " & _
            ...
            Using conn2 As New System.Data.SqlClient.SqlConnection(ConfigurationManager.ConnectionStrings("PRDconn").ConnectionString)
                ...
                txtCurrentEndD.Text = rowboat("End Date").ToString
                txtCurrentEndD.DataBind()
                txtCurrentMN.Text = rowboat("Manager's Notes").ToString
                txtCurrentMN.DataBind()
                txtPayorName.Text = rowboat("Payor Name").ToString
                txtPayorName.DataBind()
                txtFSName.Text = rowboat("FeeSched Name").ToString
                txtFSName.DataBind()
                ...
                FakelblEndDate.Text = rowboat("End Date").ToString
                FakelblEndDate.DataBind()
                FakelblManagersNotes.Text = rowboat("Manager's Notes").ToString
                FakelblManagersNotes.DataBind()
                FakelblPayorName.Text = rowboat("Payor Name").ToString
                FakelblPayorName.DataBind()
                FakelblFSName.Text = rowboat("FeeSched Name").ToString
                FakelblFSName.DataBind()
    ...
    Private Sub btnGUUpdateCode_Click(sender As Object, e As System.EventArgs) Handles btnGUUpdateCode.Click
        ...
        Try
            ...
            Dim cmd As New SqlCommand
            Dim sqlupdate As String = "Update DWH.Reference.ContractID_Profee_LU set STARINSPLANCODE = '" & txtGalenUpdatesSTARINSPLANCODE.Text & "' " & _
                " where INSPLAN = '" & lblGalenUpdatesINSPLAN.Text & "' and IDGroup = '" & lblGalenUpdatesIDGroup.Text & "' and " & _
                " INSPLAN_NAME = '" & lblGalenUpdatesINSPLAN_NAME.Text & "' "
    ...
    Private Sub ExportGridToExcel()
        ...
        'Create new gridview and add data using galenUpdate datasource
        'Dim gvGalenTemp As New GridView()
        'gvGalenTemp.DataSource = dsGVGalenUpdates
        'gvGalenTemp.DataBind()
        'gvGalenTemp.AllowPaging = False

        ''Get the Header Names from the GridView on page
        'For Each column As DataControlField In gvGalenUpdates.Columns
        '    'For Each column As DataControlField In gvGalenTemp.Columns
        '    If column.HeaderText.ToString <> "" Then
        '        dt.Columns.Add(column.HeaderText)
        '    End If
        'Next

        ''MsgBox(dt.Columns.Count) 'for testing
        ''MsgBox(gvGalenTemp.Rows.Count) 'for testing

        ''Add Records to Datatable
        'For Each row As GridViewRow In gvGalenTemp.Rows ' gvGalenUpdates.Rows
        '    dt.Rows.Add()
        '    'MsgBox(row.Cells.Count) 'for testing
        '    Dim y As Integer = 0 'Datatable Cell Index Counter
        '    For i As Integer = 1 To row.Cells.Count - 1
        '        If i = 9 OrElse i = 10 Then 'This excludes two additional datafields in the Datasource that isn't presented in the Gridview
        '            'Do Nothing
        '        Else
        '            'dt.Rows(dt.Rows.Count - 1)(i) = row.Cells(i).Text 'for testing'
        '            dt.Rows(dt.Rows.Count - 1)(y) = row.Cells(i).Text.Replace("&nbsp;", "")
        '            y = y + 1
        '        End If
        '    Next
        'Next
        ...
        'Using NPOI Libary (exports .xls)
        'Using exporter As New NpoiExport
        '    exporter.ExportDataTableToWorkbook(dt, "Galen_STARINSPLAN")

        '    Dim saveAsFileName As String = String.Format("Galen_STARINSPLAN-{0:d}.xls", DateTime.Now)
    ...
    Private Sub btnExportSTARINSPLANs_Click(sender As Object, e As EventArgs) Handles btnExportSTARINSPLANs.Click

        Try
            Dim insplanSql As String = "select INSPLAN, INSPLAN_NAME, CONTRACT_NAME from ( " & _
    "select INSPLAN, INSPLAN_NAME, CONTRACT_NAME, ROW_NUMBER() over (partition by INSPLAN order by DC_TO desc) as RN " & _
    " from DWH.dbo.ContractID_Profee_LU " & _
    " where FAC = 'A' " & _
    "group by INSPLAN, INSPLAN_NAME, CONTRACT_NAME, DC_TO " & _
    ") x " & _
    "where RN = 1"

            Dim dfs As New DataSet
            Dim dt As New DataTable
            Dim dfa As New SqlDataAdapter
            Dim cmd As New SqlCommand

            Using conn As New SqlConnection(ConfigurationManager.ConnectionStrings("PRDconn").ConnectionString)
                dfa = New SqlDataAdapter(insplanSql, conn)
                dfa.Fill(dt)

            End Using

            dt.TableName = "INSPLANs"
            ...
            Using wb As New ClosedXML.Excel.XLWorkbook()
                ...
                Response.Charset = ""
                Response.ContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                Response.AddHeader("content-disposition", "attachment;filename=ContractID_INSPLAN.xlsx")
                Using MyMemoryStream As New MemoryStream()
                    wb.SaveAs(MyMemoryStream)
                    MyMemoryStream.WriteTo(Response.OutputStream)
                    Response.Flush()
