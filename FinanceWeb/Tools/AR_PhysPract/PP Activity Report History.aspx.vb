...
Public Class PP_Activity_Report_History
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            Else
                ...
                Dim adminsql As String = "SELECT count(*) FROM DWH.AR_PP.ActivityReport_Users where UserLogin = '" & _
            ...
            If Request.QueryString("Detail_ID") IsNot Nothing Then
                ...
                '    "from DWH.AR_PP.ActivityReport ar join DWH.AR_PP.ActivityReport_Detail ad on ar.ActivityID = ad.ActivityID " & _
...
"	select * " & _
" , case when FinalActivityDate is not null then 'Closed' else (select top 1 ar.DetailStatus from DWH.AR_PP.ActivityReport_Detail ar " & _
...
"	join (select NetID as ActivityID from DWH.AR_PP.ActivityReport_Netting  " & _
"	where InitialID = @SearchID and Active = 1  " & _
"	union   " & _
"	select InitialID from DWH.AR_PP.ActivityReport_Netting  " & _
"	where NetID = @SearchID  and Active = 1  " & _
"	union  " & _
"	select TransferFrom from  (select * from DWH.AR_PP.ActivityReport_Transfers union all select * from DWH.AR_PP.ActivityReport_TransfersHistorical) x  " & _
"	where isnull(IntermediateRow, 0) = @SearchID or TransferTo = @SearchID and Active = 1  " & _
"	union   " & _
"	select TransferTo from  (select * from DWH.AR_PP.ActivityReport_Transfers union all select * from DWH.AR_PP.ActivityReport_TransfersHistorical) x  " & _
...
"left join DWH.AR_PP.ActivityReport_2_Source a2s on ar.ActivityID = a2s.ActivityID and a2s.Active = 1 " & _
"left join DWh.AR_PP.ActivityReport_Source s on a2s.Base_ID = s.Base_ID and s.Active = 1 " & _
"left join DWH.AR_PP.ActivityReport_Users u on ar.CreatedBy = u.UserLogin and u.Active = 1 " & _
"where ar.Active = 1  ) N "
...
"	from DWH.AR_PP.ActivityReport ar " & _
"	left join DWH.AR_PP.ActivityReport_2_Source two on ar.ActivityID = two.ActivityID and two.Active = 1 " & _
"	join (select NetID as ActivityID from DWH.AR_PP.ActivityReport_Netting " & _
"	where InitialID = @SearchID and Active = 1 " & _
"	union  " & _
"	select InitialID from DWH.AR_PP.ActivityReport_Netting " & _
...
"	where isnull(IntermediateRow, 0) = @SearchID or TransferFrom = @SearchID and Active = 1 " & _
"	union select @SearchID) find on ar.ActivityID = find.ActivityID " & _
"	left join DWH.AR_PP.ActivityReport_Source s on two.Base_ID = s.Base_ID and s.Active = 1 " & _
"	) N " & _
"	 " & _
...
"            From (select art.*, t.*  " & _
"	from  (select * from DWH.AR_PP.ActivityReport_Transfers union all select * from DWH.AR_PP.ActivityReport_TransfersHistorical) art " & _
"	join DWH.AR_PP.ActivityReport ar on ar.ActivityID  in (art.IntermediateRow, art.TransferFrom, art.TransferTo) and ar.Active = 1 " & _
"	left join DWH.AR_PP.ActivityReport_2_Source ars on ars.ActivityID = ar.ActivityID and ars.Active = 1 " & _
"	left join DWH.AR_PP.ActivityReport_Source s on ars.Base_ID = s.Base_ID and s.Active = 1 " & _
...
"	left join (select *, ROW_NUMBER() over (partition by UserLogin order by Active desc, DateModified desc) rn From DWH.AR_PP.ActivityReport_Users) u on u.rn = 1 and ST2.ModifyUser = u.UserLogin " & _
"	 " & _
"	union all " & _
"" & _
...
"            Select ','+ST1.LetterCode  AS [text()] " & _
"            From (select art.*, t.*  " & _
"			from DWH.AR_PP.ActivityReport_Netting art " & _
"	join DWH.AR_PP.ActivityReport ar on ar.ActivityID  in (art.InitialID, art.NetID) and ar.Active = 1 " & _
"	left join DWH.AR_PP.ActivityReport_2_Source ars on ars.ActivityID = ar.ActivityID and ars.Active = 1 " & _
"	left join DWH.AR_PP.ActivityReport_Source s on ars.Base_ID = s.Base_ID and s.Active = 1 " & _
"	join #TmpBases t on s.Base_ID = t.Base_ID or t.Base_ID = -ar.ActivityID " & _
"	where art.Active = 1) ST1 " & _
...
"From (select art.*, t.*  " & _
"	from DWH.AR_PP.ActivityReport_Splitting art " & _
"	join DWH.AR_PP.ActivityReport ar on ar.ActivityID  in (art.InitialID) and ar.Active = 1 " & _
"	left join DWH.AR_PP.ActivityReport_2_Source ars on ars.ActivityID = ar.ActivityID and ars.Active = 1 " & _
"	left join DWH.AR_PP.ActivityReport_Source s on ars.Base_ID = s.Base_ID and s.Active = 1 " & _
"	join #TmpBases t on s.Base_ID = t.Base_ID or t.Base_ID = -ar.ActivityID " & _
"	where art.Active = 1) ST2 " & _
"	left join (select *, ROW_NUMBER() over (partition by UserLogin order by Active desc, DateModified desc) rn From DWH.AR_PP.ActivityReport_Users) u on u.rn = 1 and ST2.ModifyUser = u.UserLogin " & _
"	left join DWH.AR_PP.ActivityReport_Detail ad on ST2.InitialID = ad.ActivityID and convert(date, ad.ActivityDate) <= ST2.ActivityDate " & _
"		and not exists (select * from DWH.AR_PP.ActivityReport_Detail adfake where adfake.ActivityID = ad.ActivityID and adfake.ActivityDate > ad.ActivityDate " & _
...
"	, t.LetterCode " & _
"	from DWH.AR_PP.ActivityReport ar " & _
"	join DWH.AR_PP.ActivityReport_Detail ard on ar.ActivityID = ard.ActivityID " & _
"	left join DWH.AR_PP.ActivityReport_2_Source ars on ars.ActivityID = ar.ActivityID and ars.Active = 1 " & _
"	left join DWH.AR_PP.ActivityReport_Source s on ars.Base_ID = s.Base_ID and s.Active = 1 " & _
"	join #TmpBases t on s.Base_ID = t.Base_ID or t.Base_ID = -ar.ActivityID " & _
"	left join (select *, ROW_NUMBER() over (partition by UserLogin order by Active desc, DateModified desc) rn From DWH.AR_PP.ActivityReport_Users) u on u.rn = 1 and ard.ModifiedBy = u.UserLogin " & _
"" & _
"	order by 3, 5 "
...
"	from (select * from DWH.AR_PP.ActivityReport_Transfers union all select * from DWH.AR_PP.ActivityReport_TransfersHistorical) art  " & _
"	join DWH.AR_PP.ActivityReport ar on ar.ActivityID  in (art.IntermediateRow, art.TransferFrom, art.TransferTo) and ar.Active = 1  " & _
"	left join (select *, ROW_NUMBER() over (partition by UserLogin order by Active desc, DateModified desc) rn From DWH.AR_PP.ActivityReport_Users) u on u.rn = 1 and art.ModifyUser = u.UserLogin " & _
"	left join DWH.AR_PP.ActivityReport_Detail ard on ard.Active = 1 and ar.ActivityID = ard.ActivityID and ard.ActivityDate <= art.TransferDate and ard.ModifiedDate <= art.ModifyDate " & _
"		and not exists (select * from DWH.AR_PP.ActivityReport_Detail badard where badard.activityID = ar.ActivityID and badard.Active = 1  " & _
...
"	left join DWH.AR_PP.ActivityReport_Detail ttard on ttard.Active = 1 and art.TransferTo = ttard.ActivityID and ttard.ActivityDate <= art.TransferDate  " & _
"		and not exists (select * from DWH.AR_PP.ActivityReport_Detail badard where badard.activityID = ttard.ActivityID and badard.Active = 1  " & _
...
"	left join DWH.AR_PP.ActivityReport_Detail tfard on tfard.Active = 1 and art.TransferFrom = tfard.ActivityID and tfard.ActivityDate <= art.TransferDate and tfard.ModifiedDate <= art.ModifyDate " & _
"		and not exists (select * from DWH.AR_PP.ActivityReport_Detail badard where badard.activityID = tfard.ActivityID and badard.Active = 1  " & _
"			and badard.ActivityDate <= art.TransferDate and badard.ModifiedDate <= art.ModifyDate  " & _
...
"	left join (select distinct DisplayDescription as FacDisplay, IDGroup Facility  " & _
...
"            		DWH.AR_PP.ActivityReport_MiscGL m " & _
"					join (select distinct TFID, TransferDate, ModifyDate from DWH.AR_PP.ActivityReport_Transfers where Active = 1) t on convert(date, m.ActivityDate) <= t.TransferDate " & _
...
"            left join (select t.TFID, TransferFrom, sum(Amount) TransferredOut, convert(Date, tt.TransferDate) as TFDate from DWH.AR_PP.ActivityReport_Transfers tt " & _
"				join (select distinct TFID, TransferDate, ModifyDate from DWH.AR_PP.ActivityReport_Transfers where Active = 1) t  " & _
...
"            left join (select t.TFID, TransferTo, sum(Amount) TransferredIn, convert(date, tf.TransferDate) as TFTDate from DWH.AR_PP.ActivityReport_Transfers  tf " & _
"				join (select distinct TFID, TransferDate, ModifyDate from DWH.AR_PP.ActivityReport_Transfers where Active = 1) t  " & _
...
"	) ST2  " & _
"	/*left join DWH.AR_PP.ActivityReport_Detail ard on ard.Active = 1 and ChangeType <> 'Row Created by Netting Others' and ST2.ActivityID = ard.ActivityID and ard.ActivityDate <= ST2.ActivityDate  " & _
"		and ard.ModifiedDate <= ST2.ModifyDate " & _
"		and not exists (select * from DWH.AR_PP.ActivityReport_Detail badard where badard.activityID = ST2.ActivityID and badard.Active = 1  " & _
"			and badard.ActivityDate <= ST2.ActivityDate and badard.ModifiedDate <= ST2.ModifyDate  " & _
"			and (badard.ActivityDate > ard.ActivityDate or (badard.ActivityDate = ard.ActivityDate and badard.ModifiedDate > ard.ModifiedDate))) */ " & _
"	left join DWH.AR_PP.ActivityReport_Detail neward on neward.Active = 1 and ChangeType = 'Row Created by Netting Others' and ST2.ActivityID = neward.ActivityID  " & _
...
"            left join (select  TransferFrom, sum(Amount) TransferredOut, convert(Date, tt.TransferDate) as TFDate from DWH.AR_PP.ActivityReport_Transfers tt " & _
"            		where Active = 1    " & _
...
"            left join (select  TransferTo, sum(Amount) TransferredIn, convert(date, tf.TransferDate) as TFTDate from DWH.AR_PP.ActivityReport_Transfers  tf " & _
"            		where Active = 1 " & _
...
"left join DWH.AR_PP.ActivityReport_2_Source a2s on ar.ActivityID = a2s.ActivityID and a2s.Active = 1 " & _
"left join DWh.AR_PP.ActivityReport_Source s on a2s.Base_ID = s.Base_ID and s.Active = 1 " & _
"left join (select *, ROW_NUMBER() over (partition by UserLogin order by Active desc, DateModified desc) rn From DWH.AR_PP.ActivityReport_Users) u on ar.CreatedBy = u.UserLogin and u.rn = 1 " & _
