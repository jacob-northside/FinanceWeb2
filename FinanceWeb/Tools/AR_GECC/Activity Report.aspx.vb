...
Public Class Activity_Report1
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Else
            Try
                ...
                ExecuteSql("Update DWH.AR_GECC.ActivityReport_Detail set Carry_Forward = null where Carry_Forward = 0")
    ...
    Private Sub GetAdminStatus()
        Try
            ...
            'Dim s As String = "select count(*) from DWH.AR_GECC.Activity_Users " & _
            ...
            Dim s As String = "select count(*) from DWH.AR_GECC.ActivityReport_Users " & _
            ...
            If GetScalar("select count(*) from DWH.AR_GECC.ActivityReport_Users " & _
            ...
            Dim s2 As String = "select count(*) from DWH.AR_GECC.ActivityReport_Users " & _
            ...
            Dim r As String = "select count(*) from DWH.AR_GECC.ActivityReport_Users " & _
            ...
            Dim x As String = "select isnull(max(isnull(MultipleLocks, 2)), 1) from DWH.AR_GECC.ActivityReport_Users " & _
            ...
            Dim f As String = "select isnull(max(isnull(Netting, 2)), 1) from DWH.AR_GECC.ActivityReport_Users " & _
                "where UserLogin = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & "' and Active = 1 "
    ...
    Private Sub RefreshDates()
        ...
        Try
            ...
            Dim RunDate As String = "(select Value from DWH.AR_GECC.ActivityReport_ExtraValues where Description = 'RunDate' and Active = 1)"
    ...
    Sub UserDropDown()
        ...
        'Dim s As String = "select UserLogin, UserDisplayName, 1 as ord from DWH.AR_GECC.Activity_Users " & _
        '"where Active = 1 " & _
        ...
        '"from DWH.AR_GECC.Activity_Detail ad " & _
        '"left join DWH.AR_GECC.Activity_Users au on ad.AssignedUser = au.UserLogin   " & _
        '"where ad.Active = 1 and ActivityDate = '" & Trim(Replace(txtARDate.Text, "'", "''")) & "' " & _
        '"order by 3, 2 "

        Dim s As String = "select UserLogin, isnull(isnull(UserDropDownListName, left(UserFullName, 10)), UserLogin) as UserDisplayName, 1 as ord from DWH.AR_GECC.ActivityReport_Users " & _
        "where Active = 1 " & _
        "  union select distinct ' - View All - ' , '- View All -', -1 union select distinct 'Unassigned' , '- Unassigned -', 0 union " & _
        ...
        "join DWH.AR_GECC.ActivityReport_Assignments aa on ar.ActivityID = aa.ActivityID " & _
        "left join DWH.AR_GECC.ActivityReport_Users au on aa.UserAssigned = au.UserLogin " & _
    ...
    Sub InitialPopulateCheckboxes()
        ...
        'Dim s As String = "select distinct CashCategory, 'True' as Checked from DWH.AR_GECC.Activity_Detail " & _
        '"where Active = 1 " & _
        '"order by 1 "

        Dim s As String = "select distinct CashCategory, InitiallySelected as Checked from DWH.AR_GECC.ActivityReport_CashCategory " & _
         "where Active = 1 " & _
         "order by 2 desc, 1 "
        ...
        '        Dim s2 As String = "select distinct CategoryStatus, case when CategoryStatus = 'Current' then 'True' else 'False' end as Checked from DWH.AR_GECC.Activity_Detail " & _
        '"where Active = 1 union select 'Research', 'False' where " & Research & " > 0 " & _
        '"order by 1 "

        Dim s2 As String = "select distinct DetailStatus from DWH.AR_GECC.ActivityReport_Detail " & _
        "order by 1 "
        ...
        'Dim s3 As String = "select distinct isnull(Facility, 'Unassigned') as Facility, 'True' as Checked from DWH.AR_GECC.Activity_Detail " & _
        '"where Active = 1 " & _
        '"order by 1 "
        ...
        "from DWH.AR_GECC.ActivityReport_BankAccountNumber_LU where Active = 1 union select '(Select Facility)', ' -- NoneSelected -- ', 0 " & _
        "order by ord, 1 "
...
"from DWH.AR_GECC.ActivityReport_CashCategory_Type_LU where Active = 1 and CashCategory = 'Cash Journal' union select '(Select Type)', ' -- NoneSelected -- ', 0 " & _
"order by ord, 1 "
    ...
    Private Function ARDetailView()
        ...
        If Len(Trim(txtSrch.Text)) > 0 Then
            SearchString = "declare @SearchWord varchar(max) = '" & Replace(Trim(txtSrch.Text), "'", "''") & "'  " & _
                    ...
                    "into #TempFound " & _
                    "from DWH.AR_GECC.ActivityReport_Detail with (nolock)  " & _
                    ...
                    "from DWH.AR_GECC.ActivityReport_AttachedFiles with (nolock)  " & _
                    ...
                    "from DWH.AR_GECC.ActivityReport_Comments with (nolock)  " & _
                    ...
                    "from DWH.AR_GECC.ActivityReport_MiscGL  with (nolock) " & _
                    "where Active = 1 and isnull(convert(varchar, Comment), '') <> ''  " & _
                    "union all " & _
                    ...
                    "from DWH.AR_GECC.ActivityReport_Netting  with (nolock) " & _
                    ...
                    "from DWH.AR_GECC.ActivityReport_Transfers  with (nolock) " & _
                    "where Active = 1 and isnull(convert(varchar, Amount), '') <> ''  " & _
                    "union all " & _
...
", case when @SearchDate <> (select Value from DWH.AR_GECC.ActivityReport_ExtraValues with (nolock)  where Description = 'RunDate' and Active = 1) then 2 " & _
...
"from DWH.AR_GECC.ActivityReport ar  with (nolock)" & _
"left join DWH.AR_GECC.ActivityReport_Locking al with (nolock)  on ar.ActivityID = al.ActivityID and al.Active = 1 " & _
"left join  DWH.AR_GECC.ActivityReport_Users usr with (nolock)  on al.UserLocked = usr.UserLogin and usr.Active = 1 " & _
"join (select *, ROW_NUMBER() over (partition by ActivityID order by ActivityDate desc, ModifiedDate desc) RN  from  " & _
"		DWH.AR_GECC.ActivityReport_Detail with (nolock)  where Active = 1 and convert(date, ActivityDate) <= @SearchDate) ad on ar.ActivityID = ad.ActivityID and ad.RN = 1 " & _
...
"		DWH.AR_GECC.ActivityReport_Comments with (nolock)  where Active = 1 and CommentType is null and convert(date, ActivityDate) <= @SearchDate) UnCom on ar.ActivityID = UnCom.ActivityID and UnCom.RN = 1 " & _
"left join (select ActivityID, sum(MiscAmt) as MiscAmt, convert(date, ActivityDate) as ActivityDate from  " & _
"		DWH.AR_GECC.ActivityReport_MiscGL with (nolock)  where Active = 1 and convert(date, ActivityDate) <= @SearchDate group by ActivityID, convert(date, ActivityDate)) mgl on ar.ActivityID = mgl.ActivityID " & _
" and mgl.ActivityDate = ad.ActivityDate " & _
"left join (select TransferFrom, sum(Amount) TransferredOut, convert(date, TransferDate) as TFDate from DWH.AR_GECC.ActivityReport_Transfers with (nolock)  " & _
...
"left join (select TransferTo, sum(Amount) TransferredIn, convert(date, TransferDate) as TFTDate from DWH.AR_GECC.ActivityReport_Transfers with (nolock)  " & _
"		where Active = 1 and convert(date, TransferDate) <= @SearchDate " & _
"		group by TransferTo, convert(date, TransferDate)) tft on tft.TransferTo = ar.ActivityID and TFTDate = ad.ActivityDate " & _
"left join (select max(AttachId) AttachId, ActivityId from DWH.AR_GECC.ActivityReport_AttachedFiles  with (nolock)  " & _
"	where Active = 1 and convert(date, ActivityDate) <= @SearchDate group by ActivityID) atch on atch.ActivityId = ar.ActivityID " & _
"left join DWH.AR_GECC.ActivityReport_Assignments ass with (nolock)  on ass.ActivityID = ar.ActivityID " & _
"left join DWH.AR_GECC.ActivityReport_Users assu with (nolock)  on ass.UserAssigned = assu.UserLogin " & _
"left join (select case when count(ar.ActivityID) >= MultipleLocks then 2 else 0 end as Locking  " & _
"from DWH.AR_GECC.ActivityReport_Users usr with (nolock)  " & _
"left join DWH.AR_GECC.ActivityReport_Locking al with (nolock)  on al.UserLocked = usr.UserLogin and al.Active = 1  " & _
"left join DWH.AR_GECC.ActivityReport ar with (nolock)  on ar.ActivityID = al.ActivityID  " & _
...
"					From DWH.AR_GECC.ActivityReport_Comments ST1 " & _
"					Where ST1.ActivityID = ST2.ActivityID " & _
...
"		From DWH.AR_GECC.ActivityReport_Comments ST2  with (nolock)" & _
"       join DWH.AR_GECC.ActivityReport ar2 with (nolock) on ST2.ActivityID = ar2.ActivityID " & _
...
"where @SearchDate between FirstActivityDate and isnull(FinalActivityDate, '12/31/9999') and ar.Active = 1 and not exists (select * from DWH.AR_GECC.ActivityReport_Netting net with (nolock)  where net.InitialID = ar.ActivityID and convert(date, net.ActivityDate) <= @SearchDate " & _
...
"	) zq " & JoinString & " where 1=1  " & ResearchQ & Balanced & ") "
        ...
        '            "and not exists (select * from DWH.AR_GECC.ActivityReport_Splitting split where split.InitialID = ar.ActivityID and convert(date, split.ActivityDate) <= @SearchDate " & _
        ...
        '    "from DWH.AR_GECC.Activity_Detail ad " & _
        '    "left join (select max(FileID) FileID, Detail_ID from DWH.AR_GECC.Activity_Detail_AttachedFiles group by Detail_ID) af on ad.Detail_ID  = af.Detail_ID " & _
        '    "left join DWH.AR_GECC.Activity_Users u on ad.Locked = u.UserLogin  " & _
        '    "left join (select case when count(ad.Locked) >= MultipleLocks then 2 else 0 end as Locking  " & _
        '    "from DWH.AR_GECC.Activity_Users au " & _
        '    "left join DWH.AR_GECC.Activity_Detail ad on ad.Locked = au.UserLogin " & _
        ...
        '    "from DWH.AR_GECC.Activity_Transfers at " & _
        '    "join DWH.AR_GECC.Activity_Detail tf on (at.TransferFrom = tf.Detail_ID or (at.IntermediateRow = tf.Detail_ID and isnull(IntermediateActive, 0)= 1)) " & _
        '    "join DWH.AR_GECC.Activity_Detail tt on at.TransferTo = tt.Detail_ID " & _
        ...
        '    "from DWH.AR_GECC.Activity_Transfers at " & _
        '    "join DWH.AR_GECC.Activity_Detail tf on at.TransferFrom = tf.Detail_ID " & _
        '    "join DWH.AR_GECC.Activity_Detail tt on (at.TransferTo = tt.Detail_ID or (at.IntermediateRow = tt.Detail_ID and isnull(IntermediateActive, 0) = 1)) " & _
        '    "where at.Active = 1 and tf.Active = 1 and tt.Active = 1 " & _
        '    "group by tt.Detail_ID ) tt on tt.TTID = ad.Detail_ID " & _
        '    "where ad.Active = 1 and not exists (select * from DWH.AR_GECC.Activity_Netting ne where ne.InitialID = ad.Row_ID  ) and " & _
    ...
    Sub BalanceCheck()
        ...
        Try
            ...
            ", isnull(isnull(isnull(assu.UserDropDownListName, assu.UserFullName), ass.UserAssigned), 'Unassigned') as AssignedDisplay " & _
            ", isnull(ass.UserAssigned, 'Unassigned') as AssignedUser   " & _
            ", case when @SearchDate <> (select Value from DWH.AR_GECC.ActivityReport_ExtraValues where Description = 'RunDate' and Active = 1) then 2 " & _
            "   when isnull(al.UserLocked, '--__NULL__--') = '" & Mimic.Text & "' " & _
            " then 1 when al.UserLocked is not null then -1 else isnull(Locking, 2) end as EditIndex " & _
            " , 'Locked - (' + isnull(isnull(usr.UserDropDownListName, usr.UserFullName), al.UserLocked) + ')' as RowLocked  " & _
            ", MiscAmt, case when MiscAmt <> 0 then 1 else 0 end as MiscChecked " & _
            ...
            "left join (select case when count(ar.ActivityID) >= MultipleLocks then 2 else 0 end as Locking  " & _
            "from DWH.AR_GECC.ActivityReport_Users usr with (nolock)  " & _
            "join DWH.AR_GECC.ActivityReport_Locking al with (nolock)  on al.UserLocked = usr.UserLogin and al.Active = 1  " & _
            "left join DWH.AR_GECC.ActivityReport ar with (nolock)  on ar.ActivityID = al.ActivityID  " & _
            "where usr.UserLogin = '" & Mimic.Text & "' and usr.Active = 1   " & _
            "group by MultipleLocks) x on 1=1   " & _
            "where @SearchDate between FirstActivityDate and isnull(FinalActivityDate, '12/31/9999') and ar.Active = 1 and not exists (select * from DWH.AR_GECC.ActivityReport_Netting net with (nolock)  where net.InitialID = ar.ActivityID and convert(date, net.ActivityDate) <= @SearchDate " & _
...
#Region "ARData"
    ...
    Private Sub gv_AR_MainData_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles gv_AR_MainData.RowDataBound
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            HistoryBtn.OnClientClick = "javascript:window.open(""" & "AR GECC History/?Detail_ID=" & lbl.Text.ToString & """, 'AR GECC Activity Detail History', 'height=700, scrollbars, resizable');"
            ...
            If e.Row.DataItem("EditIndex") = "1" Then
                ...
                If attch = 1 Then
                    ...
                    AttachBtn.OnClientClick = "javascript:window.open(""" & "AR GECC Attachments/?Detail_ID=" & lbl.Text.ToString & """, 'AR GECC Activity Detail Attachments', 'height=700,width=620, scrollbars, resizable');"
                ...
                If lblARCategory.Text = "Cash Journal" Then
                    ...
                    'Dim ARTypes As String = "select isnull(SelectionDisplay, SelectionValue) as DisplaySelection, SelectionValue, 1 as ord from DWH.AR_GECC.Activity_Selections " & _
                    ...
                    Dim ARTypes As String = "select isnull(isnull(DropDownListDisplay, FullDisplay), Type) as DisplaySelection, Type, 1 as ord from DWH.AR_GECC.ActivityReport_CashCategory_Type_LU " & _
                    ...
                    " union select '" & Replace(lblARType.Text, "'", "''") & "', '" & Replace(lblARType.Text, "'", "''") & _
                        "', 0 where not exists (select * from DWH.AR_GECC.ActivityReport_CashCategory_Type_LU where CashCategory = 'Cash Journal' and Type = '" & Trim(Replace(lblARType.Text, "'", "''")) & "') " & _
                    "order by 3, 2 "
                ...
                'Dim GLNos As String = "select isnull(DropDownDisplay, FullDisplay) as DisplaySelection, Misc_Identity, 1 as ord from DWH.AR_GECC.ActivityReport_MiscGLCodes_LU " & _
                ...
                Dim CC As String = "select distinct CashCategory from DWH.AR_GECC.ActivityReport_CashCategory " & _
            ...
            ElseIf e.Row.DataItem("EditIndex") = "0" Then
                If attch = 0 Then
                    If e.Row.RowIndex Mod 2 = 0 Then
                        ...
                        AttachBtn.OnClientClick = "javascript:window.open(""" & "AR GECC Attachments/?Detail_ID=" & lbl.Text.ToString & """, 'AR GECC Attachments', 'height=700,width=620, scrollbars, resizable');"
                        AttachBtn.Visible = True
                    Else
            ...
            ElseIf e.Row.DataItem("EditIndex") = "2" Then
                If attch = 0 Then
                    If e.Row.RowIndex Mod 2 = 0 Then
                        Dim AttachBtn As ImageButton = e.Row.FindControl("imgblue")
                        AttachBtn.OnClientClick = "javascript:window.open(""" & "AR GECC Attachments/?Detail_ID=" & lbl.Text.ToString & """, 'AR Activity Detail Attachments', 'height=700,width=620, scrollbars, resizable');"
                        AttachBtn.Visible = True
                    Else
                        Dim AttachBtn As ImageButton = e.Row.FindControl("imgwhite")
                        AttachBtn.OnClientClick = "javascript:window.open(""" & "AR GECC Attachments/?Detail_ID=" & lbl.Text.ToString & """, 'AR Activity Detail Attachments', 'height=700,width=620, scrollbars, resizable');"
            ...
            '        Dim s As String = "select UserLogin, isnull(isnull(UserDropDownListName, left(UserFullName, 12)), UserLogin) as UserDisplayName, 1 as ord from DWH.AR_GECC.ActivityReport_Users " & _
            ...
            Dim s As String = "select Upper(UserLogin) as UserLogin, isnull(isnull(UserDropDownListName, left(UserFullName, 12)), UserLogin) as UserDisplayName, 1 as ord from DWH.AR_GECC.ActivityReport_Users " & _
                "where Active = 1 and Assignable = 1 or UserLogin = '" & Trim(Replace(lblARAssignedUser.Text, "'", "''")) & "' " & _
...
#End Region
    ...
    Private Sub gv_AR_MainData_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gv_AR_MainData.RowCommand
        ...
        Try
            ...
            If Commander = "Transfers" Then
                ...
                Dim PrepSQL As String = "delete from DWH.AR_GECC.ActivityReport_StagingTransfers " & _
                "where ModifyUser = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "' " & _
                " " & _
                "insert into DWH.AR_GECC.ActivityReport_StagingTransfers " & _
                ...
                "from DWH.AR_GECC.ActivityReport_Transfers art " & _
                "left join DWH.AR_GECC.ActivityReport_Detail ard on art.TransferFrom = ard.ActivityID and ard.Active = 1 " & _
                "	and ard.ActivityDate <= '" & Trim(Replace(txtARDate.Text, "'", "''")) & "' " & _
                "and not exists (select * from DWH.AR_GECC.ActivityReport_Detail ard2  " & _
                ...
                '"from DWH.AR_GECC.ActivityReport_Transfers " & _
            ...
            ElseIf Commander = "EditRow" Then
                ...
                Dim sql As String = "Update DWH.AR_GECC.ActivityReport_Locking " & _
                    ...
                    " " & _
                    "insert into DWH.AR_GECC.ActivityReport_Locking " & _
                    "select '" & Detail_ID & "', '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "', getdate(), 1 " & _
                    "where not exists (select * from DWH.AR_GECC.ActivityReport_Locking where ActivityID = '" & Detail_ID & "' ) "
            ...
            ElseIf Commander = "SplitRow" Then
                ...
                '    "		from DWH.AR_GECC.ActivityReport_Detail where Active = 1 and ActivityDate <= @SearchDate and ActivityID = " & Detail_ID & ") x " & _
                '    "		where x.RN = 1 "
                ...
                Dim s2 As String = "declare @SearchDate date = ' " & Trim(Replace(txtARDate.Text, "'", "''")) & "'  " & _
                    ...
                    "from DWH.AR_GECC.ActivityReport_Detail ard " & _
                    "join DWH.AR_GECC.ActivityReport ar on ard.ActivityID = ar.ActivityID and ar.Active = 1 " & _
                    "left join ( select sum(Amount) TransferAmt from DWH.AR_GECC.ActivityReport_Transfers " & _
                ...
                'Dim s2 As String = "select convert(varchar, isnull(WF_Cash_Received, 0.00)) from DWH.AR_GECC.Activity_Detail where Row_ID = " & Detail_ID
        ...
        For i As Integer = 0 To gv_AR_MainData.Rows.Count - 1
            ...
            If gv_AR_MainData.DataKeys(i)("ActivityID").ToString = detid.ToString Then
                ...
                ElseIf txtARSTARBatchNo.Visible = True And Len(Trim(txtARSTARBatchNo.Text)) <> 3 Then
                    explanationlabel.Text = Trim(Replace(Replace(txtARSTARBatchNo.Text, "'", ""), ",", "")) & " -- Batch No must be 4 digits "
                    ModalPopupExtender.Show()
                    Exit Sub
                ElseIf txtARSTARBatchNo.Visible = True And Integer.TryParse(txtARSTARBatchNo.Text, False) = False Then
                    explanationlabel.Text = Trim(Replace(Replace(txtARSTARBatchNo.Text, "'", ""), ",", "")) & " (Batch No) must be an integer "
                    ModalPopupExtender.Show()
                    Exit Sub
                Else
                ...
                'MiscPosted = "(select sum(MiscAmt) from DWH.AR_GECC.ActivityReport_MiscGL where Active = 1 and ActivityID = '" & _
                ...
                PrepSql += "update a " & _
                    ...
                    "from DWH.AR_GECC.ActivityReport_MiscGL a " & _
                    "left join DWH.AR_GECC.ActivityReport_StagingMiscGL b  on a.ActivityID = b.ActivityId " & _
                    ...
                    "insert into DWH.AR_GECC.ActivityReport_MiscGL (ActivityId, Misc_Identity, ActivityDate, Comment, UserID, ModifiedDate, Active, MiscAmt) " & _
                    "select ActivityId, Misc_Identity, ActivityDate, Comment, UserID, ModifiedDate, Active, MiscAmt " & _
                    "from DWH.AR_GECC.ActivityReport_StagingMiscGL b " & _
                    "where Active = 1 and not exists (select * from DWH.AR_GECC.ActivityReport_MiscGL a " & _
                ...
                If lblCategory.Text = "Current" Then
                    If cbARCarryForward.Checked Then
                        ...
                        CR = "isnull(" & CashRec & ", 0) - isnull(" & ARPosted & ", 0) - isnull(" & MiscPosted & ", 0) - isnull(" & interest & ", 0)" & _
                            " - isnull((select sum(Amount) from DWH.AR_GECC.ActivityReport_Transfers " & _
                    ...
                    If cbARUnresolved.Checked Then
                        ...
                        Dim CheckAttachments As String = "Select count(*) from DWH.AR_GECC.ActivityReport_AttachedFiles where Active = 1 and ActivityID = '" & gv_AR_MainData.DataKeys(i)("ActivityID").ToString & "' and len(Content) > 0 "
                        ...
                        Dim CheckUR As String = "Select count(*) from DWH.AR_GECC.ActivityReport_Comments where Active = 1 and ActivityID = '" & gv_AR_MainData.DataKeys(i)("ActivityID").ToString & "'"
                        If GetScalar(CheckAttachments) = 0 Then
                ...
                'UpdatesSql = "Insert into DWH.AR_GECC.Activity_Detail_History SELECT Row_ID, Detail_ID " & _
                '    "      ,Base_ID " & _
                '    "      ,ActivityDate " & _
                '    "     ,Active " & _
                '    "     ,AssignedUser " & _
                ...
                '    "      ,Research       " & _
                '    "      ,Misc_GL_Text       " & _
                '    "  FROM DWH.AR_GECC.Activity_Detail " & _
                ...
                'UpdatesSql += " Update DWH.AR_GECC.Activity_Detail set Batch_Number = " & BatchNo & ", STAR_Batch_Number = " & STARBatchNo & _
                ...
                UpdatesSql += "Insert into DWH.AR_GECC.ActivityReport_Detail values ('" & gv_AR_MainData.DataKeys(i)("ActivityID").ToString() & "', '" & _
                    ...
                    "Update DWH.AR_GECC.ActivityReport_Locking set Active = 0 where ActivityID = '" & gv_AR_MainData.DataKeys(i)("ActivityID").ToString() & "' "

                If Trim(Replace(txtARComments.Text, "'", "''")) = "" Then
                    UpdatesSql += "  Update DWH.AR_GECC.ActivityReport_Comments set Active = 0, UserID = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & _
                        "', CommentDate = getdate() where ActivityID = '" & _
                    gv_AR_MainData.DataKeys(i)("ActivityID").ToString() & "' and Active = 1 and CommentType is null "
                Else
                    UpdatesSql += "Insert into DWH.AR_GECC.ActivityReport_Comments select '" & gv_AR_MainData.DataKeys(i)("ActivityID").ToString() & "', '" & Trim(Replace(txtARDate.Text, "'", "''")) & _
                    "', '" & Replace(txtARComments.Text, "'", "''") & "', null, '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "', getdate(), 1 " & _
                    "where not exists (select * from DWH.AR_GECC.ActivityReport_Comments where ActivityID = '" & gv_AR_MainData.DataKeys(i)("ActivityID").ToString() & _
                    "' and CommentType is null and Active = 1 )  Update DWH.AR_GECC.ActivityReport_Comments set Comment = '" & Replace(txtARComments.Text, "'", "''") & _
                ...
                'If MiscGLNos <> "null" Then
                '    UpdatesSql += "Update DWH.AR_GECC.ActivityReport_MiscGL set Active = 0 where ActivityID = '" & _
                '        gv_AR_MainData.DataKeys(i)("ActivityID").ToString() & "' and Active = 1 and NOT (Comment " & eMiscGLTxt & " and Misc_Identity " & eMiscGLNos & ") " & _
                '        "Insert into DWH.AR_GECC.ActivityReport_MiscGL select '" & gv_AR_MainData.DataKeys(i)("ActivityID").ToString() & "', " & MiscGLNos & ", " & _
                '        "'" & Trim(Replace(txtARDate.Text, "'", "''")) & "', " & MiscGLTxt & ", '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "', getdate(), 1 " & _
                '        "where not exists (select * from DWH.AR_GECC.ActivityReport_MiscGL where ActivityID = '" & gv_AR_MainData.DataKeys(i)("ActivityID").ToString() & _
                '        "' and Active = 1 )   "
                'End If
    ...
    '    '"from DWH.AR_GECC.Activity_Transfers at " & _
    '    '"join DWH.AR_GECC.Activity_Detail tf on at.TransferFrom = tf.Detail_ID " & _
    '    '"join DWH.AR_GECC.Activity_Detail tt on at.IntermediateRow = tt.Detail_ID " & _
    '    '"where at.Active = 1 and tf.Active = 1 and tt.Active = 1 " & _
    '    '"and tt.Row_ID = " & detid & " and isnull(at.IntermediateActive, 0) = 1 " & _
    '    '        "union all " & _
    '    '"select tf.Facility, tt.Facility, at.Amount, tf.Detail_ID as TFID,  tt.Detail_ID as TTID " & _
    '    '"from DWH.AR_GECC.Activity_Transfers at " & _
    '    '"join DWH.AR_GECC.Activity_Detail tf on at.IntermediateRow = tf.Detail_ID " & _
    '    '"join DWH.AR_GECC.Activity_Detail tt on at.TransferTo = tt.Detail_ID " & _
    ...
    '    "	, 'Yes' as Editable	 " & _
    '    "from DWH.AR_GECC.ActivityReport_StagingTransfers at " & _
    '    "join DWH.AR_GECC.ActivityReport_Detail adfrom on at.TransferFrom = adfrom.ActivityID and adfrom.Active = 1 " & _
    '    "	and adfrom.ActivityDate <= at.TransferDate and CurrentFlag = case when adfrom.DetailStatus = 'Current' then 1 else 0 end" & _
    '    "	and not exists (select * from DWH.AR_GECC.ActivityReport_Detail ad2 where ad2.Active =1 and ad2.ActivityID = adfrom.ActivityID " & _
    ...
    '    "left join DWH.AR_GECC.ActivityReport_Detail adto on at.TransferTo = adto.ActivityID and adto.Active = 1 " & _
    '    "	and adto.ActivityDate = at.TransferDate " & _
    '    "	and not exists (select * from DWH.AR_GECC.ActivityReport_Detail ad2 where ad2.Active =1 and ad2.ActivityID = adto.ActivityID " & _
    ...
    '    "		and ((ad2.ActivityDate = adfrom.ActivityDate and ad2.ModifiedDate > adfrom.ModifiedDate) " & _
    '    "			or ad2.ActivityDate > adfrom.ActivityDate and ad2.ActivityDate <= at.TransferDate)) " & _
    '    "join DWH.AR_GECC.ActivityReport_Detail adto on at.TransferTo = adto.ActivityID and adto.Active = 1 " & _
    '    "	and adto.ActivityDate = at.TransferDate " & _
    '    "	and not exists (select * from DWH.AR_GECC.ActivityReport_Detail ad2 where ad2.Active =1 and ad2.ActivityID = adto.ActivityID " & _
    '    "		and ((ad2.ActivityDate = adto.ActivityDate and ad2.ModifiedDate > adto.ModifiedDate) " & _
    '    "			or ad2.ActivityDate > adto.ActivityDate and ad2.ActivityDate <= at.TransferDate)) " & _
    ...
    '            'UpdatesSql += " Update DWH.AR_GECC.Activity_Detail set Facility = '" & NewTTFAC.SelectedValue & "', ModifyDate = getdate(), ModifyUser = '" & _
    '            '    Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "' where  Detail_ID = " & lblTTID.Text & " and ActivityDate = '" & txtARDate.Text & "' "

    '            UpdatesSql += " Update DWH.AR_GECC.ActivityReport_StagingTransfers set TTFAC = '" & NewTTFAC.SelectedValue & "' " & _
    ...
    '            'UpdatesSql += "Insert into DWH.AR_GECC.Activity_Transfers_History SELECT " & _
    '            ' "      Base_ID " & _
    '            ' "      ,TransferFrom " & _
    '            ' "     ,TransferTo " & _
    '            ' "     ,Amount " & _
    ...
    '            ' "     ,ModifyDate " & _
    '            ' "      ,ModifyUser " & _
    '            ' "  FROM DWH.AR_GECC.Activity_Transfers " & _
    '            ' "  where TransferFrom = " & lblTFID.Text & " and TransferTo = '" & lblTTID.Text & "' "

    '            'UpdatesSql += " Update DWH.AR_GECC.Activity_Transfers set Amount = '" & Trim(Replace(NewTAmount.Text, "'", "''")) & "', ModifyDate = getdate(), ModifyUser = '" & _
    '            '    Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "' where TransferFrom = '" & lblTFID.Text & "' " & _
    '            '    " and TransferTo = '" & lblTTID.Text & "' "

    '            UpdatesSql += " Update DWH.AR_GECC.ActivityReport_StagingTransfers set Amount = '" & Trim(Replace(NewTAmount.Text, "'", "''")) & "' " & _
    '                " where  TFID = " & gvTransfers.DataKeys(i).Value & "  "
    ...
    '    Dim s As String = "insert into DWH.AR_GECC.ActivityReport_StagingTransfers (TFID, TransferDate, TransferFrom, Active, TFFac, CurrentFlag, ModifyUser ) " & _
    '        "select isnull((select min(TFID) from DWH.AR_GECC.ActivityReport_StagingTransfers where TFID < 0), 0) - 1,  '" & Replace(txtARDate.Text, "'", "''") & "', " & _
    ...
    '    "from DWH.AR_GECC.ActivityReport_Detail where Active =  1 and ActivityID = " & lblDetailIDTransfers.Text & " and ActivityDate <= '" & Trim(Replace(txtARDate.Text, "'", "''")) & _
    '    "' ) x where RN = 1 "

    '    'Dim s As String = "insert into DWH.AR_GECC.Activity_Detail " & _
    '    '"(Detail_ID, Base_ID, ActivityDate, Active, AssignedUser, ModifyDate, ModifyUser, CashCategory, CategoryStatus, Type) " & _
    '    '"select (select max(Detail_ID) from DWH.AR_GECC.Activity_Detail) + 1, Base_ID, ActivityDate, Active, AssignedUser, getdate(), '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & _
    ...
    '    '" insert into DWH.AR_GECC.Activity_Transfers (Base_ID, TransferFrom, TransferTo, Amount, Active, ModifyDate, ModifyUser) " & _
    '    '"select Base_ID, Detail_ID,  (select max(Detail_ID) from DWH.AR_GECC.Activity_Detail where ModifyUser = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & _
    ...
    '    '    " from DWH.AR_GECC.Activity_Transfers at " & _
    '    '    "join DWH.AR_GECC.Activity_Detail ad on at.TransferFrom = ad.Detail_ID and ad.Active = 1 " & _
    '    '    "join DWH.AR_GECC.Activity_Detail tt on at.TransferTo = tt.Detail_ID and tt.Active = 1 " & _
    ...
    '    '    "and at.Active = 1 " & _
    '    '    " " & _
    '    '    " insert into DWH.AR_GECC.Activity_Detail_History " & _
    ...
    '    '             "      ,Misc_GL_Text       " & _
    '    '    "from DWH.AR_GECC.Activity_Detail ad " & _
    '    '    "join DWH.AR_GECC.Activity_Transfers at on ad.Detail_ID = at.IntermediateRow " & _
    ...
    '    '    " " & _
    '    '    "  insert into DWH.AR_GECC.Activity_Detail " & _
    '    '    "select (select max(x.Detail_ID) from DWH.AR_GECC.Activity_Detail x) + ROW_NUMBER() over (order by ad.Row_ID)  " & _
    ...
    '    '    "join DWH.AR_GECC.Activity_Detail ad on at.TransferFrom = ad.Detail_ID and ad.Active = 1 " & _
    '    '    "join DWH.AR_GECC.Activity_Detail tt on at.TransferTo = tt.Detail_ID and tt.Active = 1 " & _
    '    '    "join DWH.AR_GECC.Activity_Detail it on it.ModifyUser = 'IntermediateTransferInsert-'+ convert(varchar, at.TransferFrom) + ':'+ convert(varchar, at.TransferTo) " & _
    '    '    "where ad.Facility <> 'A' and tt.Facility <> 'A' " & _
    ...
    '                Else
    '                    UpdatesSql += " Update DWH.AR_GECC.ActivityReport_StagingTransfers set TTFAC = '" & Replace(NewTTFAC.SelectedValue, "'", "''") & _
    ...
    '        Dim confirmSQl As String = "Insert DWH.AR_GECC.ActivityReport " & _
    ...
    '"into DWH.AR_GECC.ActivityReport_Transfers " & _
    ...
    '"from DWH.AR_GECC.ActivityReport_Transfers c join " & _
    '"(select *, Row_Number() over (order by TFID) RN from DWH.AR_GECC.ActivityReport_Transfers  " & _
    ...
    '"join (select *, ROW_NUMBER() over (order by TFID) RN from DWH.AR_GECC.ActivityReport_StagingTransfers  " & _
    '"	where TransferFrom = '" & Replace(lblDetailIDTransfers.Text, "'", "''") & "' and Active = 1 and ModifyUser = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "' and TFID < 0) b on a.RN = b.RN " & _
    '"	 " & _
    '"Insert into DWH.AR_GECC.ActivityReport " & _
    ...
    '"join (select *, ROW_NUMBER() over (order by TFID) RN from DWH.AR_GECC.ActivityReport_StagingTransfers  " & _
    '"	where TransferFrom = '" & Replace(lblDetailIDTransfers.Text, "'", "''") & "' and Active = 1 and ModifyUser = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "' and TFID < 0 and isnull(TTFac, 'null') <> 'null' and  'A' not in (TFFac, TTFac)) b on a.RN = b.RN " & _
    '"join (select *, Row_Number() over (partition by CreatedBy order by ActivityID) RN from DWH.AR_GECC.ActivityReport ar " & _
    ...
    '"	and not exists (select * from DWH.AR_GECC.ActivityReport_Transfers t where t.IntermediateRow = ar.ActivityID and t.Active = 1) " & _
    '"	) d " & _
    ...
    '"	 and a.RN= d.RN " & _
    '"	  " & _
    '"insert into DWH.AR_GECC.ActivityReport_TransfersHistorical " & _
    '"select t.* " & _
    '"from DWH.AR_GECC.ActivityReport_StagingTransfers st " & _
    '"join DWH.AR_GECC.ActivityReport_Transfers t on st.TFID = t.TFID  " & _
    ...
    '"from DWH.AR_GECC.ActivityReport_StagingTransfers at " & _
    '"join DWH.AR_GECC.ActivityReport_Transfers t on at.TFID = t.TFID  " & _
    ...
    '"from (select *, ROW_NUMBER() over (partition by TransferFrom, TFFac, TTFac order by TFID) RN  " & _
    '"	from DWH.AR_GECC.ActivityReport_StagingTransfers where TransferFrom = '" & Replace(lblDetailIDTransfers.Text, "'", "''") & "' and ModifyUser = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "') at " & _
    '"join DWH.AR_GECC.ActivityReport_Transfers t on at.TFID = t.TFID  " & _
    '"join (select *, ROW_NUMBER() over (partition by CreatedBy order by ActivityID) RN from DWH.AR_GECC.ActivityReport ar " & _
    '"	where not exists (select * from DWH.AR_GECC.ActivityReport_Transfers t where t.IntermediateRow = ar.ActivityID and t.Active = 1)) " & _
    ...
    '" " & _
    '"Insert into DWH.AR_GECC.ActivityReport_Detail  (ActivityID, ActivityDate, ModifiedBy, ModifiedDate, Active, Facility  " & _
    ...
    '"from DWH.AR_GECC.ActivityReport_StagingTransfers st " & _
    '"join DWH.AR_GECC.ActivityReport_Transfers t on st.TFID = t.TFID  " & _
    '"join DWH.AR_GECC.ActivityReport_Detail d on t.TransferTo = d.ActivityID and d.Active = 1  " & _
    '"	and d.ActivityDate <= t.TransferDate and not exists (select * from DWH.AR_GECC.ActivityReport_Detail d2   " & _
    ...
    '", 0 " & _
    '"from DWH.AR_GECC.ActivityReport st " & _
    '"join DWH.AR_GECC.ActivityReport_Transfers t on st.ActivityID in (t.IntermediateRow, t.TransferTo) " & _
    '"join DWH.AR_GECC.ActivityReport_StagingTransfers stf on t.TFID = stf.TFID " & _
    '"join DWH.AR_GECC.ActivityReport_Detail d on t.TransferFrom = d.ActivityID and d.Active = 1  " & _
    '"	and d.ActivityDate <= t.TransferDate and not exists (select * from DWH.AR_GECC.ActivityReport_Detail d2   " & _
    ...
    '"and not exists (select * from DWH.AR_GECC.ActivityReport_Detail ard where ard.ActivityID = st.ActivityID and ard.Active = 1) " & _
    '"	Insert into DWH.AR_GECC.ActivityReport_Detail  (ActivityID, ActivityDate, ModifiedBy, ModifiedDate, Active, Facility " & _
    ...
    '    "from DWH.AR_GECC.ActivityReport_Transfers t " & _
    '    "join DWH.AR_GECC.ActivityReport ar on ar.ActivityID in (t.TransferTo, t.IntermediateRow) and ar.Active = 1 " & _
    '    "join DWH.AR_GECC.ActivityReport_Detail d on t.TransferFrom = d.ActivityID and d.Active = 1   " & _
    '    "	and d.ActivityDate <= t.TransferDate and not exists (select * from DWH.AR_GECC.ActivityReport_Detail d2   " & _
    '    "	where d2.Active = 1 and d2.ActivityID = t.TransferFrom and " & _
    '    "	((d2.ActivityDate = d.ActivityDate and d2.ModifiedDate > d.ModifiedDate) " & _
    '    "	or (d2.ActivityDate > d.ActivityDate and d2.ActivityDate <= t.TransferDate))) " & _
    '    "where t.TransferFrom = '" & Replace(lblDetailIDTransfers.Text, "'", "''") & "' and t.ModifyUser = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & _
    '    "' and not exists (select * from DWH.AR_GECC.ActivityReport_Detail ard where ard.ActivityID = ar.ActivityID and ard.Active = 1) ) a " & _
    '    "join (select *, case when 'A' in (TFFAC, TTFAC) then 0 else 1 end as IntAct " & _
    '    "		, ROW_NUMBER() over (partition by TransferFrom, Amount, case when 'A' in (TFFAC, TTFAC) then 0 else 1 end order by TFID) RN  " & _
    '    "	from DWH.AR_GECC.ActivityReport_StagingTransfers where TFID < 0 and TTFAC is not null and Active = 1 ) st on st.TransferFrom = a.TransferFrom and st.Amount = a.Amount  " & _
    '    "	and st.RN = a.RN and isnull(st.IntAct, 0) = isnull(a.IntermediateActive, 0) " & _
    '        "insert into DWH.AR_GECC.ActivityReport_Assignments  " & _
    '    "select ActivityID, ModifiedBy, getdate(), 'Automatic' from DWH.AR_GECC.ActivityReport ar " & _
    '    "where Active = 1 and (CreatedBy like 'Transfer From %' or CreatedBy like 'Intermediate Transfer From %')" & _
    '    "and not exists (select * from DWH.AR_GECC.ActivityReport_Assignments ass where ass.ActivityID = ar.ActivityID) " & _
    ...
    '"insert into DWH.AR_GECC.ActivityReport_Detail " & _
    ...
    '", ard.Locked " & _
    '"froM DWH.AR_GECC.ActivityReport_Detail ard " & _
    '"left join (select TransferFrom, sum(Amount) TransferredOut, TransferDate as TFDate from DWH.AR_GECC.ActivityReport_Transfers " & _
    '"		where Active = 1  " & _
    '"		group by TransferFrom, TransferDate) tff on tff.TransferFrom = ard.ActivityID and TFDate >= ard.ActivityDate " & _
    '"left join (select TransferTo, sum(Amount) TransferredIn, TransferDate as TFTDate from DWH.AR_GECC.ActivityReport_Transfers  " & _
    '"		where Active = 1   " & _
    '"		group by TransferTo, TransferDate) tft on tft.TransferTo = ard.ActivityID and TFTDate = ard.ActivityDate  " & _
    '"where ard.Active = 1 and not exists (select * from DWH.AR_GECC.ActivityReport_Detail ard2 where ard2.Active = 1 and ard2.ActivityID = ard.ActivityID " & _
    ...
    '        "(select * from DWH.AR_GECC.ActivityReport_Detail ardt where ardt.ActivityID	= ard.ActivityID and ardt.ActivityDate <= TFDate and " & _
    ...
    '"left join (select TransferTo, sum(Amount) TransferredIn, max(convert(date, TransferDate)) as TFTDate from DWH.AR_GECC.ActivityReport_Transfers  " & _
    '"		where Active = 1   " & _
    ...
    '"and isnull(Cash_Received, 0) - isnull(AR_Posted, 0) - isnull(Misc_Posted, 0) - isnull(Interest, 0) - isnull(TransferredOut, 0) + isnull(TransferredIn, 0) <> isnull(Unresolved, 0) " & _
    '"and (AR_Posted is not null or Misc_Posted is not null or Interest is not null or TransferredOut is not null or TransferredIn is not null or Unresolved is not null) " & _
    '        "/* '6/14/2018 CRW -- Added last two lines to prevent weird duplication possibility */  delete from DWH.AR_GECC.ActivityReport_StagingTransfers " & _
    '                "where ModifyUser = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "' " & _
    '        "/* Assingment Logging 6/11/2019 CRW */ " & _
    '        "insert into DWH.AR_GECC.ActivityReport_Assignments_Logging  " & _
    '        "select *, null, null from DWH.AR_GECC.ActivityReport_Assignments ara " & _
    '        "where not exists (select * from DWH.AR_GECC.ActivityReport_Assignments_Logging aral " & _
    ...
    '        "from DWH.AR_GECC.ActivityReport_Assignments_Logging ar1 " & _
    '        "join DWH.AR_GECC.ActivityReport_Assignments_Logging ar2 on ar1.ActivityID = ar2.ActivityID " & _
    '        "	and ar1.InactivatedDate is null and ar2.InactivatedDate is null and ar2.AddedDate > ar1.AddedDate " & _
    '            "and not exists (select * from DWH.AR_GECC.ActivityReport_Assignments_Logging ar3 where ar3.ActivityID = ar2.ActivityID and ar3.InactivatedDate is null " & _
    ...
    Protected Sub ddlARRowAssignedUser_SelectedIndexChanged1(sender As Object, e As EventArgs)
        ...
        For i As Integer = 0 To gv_AR_MainData.Rows.Count - 1
            ...
            If ddlARRowAssignedUser.SelectedValue <> lblARAssignedUser.Text Then
                'UpdatesSql += "Update DWH.AR_GECC.Activity_Detail set AssignedUser = '" & ddlARRowAssignedUser.SelectedValue & "' where Detail_ID = '" & gv_AR_MainData.DataKeys(i).Value.ToString & "' "
                UpdatesSql += "Update DWH.AR_GECC.ActivityReport_Assignments " & _
                    ...
                    "where '" & ddlARRowAssignedUser.SelectedValue & "' <> 'Unassigned' and not exists (select * from DWH.AR_GECC.ActivityReport_Assignments where ActivityID = '" & gv_AR_MainData.DataKeys(i).Value.ToString & "' ) "
            End If
...
"		and ar3.AddedDate > ar1.AddedDate and ar3.AddedDate < ar2.AddedDate ) "
    ...
    Private Sub InitialPopulateSplitGrid(detid As Integer)
        ...
        '    "from ( " & _
        '    "select  Detail_ID, Batch_Number, WF_Cash_Received from DWH.AR_GECC.Activity_Detail " & _
        ...
        "delete from DWH.AR_GECC.ActivityReport_StagingSplitting where InitialID = " & detid & " or ModifyUser = '" & _
        Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & "' " & _
        "insert into DWH.AR_GECC.ActivityReport_StagingSplitting (InitialID, SplitID, STARBatchNo, AmountSplit, ActivityDate, Active)" & _
        "select InitialID, SplitID, STARBatchNo, AmountSplit, ActivityDate, 1 from  " & _
        " DWH.AR_GECC.ActivityReport_Splitting where Active = 1 and ActivityDate = @SearchDate " & _
        "		and InitialID = " & detid & "  " & _
            ...
            "		from DWH.AR_GECC.ActivityReport_StagingSplitting where Active = 1 and ActivityDate = @SearchDate) s " & _
            "where s.RN = 1 " & _
        ...
        '        "insert into DWH.AR_GECC.ActivityReport_StagingSplitting (InitialID, SplitID, STARBatchNo, AmountSplit, ActivityDate)" & _
        '"select InitialID, SplitID, STARBatchNumber, Cash_Received, x.ActivityDate from  " & _
        '"(select *, DENSE_RANK() over (partition by InitialID order by ActivityDate desc, ModifyDate desc) as RN   " & _
        '"		from DWH.AR_GECC.ActivityReport_Splitting where Active = 1 and ActivityDate <= @SearchDate " & _
        ...
        '    "		from DWH.AR_GECC.ActivityReport_Detail where Active = 1 and ModifiedBy like 'Split - %' and ActivityDate <= @SearchDate) o on o.RN = 1 and x.SplitID = o.ActivityID " & _
        '    " where x.RN = 1 " & _
        '    "select convert(varchar, ROW_NUMBER() over (order by -SplitID desc)) + '. ' as RN " & _
        ...
        For i As Integer = 0 To gvSplitRows.Rows.Count - 1
            ...
            If lblSplitDetail_ID.Text = "0" Then
                ...
                If Len(Trim(txtSplitBatch_Number.Text)) > 0 Or Len(Trim(txtSplitWF_Cash_Received.Text)) > 0 Then
                    ...
                    '        s += "insert into DWH.AR_GECC.Activity_Detail " & _
                    ...
                    s += "insert into DWH.AR_GECC.ActivityReport_StagingSplitting (InitialID, SplitID, STARBatchNo, AmountSplit, ActivityDate, Active) values " & _
                        "( " & Replace(lblSplitDetailID.Text, "'", "''") & ", isnull((select max(SplitID) from DWH.AR_GECC.ActivityReport_StagingSplitting where InitialID = " & _
            ...
            Else
                If Len(Trim(txtSplitBatch_Number.Text)) = 0 And Len(Trim(txtSplitWF_Cash_Received.Text)) = 0 Then
                    s += "delete from DWH.AR_GECC.ActivityReport_StagingSplitting where InitialID = " & Replace(lblSplitDetailID.Text, " '", "''") & _
                ...
                Else
                    If lblSplitBatch_Number.Text <> txtSplitBatch_Number.Text Or lblSplitWF_Cash_Received.Text <> txtSplitWF_Cash_Received.Text Then
                        ...
                        's += "update DWH.AR_GECC.Activity_Detail set Batch_Number = '" & Trim(Replace(txtSplitBatch_Number.Text, "'", "''")) & "', WF_Cash_Received = '" & _
                        'Trim(Replace(txtSplitWF_Cash_Received.Text, "'", "''")) & "' where Detail_ID = " & Replace(lblSplitDetail_ID.Text, "'", "''") & " "

                        s += "update DWH.AR_GECC.ActivityReport_StagingSplitting set STARBatchNo = '" & Trim(Replace(txtSplitBatch_Number.Text, "'", "''")) & "', AmountSplit = '" & _
    ...
    Private Sub btnSubmitSplit_Click(sender As Object, e As EventArgs) Handles btnSubmitSplit.Click
        ...
        'Dim s As String = "Update DWH.AR_GECC.Activity_Detail set Active = 1, ModifyDate = getdate(), ModifyUser = '" & _
        '    Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "' where ModifyUser = 'Splitting - ' + '" & Replace(lblSplitDetailID.Text, "'", "''") & "'"

        Dim s As String = " Update s set Active = 0 from DWH.AR_GECC.ActivityReport_Splitting s left join DWH.AR_GECC.ActivityReport_StagingSplitting ss on s.SplitID = ss.SplitID and s.Active = 1 where s.InitialID = '" & _
            ...
            "insert into  DWH.AR_GECC.ActivityReport_Splitting " & _
            "select a.InitialID, STARBatchNo, AmountSplit, ActivityDate, 1, getdate(), '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "' " & _
            "from DWH.AR_GECC.ActivityReport_StagingSplitting a " & _
            "where a.InitialID = '" & Replace(lblSplitDetailID.Text, "'", "''") & "' and not exists (select * from DWH.AR_GECC.ActivityReport_Splitting b where  " & _
            ...
            "		from DWH.AR_GECC.ActivityReport_Splitting   " & _
            ...
            "            From DWH.AR_GECC.ActivityReport_Splitting ST1 " & _
            ...
            "From  DWH.AR_GECC.ActivityReport_Splitting ST2 where ST2.Active = 1  and ST2.ActivityDate = '" & Trim(Replace(txtARDate.Text, "'", "''")) & "') x2 on x2.InitialID = x.InitialID " & _
            "		join DWH.AR_GECC.ActivityReport_Detail ard on ard.Active = 1 and x.InitialID = ard.ActivityID and ard.ActivityDate <= UpdDate " & _
            "		and not exists (select * from DWH.AR_GECC.ActivityReport_Detail ard2 where ard.ActivityID = ard2.ActivityID and ard2.ActivityDate <= UpdDate and ard2.Active = 1 " & _
            "			and (ard2.ActivityDate > ard.ActivityDate or (ard.ActivityDate = ard2.ActivityDate and ard2.ModifiedDate > ard.ModifiedDate))) " & _
            "delete a  " & _
            "from DWH.AR_GECC.ActivityReport_StagingSplitting a " & _
            "where InitialID = '" & Replace(lblSplitDetailID.Text, "'", "''") & "' and exists (select * from DWH.AR_GECC.ActivityReport_Splitting b where  " & _
        ...
        '        "insert DWH.AR_GECC.ActivityReport output right(inserted.CreatedBy, len(inserted.CreatedBy) - 10), Inserted.ActivityID  " & _
        '", inserted.FirstActivityDate, 1, getdate(), '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "' into DWH.AR_GECC.ActivityReport_Splitting " & _
        ...
        '"from DWH.AR_GECC.ActivityReport_StagingSplitting a " & _
        '"join DWH.AR_GECC.ActivityReport ar on a.InitialID = ar.ActivityID and ar.Active = 1 " & _
        '"where a.InitialID = '" & Replace(lblSplitDetailID.Text, "'", "''") & "' and not exists (select * from DWH.AR_GECC.ActivityReport_Splitting b where  " & _
        '"	a.initialID = b.InitialID and a.SplitID = b.SplitID and b.Active = 1) " & _
        '"	 " & _
        '        "" & _
        ...
        '"from DWH.AR_GECC.ActivityReport_StagingSplitting a  " & _
        '"join DWH.AR_GECC.ActivityReport_Detail d on a.SplitID = d.ActivityID and a.AmountSplit <> d.Cash_Received and d.Active = 1 " & _
        '"	and d.ActivityDate <= a.ActivityDate and not exists (select * from DWH.AR_GECC.ActivityReport_Detail d2  " & _
        ...
        '"where a.InitialID = '" & Replace(lblSplitDetailID.Text, "'", "''") & "' and exists (select * from DWH.AR_GECC.ActivityReport_Splitting b where " & _
        '"	a.initialID = b.InitialID and a.SplitID = b.SplitID and b.Active = 1) " & _
        '"	 " & _
        '        " " & _
        '"insert into DWH.AR_GECC.ActivityReport_Detail (ActivityID, ActivityDate, ModifiedBy, ModifiedDate, Active, Facility " & _
        ...
        '"	from DWH.AR_GECC.ActivityReport where CreatedBy like 'Split from %' and Active = 1) r " & _
        '"join (select *, Row_Number() over (order by SplitID) rn  " & _
        '"	from DWH.AR_GECC.ActivityReport_StagingSplitting  where InitialID = '" & Replace(lblSplitDetailID.Text, "'", "''") & "' and Active = 1) a  " & _
        '"		on r.CreatedBy =  'Split from ' + convert(varchar, a.InitialID) and  r.rn = a.rn " & _
        '"join DWH.AR_GECC.ActivityReport_Detail d on d.ActivityID = a.InitialID and d.Active = 1 and d.ActivityDate <= a.ActivityDate " & _
        '"	and not exists (select * from DWH.AR_GECC.ActivityReport_Detail d2 where d2.Active = 1 and d2.ActivityID = a.InitialID and  " & _
        '"		((d2.ActivityDate = d.ActivityDate and d2.ModifiedDate > d.ModifiedDate)  " & _
        '"		or (d2.ActivityDate > d.ActivityDate and d2.ActivityDate <= a.ActivityDate)))	 " & _
        '"where not exists (select * from DWH.AR_GECC.ActivityReport_Detail ad where ad.ActivityId = r.ActivityID and ad.Active = 1) " & _
        '        "	 " & _
        '"update DWH.AR_GECC.ActivityReport " & _
    ...
    Private Sub btnCancelSplit_Click(sender As Object, e As EventArgs) Handles btnCancelSplit.Click
        ...
        '    Dim s As String = "Update DWH.AR_GECC.Activity_Detail set WF_Cash_Received = " & lblSplitTotal.Text & ", ModifyDate = getdate(), ModifyUser = '" & _
        'Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "' where Detail_ID = '" & Replace(lblSplitDetailID.Text, "'", "''") & "'   " & _
        '    "Update DWH.AR_GECC.Activity_Detail set ModifyDate = getdate(), ModifyUser = '" & _
    ...
    Private Sub btnSubmitAllRows_Click(sender As Object, e As EventArgs) Handles btnSubmitAllRows.Click
        ...
        For i As Integer = 0 To gv_AR_MainData.Rows.Count - 1
            ...
            If gv_AR_MainData.Rows(i).BackColor = System.Drawing.ColorTranslator.FromHtml("#bbffbb") Then
                ...
                '    Dim CheckAttachments As String = "Select count(*) from DWH.AR_GECC.ActivityReport_AttachedFiles where ActivityID = '" & gv_AR_MainData.DataKeys(i)("ActivityID").ToString & "'"
                '    If GetScalar(CheckAttachments) = 0 Then
                '        explanationlabel.Text = "Attachments required when row is unresolved"
                '        ModalPopupExtender.Show()
                '        Exit Sub
    ...
    Private Sub btnNetRows_Click(sender As Object, e As EventArgs) Handles btnNetRows.Click
        ...
        '    Dim s3 As String = "select distinct isnull(Facility, 'Unassigned') as Facility from DWH.AR_GECC.Activity_Detail " & _
...
"from DWH.AR_GECC.ActivityReport ar " & _
"join (select *, ROW_NUMBER() over (partition by ActivityID order by ActivityDate desc, ModifiedDate desc) RN  from  " & _
"		DWH.AR_GECC.ActivityReport_Detail where Active = 1 and convert(date, ActivityDate) <= @SearchDate) ad on ar.ActivityID = ad.ActivityID and ad.RN = 1 " & _
"		and @SearchDate between FirstActivityDate and isnull(FinalActivityDate, '12/31/9999') " & _
" left join (select ActivityID, sum(MiscAmt) as MiscAmt from DWH.AR_GECC.ActivityReport_MiscGL where Active = 1 and ActivityDate = @SearchDate " & _
"           group by ActivityID) misc on misc.ActivityID = ar.ActivityID  " & _
"left join (select TransferFrom, sum(Amount) TransferredOut from DWH.AR_GECC.ActivityReport_Transfers " & _
"		where Active = 1 and convert(date, TransferDate) = @SearchDate " & _
"		group by TransferFrom) tff on tff.TransferFrom = ar.ActivityID  " & _
"left join (select TransferTo, sum(Amount) TransferredIn, convert(date, TransferDate) as TFTDate from DWH.AR_GECC.ActivityReport_Transfers " & _
"		where Active = 1 and convert(date, TransferDate) <= @SearchDate " & _
"		group by TransferTo, convert(date, TransferDate)) tft on tft.TransferTo = ar.ActivityID and tftDate = ad.ActivityDate " & _
"left join (select *, ROW_NUMBER() over (partition by ActivityID order by ActivityDate desc) RN  from " & _
"		DWH.AR_GECC.ActivityReport_Comments where Active = 1 and CommentType is null and convert(date, ActivityDate) <= @SearchDate) UnCom on ar.ActivityID = UnCom.ActivityID and UnCom.RN = 1 " & _
"left join (select max(AttachID) AttachID, ActivityID from DWH.AR_GECC.ActivityReport_AttachedFiles where Active = 1 group by ActivityID) atch on ar.ActivityID  = atch.ActivityID " & _
"where not exists (select * from DWH.AR_GECC.ActivityReport_Netting net where net.InitialID = ar.ActivityID and convert(date, net.ActivityDate) <= @SearchDate " & _
"	and net.ActivityDate >= ad.ActivityDate and net.Active = 1) " & _
"and not exists (select * from DWH.AR_GECC.ActivityReport_Splitting split where split.InitialID = ar.ActivityID and convert(date, split.ActivityDate) <= @SearchDate " & _
"	and split.ActivityDate >= ad.ActivityDate and split.Active = 1) " & _
"	) zq where 1=1  " & ResearchQ & Balanced & " "
        ...
        '    " and not exists (Select * from DWH.AR_GECC.Activity_Netting ne where ne.InitialID = ad.Row_ID)"
    ...
    Private Sub btnOKNetRows_Click(sender As Object, e As EventArgs) Handles btnOKNetRows.Click
        ...
        For i As Integer = 0 To gvNettingRows.Rows.Count - 1
            ...
            If cbNetRow.Checked Then
                ...
                'UpdatesSql += "Insert into DWH.AR_GECC.Activity_Netting SELECT Row_ID, -1 " & _
                ...
                UpdatesSql += "Insert into DWH.AR_GECC.ActivityReport_Netting SELECT ActivityID, -1, '" & Trim(Replace(txtARDate.Text, "'", "''")) & "', Cash_Received " & _
                   "     ,Active, getdate(), '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & "',  '" & Replace(txtNettingComment.Text, "'", "''") & "'" & _
                   "  FROM DWH.AR_GECC.ActivityReport_Detail ad " & _
                   "  where Active = 1 and ActivityDate <= '" & Replace(txtARDate.Text, "'", "''") & "' and ActivityID = " & gvNettingRows.DataKeys(i)("ActivityID").ToString() & _
                   " and not exists (select * from DWH.AR_GECC.ActivityReport_Detail ad2 " & _
                ...
                CheckingSql += "select top 1 * froM DWH.AR_GECC.ActivityReport_Detail " & _
                ...
                'UpdatesSql += " Update DWH.AR_GECC.ActivitY_Detail set Active = 0, ModifyDate = getdate(), ModifyUser = 'Netting; '+ '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & _
        ...
        If Len(UpdatesSql) > 0 Then
            ...
            UpdatesSql = ""
            'UpdatesSql += " Insert into DWH.AR_GECC.Activity_Detail " & _
            ...
            '    "select (select max(Detail_ID) from DWH.AR_GECC.Activity_Detail) + 1, (select max(Base_ID) from DWH.AR_GECC.Activity_Detail) + 1, Facility, ActivityDate, 1, " & _
            ...
            '    "from DWH.AR_GECC.Activity_Detail ad join DWH.AR_GECC.Activity_Netting n on ad.Row_ID = InitialID " & _
            ...
            '    " Update n set NetID = ad.Row_ID from DWH.AR_GECC.Activity_Netting n join DWH.AR_GECC.Activity_Detail ad on ad.ModifyUser = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & _
            '    "'+ 'Net - ' + '" & NetString & "' " & _
            '    "where NetID = -1 and ActivityDate = '" & Replace(txtARDate.Text, "'", "''") & "' and n.ModifyUser = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "'"

            UpdatesSql += "insert into DWH.AR_GECC.ActivityReport " & _
            "select distinct ActivityDate, null, null, 'Net Rows', getdate(), '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "', getdate(), 1  " & _
            "from DWH.AR_GECC.ActivityReport_Netting " & _
            "where NetID = -1 and ModifyUser = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "' " & _
            " " & _
            "update DWH.AR_GECC.ActivityReport_Netting " & _
            "set NetID = (select ActivityID from DWH.AR_GECC.ActivityReport ar where not exists (select *  " & _
            "		from DWH.AR_GECC.ActivityReport_Detail rd where rd.ActivityID = ar.ActivityID) " & _
            "		and ar.CreatedBy = 'Net Rows' and ar.ModifiedBy = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "') " & _
            "where NetID = -1 and ModifyUser = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "' " & _
            "insert into DWH.AR_GECC.ActivityReport_Detail output inserted.ActivityID, '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "', getdate(), 'NetInsert' " & _
            " into DWH.AR_GECC.ActivityReport_Assignments " & _
            ...
            ", null   " & _
            "from DWH.AR_GECC.ActivityReport_Detail ad " & _
            "join DWH.AR_GECC.ActivityReport_Netting n on n.InitialID = ad.ActivityID and n.Active = 1 and ad.ActivityDate <= n.ActivityDate " & _
            "join DWH.AR_GECC.ActivityReport ar on n.NetID = ar.ActivityID and ar.Active = 1 " & _
            "where ad.Active = 1 /* added ad.Active = 1 CRW 7/17/2019 re bug from Korene */ and not exists (select * from DWH.AR_GECC.ActivityReport_Detail rd where rd.ActivityID = NetID) " & _
            " and not exists (select * from DWH.AR_GECC.ActivityReport_Detail ad2 where ad2.ActivityDate <= n.ActivityDate and ad2.ActivityID = ad.ActivityID and ad2.Active = 1 and " & _
            ...
            "insert into DWH.AR_GECC.ActivityReport_MiscGL " & _
            "Select distinct n.NetID, ST2.Misc_Identity, n.ActivityDate, " & _
            "    substring( " & _
            "        ( " & _
            "            Select '; '+ST1.Comment + ' (' + UserID +')'  AS [text()] " & _
            "            From DWH.AR_GECC.ActivityReport_MiscGL ST1 " & _
            "			join DWH.AR_GECC.ActivityReport_Netting na on ST1.ActivityId = na.InitialID and na.Active = 1  " & _
            ...
            "From DWH.AR_GECC.ActivityReport_MiscGL ST2 " & _
            "join DWH.AR_GECC.ActivityReport_Netting n on ST2.ActivityId = n.InitialID and n.Active = 1  " & _
            "  left join DWH.AR_GECC.ActivityReport_Detail ard on n.NetID = ard.ActivityID and ard.Active = 1 " & _
            ...
            "   and ard.ActivityID = ard2.ActivityID and ard2.Active = 1)  " & _
            "where ST2.ActivityDate = n.ActivityDate and not exists (select * from DWH.AR_GECC.ActivityReport_MiscGL mg where mg.ActivityId = n.NetID) " & _
            "update a " & _
            "set MiscAmt = SumMiscAmt " & _
            "from DWH.AR_GECC.ActivityReport_MiscGL a " & _
            "join (select sum(MiscAmt) SumMiscAmt, NetID, convert(date, m.ActivityDate) as MiscDate from DWH.AR_GECC.ActivityReport_MiscGL m " & _
            "	join DWH.AR_GECC.ActivityReport_Netting n on m.ActivityID = n.InitialID and n.Active = 1 and m.Active = 1 " & _
...
"		and ar3.AddedDate > ar1.AddedDate and ar3.AddedDate < ar2.AddedDate ) "
    ...
    Private Sub btnNewCashJournal_Click(sender As Object, e As EventArgs) Handles btnNewCashJournal.Click
        ...
        'If GetScalar("select isnull(Administrator, 0) from DWH.AR_GECC.ActivityReport_Users where UserLogin = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & "'") > 0 Then
    ...
    Private Sub btnSubmitNewRow_Click(sender As Object, e As EventArgs) Handles btnSubmitNewRow.Click
        ...
        If GetScalar("select count(*) from DWH.AR_GECC.ActivityReport_CashCategory_Type_LU " & _
        ...
        Dim SubmitString As String = "Insert DWH.AR_GECC.ActivityReport output Inserted.ActivityID, Inserted.FirstActivityDate, Inserted.ModifiedBy, getdate(), 1, '" & _
            Replace(ddlNCJFacility.SelectedValue, "'", "''") & "', 'Cash Journal', 'Current', '" & Replace(ddlNCJType.SelectedValue, "'", "''") & "', null, null, null, '" & _
            Trim(Replace(x, "'", "''")) & "', null, null, null, null, null, null, null into DWH.AR_GECC.ActivityReport_Detail " & _
            ...
            "DWH.AR_GECC.ActivityReport r where CreatedBy = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & "' " & _
            "and not exists (select * from DWH.AR_GECC.ActivityReport_Assignments a where r.ActivityID = a.ActivityID) " & _
                    "/* Assingment Logging 6/11/2019 CRW */ " & _
                "insert into DWH.AR_GECC.ActivityReport_Assignments_Logging  " & _
                "select *, null, null from DWH.AR_GECC.ActivityReport_Assignments ara " & _
                "where not exists (select * from DWH.AR_GECC.ActivityReport_Assignments_Logging aral " & _
...
"		and ar3.AddedDate > ar1.AddedDate and ar3.AddedDate < ar2.AddedDate ) "
        ...
        For i As Integer = 0 To gv_AR_MainData.Rows.Count - 1
            ...
            If cbARUnresolved.Checked And hfARUnresolved.Text = "0" Then
                ...
                Dim gvUnresolvedsql As String = "select isnull(UnresolvedDropDownList, UnresolvedFullName) as Display " & _
                    ...
                    "from DWH.AR_GECC.ActivityReport_Unresolved_LU ul " & _
                    "left join DWH.AR_GECC.ActivityReport_Comments c on c.ActivityId = " & lblUnresolvedID.Text & "  " & _
        ...
        For i As Integer = 0 To gv_AR_MainData.Rows.Count - 1
            ...
            If cbMisc.Checked And hfARUnresolved.Text = "0" Then
                ...
                Dim gvMiscsql As String = "select isnull(DropDownDisplay, FullDisplay) as display,  " & _
                    ...
                    "from DWH.AR_GECC.ActivityReport_MiscGLCodes_LU ml " & _
                    "left join DWH.AR_GECC.ActivityReport_StagingMiscGL m on ml.Misc_Identity = m.Misc_Identity and m.Active = 1 and m.ActivityId = " & lblMiscID.Text & " " & _
        ...
        For i As Integer = 0 To gvUnresolved.Rows.Count - 1
            ...
            If cbARUnresolved.Checked Then
                ...
                If lblSubQuestion.Text = "Other" And Len(Trim(txtUnresolved.Text)) = 0 Then
                    ...
                    Exit Sub
                End If
                submitsql += "Insert into DWH.AR_GECC.ActivityReport_Comments Select '" & lblUnresolvedID.Text & "', '" & Replace(txtARDate.Text, "'", "''") & "' " & _
                ", " & txt & ", '" & Replace(lblSubQuestion.Text, "'", "''") & "', '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & "', getdate(), 1 " & _
                " where not exists (select * from DWH.AR_GECC.ActivityReport_Comments where ActivityID = '" & lblUnresolvedID.Text & "' and CommentType = '" & Replace(lblSubQuestion.Text, "'", "''") & "' and Active = 1 ) " & _
                " Update DWH.AR_GECC.ActivityReport_Comments set Comment = " & txt & ", UserID = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & _
                "' where ActivityID = '" & lblUnresolvedID.Text & "' and CommentType = '" & Replace(lblSubQuestion.Text, "'", "''") & "' and Active = 1 "
            Else
                submitsql += "Update DWH.AR_GECC.ActivityReport_Comments set Active = 0 where ActivityID = '" & lblUnresolvedID.Text & "' and CommentType = '" & Replace(lblSubQuestion.Text, "'", "''") & "' and Active = 1 "
    ...
    Private Sub btnOKMiscCodes_Click(sender As Object, e As EventArgs) Handles btnOKMiscCodes.Click
        ...
        For i As Integer = 0 To gvMiscSelections.Rows.Count - 1
            ...
            If cbARMisc.Checked Then
                ...
                submitsql += " Update DWH.AR_GECC.ActivityReport_StagingMiscGL set Active = 0 " & _
                    ...
                    "Insert into DWH.AR_GECC.ActivityReport_StagingMiscGL Select '" & lblMiscID.Text & "', '" & Replace(miscSelection, "'", "''") & "',  '" & Replace(txtARDate.Text, "'", "''") & "' " & _
                    ", " & txt & ", '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & "', getdate(), 1 , '" & Replace(miscAmt.ToString, "'", "''") & "', null " & _
                    " where not exists (select * from DWH.AR_GECC.ActivityReport_StagingMiscGL where ActivityID = '" & lblMiscID.Text & "' and ActivityDate = '" & Replace(txtARDate.Text, "'", "''") & "' " & _
                ...
                '            submitsql += " Update DWH.AR_GECC.ActivityReport_MiscGL set Active = 0 " & _
                '" where ActivityID = '" & lblMiscID.Text & "' and Misc_Identity = '" & Replace(miscSelection, "'", "''") & "' and Active = 1 and (isnull(Comment, '') <> isnull(" & txt & ", '') " & _
                '" or MiscAmt <> '" & miscAmt & "') and ActivityDate = '" & Replace(txtARDate.Text, "'", "''") & "' " & _
                '"Insert into DWH.AR_GECC.ActivityReport_MiscGL Select '" & lblMiscID.Text & "', '" & Replace(miscSelection, "'", "''") & "',  '" & Replace(txtARDate.Text, "'", "''") & "' " & _
                '", " & txt & ", '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & "', getdate(), 1 , '" & Replace(miscAmt.ToString, "'", "''") & "', null " & _
                '" where not exists (select * from DWH.AR_GECC.ActivityReport_MiscGL where ActivityID = '" & lblMiscID.Text & "' and ActivityDate = '" & Replace(txtARDate.Text, "'", "''") & "' " & _
                '" and Misc_Identity = '" & Replace(miscSelection, "'", "''") & "' and Active = 1 ) "


                'Altered 2/21/2018 CRW for better tracking
                'submitsql += "Insert into DWH.AR_GECC.ActivityReport_MiscGL Select '" & lblMiscID.Text & "', '" & Replace(miscSelection, "'", "''") & "',  '" & Replace(txtARDate.Text, "'", "''") & "' " & _
                '", " & txt & ", '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & "', getdate(), 1 , '" & Replace(miscAmt.ToString, "'", "''") & "', null " & _
                '" where not exists (select * from DWH.AR_GECC.ActivityReport_MiscGL where ActivityID = '" & lblMiscID.Text & "' and Misc_Identity = '" & Replace(miscSelection, "'", "''") & "' and Active = 1 ) " & _
                '" Update DWH.AR_GECC.ActivityReport_MiscGL set Comment = " & txt & ", MiscAmt = '" & Replace(miscAmt.ToString, "'", "''") & "', UserID = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & _
                '"' where ActivityID = '" & lblMiscID.Text & "' and Misc_Identity = '" & Replace(miscSelection, "'", "''") & "' and Active = 1 "
            Else
                submitsql += "Update DWH.AR_GECC.ActivityReport_StagingMiscGL set Active = 0 where ActivityID = '" & lblMiscID.Text & "' and ActivityDate = '" & Replace(txtARDate.Text, "'", "''") & "' " & _
                    " and Misc_Identity = '" & Replace(miscSelection, "'", "''") & "' and Active = 1 "

                'Staging Table 2/21/2018 CRW
                '           submitsql += "Update DWH.AR_GECC.ActivityReport_MiscGL set Active = 0 where ActivityID = '" & lblMiscID.Text & "' and ActivityDate = '" & Replace(txtARDate.Text, "'", "''") & "' " & _
                '" and Misc_Identity = '" & Replace(miscSelection, "'", "''") & "' and Active = 1 "
        ...
        '    submitsql += " insert into DWH.AR_GECC.ActivityReport_Detail " & _
        ...
        '    "(select sum(MiscAmt) from DWH.AR_GECC.ActivityReport_MiscGL where Active = 1 and ActivityID = '" & _
        '           lblMiscID.Text & "' and  ActivityDate = '" & Replace(txtARDate.Text, "'", "''") & "') " & _
        '    "	, Interest, Carry_Forward, Unresolved, Locked   " & _
        '     " from  DWH.AR_GECC.ActivityReport_Detail ard where ard.Active = 1 and '" & lblMiscID.Text & "' = ard.ActivityID and ard.ActivityDate <= '" & Replace(txtARDate.Text, "'", "''") & "' " & _
        '    "		and not exists (select * from DWH.AR_GECC.ActivityReport_Detail ard2 where ard.ActivityID = ard2.ActivityID and ard2.ActivityDate <= '" & Replace(txtARDate.Text, "'", "''") & "' and ard2.Active = 1 " & _
        ...
        '    '"from DWH.AR_GECC.ActivityReport_MiscGL art " & _
        '    '"left join DWH.AR_GECC.ActivityReport_Detail ard on art.ActivityID = ard.ActivityID and ard.Active = 1 " & _
    ...
    Private Sub btnRunDailyProcessing_Click(sender As Object, e As EventArgs) Handles btnRunDailyProcessing.Click
        ...
        'Dim ProcessCheck As String = "declare @RunDate date = (select Value from DWH.AR_GECC.ActivityReport_ExtraValues where Description = 'RunDate' and Active = 1) " & _
        ...
        '    " from DWH.AR_GECC.ActivityReport ar " & _
        '    "left join DWH.AR_GECC.ActivityReport_Splitting sl on ar.ActivityID = sl.InitialID and sl.Active = 1 and sl.ActivityDate = @RunDate " & _
        '    "join DWH.AR_GECC.ActivityReport_Detail ard on ar.ActivityID = ard.ActivityID and ard.Active = 1 " & _
        '    "	and ard.ActivityDate <= @RunDate " & _
        '    "	and not exists (select * from DWH.AR_GECC.ActivityReport_Detail ard2 where ard2.ActivityID = ard.ActivityID and ard2.Active = 1 " & _
        ...
        '    "and not exists (select * from DWH.AR_GECC.ActivityReport_Netting arn where arn.InitialID = ar.ActivityID and arn.Active = 1 and arn.ActivityDate <= @RunDate) " & _
        ...
        'Dim ProcessCheck3 As String = "declare @RunDate date = (select Value from DWH.AR_GECC.ActivityReport_ExtraValues where Description = 'RunDate' and Active = 1) " & _
        '    "select isnull(sum(isnull(ARPosted, 0) - isnull(AR, 0)), 0) as ARVariance " & _
        '    "from ( " & _
        ...
        Dim ProcessCheck2 As String = "declare @RunDate date = (select Value from DWH.AR_GECC.ActivityReport_ExtraValues where Description = 'RunDate' and Active = 1) " & _
            "select count(*) from ( " & _
            ...
            "	DWH.AR_GECC.ActivityReport_MiscGL where Active = 1 and convert(date, ActivityDate) = @RunDate group by ActivityID) mgl on ar.ActivityID = mgl.ActivityID  " & _
            "	  " & _
            "left join DWH.AR_GECC.ActivityReport_Assignments ass on ass.ActivityID = ar.ActivityID  " & _
            "left join DWH.AR_GECC.ActivityReport_Users assu on ass.UserAssigned = assu.UserLogin  " & _
            "left join DWH.AR_GECC.ActivityReport_Users modu on ard.ModifiedBy = modu.UserLogin  " & _
        ...
        If GetScalar("select count(*) from DWH.WF.vw_WFAllActivity where LoadDate = (select dateadd(day, 1, Value) from DWH.AR_GECC.ActivityReport_ExtraValues where Description = 'RunDate' and Active = 1)") = 0 Then
            Dim x As String = Trim(GetString("select Week_Day_Name from DWH.dbo.DimDate " & _
                    "where Calendar_Date = (select dateadd(day, 1, Value) from DWH.AR_GECC.ActivityReport_ExtraValues where Description = 'RunDate' and Active = 1)"))
    ...
    Private Sub btnConfirmSrs_Click(sender As Object, e As EventArgs) Handles btnConfirmSrs.Click
        Try
            ...
            ExecuteSql("exec DWH.AR_GECC.usp_DailyProcessing ")
            RefreshDates()
            gv_AR_MainData.DataSource = ARDetailView()
            gv_AR_MainData.DataBind()
            BalanceCheck()
    ...
    Private Sub PopulateTestTypes()
        ...
        Dim testsql As String = "declare @TestType varchar(max) = '" & Trim(Replace(txtAssignUserKeyword.Text, "'", "''")) & "' " & _
            ...
            "from DWH.AR_GECC.ActivityReport_Detail ard " & _
            "join DWH.AR_GECC.ActivityReport_User_Responsibility ur1 on ard.Type like '%' + ur1.Type + '%'			 " & _
            "			and not exists (select * from DWH.AR_GECC.ActivityReport_User_Responsibility ur4 where ard.Type like '%' + ur4.Type + '%' " & _
            "			and (len(ur4.Type ) > len(ur1.Type) or ( len(ur4.Type ) = len(ur1.Type) and CHARINDEX(ur1.Type, ard.Type) > CHARINDEX(ur4.Type, ard.Type)))) " & _
            " left join DWH.AR_GECC.ActivityReport_Users u on ur1.UserLogin = u.UserLogin " & _
    ...
    Private Sub PopulateCurrentAssignments()
        ...
        Dim SQL As String = "select ur.Type, isnull(isnull(u.UserDropDownListName, u.UserFullName), ur.UserLogin) as UserAssigned " & _
            ...
            "from DWH.AR_GECC.ActivityReport_User_Responsibility ur " & _
            "left join DWH.AR_GECC.ActivityReport_Users u on ur.UserLogin = u.UserLogin " & _
    ...
    Private Sub PopulateAssignmentDropDown()
        ...
        Dim SQL As String = "select isnull(UserDropDownListName, UserFullName) as DisplayName, UserLogin from DWH.AR_GECC.ActivityReport_Users " & _
    ...
    Private Sub PopulateBAICategoryDropDown()
        ...
        Dim SQL As String = "select CashCategory, 1 as ord from DWH.AR_GECC.ActivityReport_CashCategory " & _
    ...
    Private Sub PopulateBAITypeCodes()
        Dim SQL As String = "select *, case when Active = 1 then 'Remove Code' else 'Reactivate' end as Clicky from " & _
            "(select *, Row_Number() over (partition by BAITypeCode order by Active desc, ModifiedDate desc) RN froM DWH.AR_GECC.ActivityReport_BAITypeCodes_LU " & _
    ...
    Private Sub PopulateBankAccountNumbers()
        Dim SQL As String = "select *, case when Active = 1 then 'Remove' else 'Reactivate' end as Clicky from (" & _
            ...
            ", Row_Number() over (partition by AccountNumber order by Active desc, ModifiedDate desc) RN  froM DWH.AR_GECC.ActivityReport_BankAccountNumber_LU " & _
            ") x where RN = 1 "
    ...
    Private Sub PopulateCJTypes()
        Dim SQL As String = "select *, case when Active = 1 then 'Remove' else 'Reactivate' end as Clicky from (select *, isnull(DropDownListDisplay, isnull(left(FullDisplay, 12), Type)) as ShortDisplay " & _
            ...
            ", Row_Number() over (partition by Type order by Active desc, ModifiedDate desc) RN from DWH.AR_GECC.ActivityReport_CashCategory_Type_LU where CashCategory = 'Cash Journal' ) x where RN = 1 "
    ...
    Private Sub PopulateLBCodes()
        Dim SQL As String = "select *, case when Active = 1 then 'Remove' else 'Reactivate' end as Clicky, case when LDRollup = 1 then 'Yes' else 'No' end as Rollup " & _
            "from (select *, Row_Number() over (partition by LockboxCode order by Active desc, ModifiedDate desc) RN  from DWH.AR_GECC.ActivityReport_LockboxCodes_LU) x where RN = 1 "
    ...
    Private Sub PopulateMiscGLCodes()
        Dim SQL As String = "select *, case when Active = 1 then 'Remove' else 'Reactivate' end as Clicky " & _
            " from DWH.AR_GECC.ActivityReport_MiscGLCodes_LU where Misc_Identity <> 11 "
    ...
    Private Sub PopulateAdminUnresolved()
        Dim SQL As String = "select *, case when Active = 1 then 'Remove' else 'Reactivate' end as Clicky  from DWH.AR_GECC.ActivityReport_Unresolved_LU where UnresolvedId <> 15 "
    ...
    "from DWH.AR_GECC.ActivityReport_Users u " & _
    " "
    ...
    Private Sub btnAddAssignment_Click(sender As Object, e As EventArgs) Handles btnAddAssignment.Click
        ...
        If Trim(txtAssignUserKeyword.Text) = "" Then
            ...
            Exit Sub
        End If
        If GetScalar("Select count(*) from DWH.AR_GECC.ActivityReport_User_Responsibility where Type = '" & Trim(Replace(txtAssignUserKeyword.Text, "'", "''")) & "' and Active = 1") > 0 Then
            explanationlabelAdmin.Text = "This keyword is already assigned"
            mpeAdminPage.Show()
            Exit Sub
        End If
        Dim SQL As String
        If GetScalar("Select count(*) from DWH.AR_GECC.ActivityReport_User_Responsibility where Type = '" & Trim(Replace(txtAssignUserKeyword.Text, "'", "''")) & "' and Active = 0") > 0 Then
            SQL = "update DWH.AR_GECC.ActivityReport_User_Responsibility set Active = 1, ModifyDate = getdate(), ModifyUser = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & _
                "', UserLogin = '" & Replace(ddlAssignUserSelection.SelectedValue, "'", "''") & "' where Type = '" & _
                Trim(Replace(txtAssignUserKeyword.Text, "'", "''")) & "'"
        Else
            SQL = "Insert into DWH.AR_GECC.ActivityReport_User_Responsibility values ('" & Replace(ddlAssignUserSelection.SelectedValue, "'", "''") & "', '" & _
    ...
    Private Sub gvAssignments_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvAssignments.RowCommand
        ...
        Try
            ...
            If Commander = "RemoveAssignation" Then
                ...
                'Dim PrepSQL As String = "Update DWH.AR_GECC.ActivityReport_User_Responsibility set Active = 0, ModifyDate = getdate(), ModifyUser = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & _
                '    "' where Type = '" & Replace(Detail_ID, "'", "''") & "' and Active = 1 "

                Dim PrepSQL As String = "with cte as ( " & _
                    "select top 1 * from DWH.AR_GECC.ActivityReport_User_Responsibility " & _
    ...
    Private Sub gvAssignments_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles gvAssignments.RowDataBound
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            Dim AvailableUsers As String = "select isnull(isnull(u.UserDropDownListName, u.UserFullName), u.UserLogin) as UserDisplay, UserLogin, 1 as ord " & _
                "from DWH.AR_GECC.ActivityReport_Users u where Active = 1 and Assignable = 1 " & _
                "union " & _
                "select isnull(isnull(u.UserDropDownListName, u.UserFullName), u.UserLogin) + ' (Inactive)' as UserDisplay, UserLogin, 0  " & _
                "from DWH.AR_GECC.ActivityReport_Users u where Active = 0 and UserLogin = '" & Replace(lblAssignedTo.Text, "'", "''") & "' "
    ...
    Protected Sub ddlAssignedUser_SelectedIndexChanged(sender As Object, e As EventArgs)
        ...
        For i As Integer = 0 To gvAssignments.Rows.Count - 1
            ...
            If ddlAssignedTo.SelectedValue <> lblAssignedTo.Text Then
                'UpdatesSql += "Update DWH.AR_GECC.Activity_Detail set AssignedUser = '" & ddlARRowAssignedUser.SelectedValue & "' where Detail_ID = '" & gv_AR_MainData.DataKeys(i).Value.ToString & "' "
                UpdatesSql += "Update DWH.AR_GECC.ActivityReport_User_Responsibility " & _
    ...
    Private Sub btnAdminAddBAICode_Click(sender As Object, e As EventArgs) Handles btnAdminAddBAICode.Click
        ...
        If Trim(ddlAdminBAICategories.SelectedValue) = "(Select Category)" Then
            ...
            Exit Sub
        End If
        If GetScalar("Select count(*) from DWH.AR_GECC.ActivityReport_BAITypeCodes_LU where BAITypeCode = '" & Trim(Replace(txtAdminBAITypeCodes.Text, "'", "''")) & "' and Active = 1") > 0 Then
            explanationlabelAdmin.Text = "This BAI Type Code is already assigned"
            mpeAdminPage.Show()
            Exit Sub
        End If
        Dim SQL As String
        SQL = "Insert into DWH.AR_GECC.ActivityReport_BAITypeCodes_LU values ('" & Trim(Replace(txtAdminBAITypeCodes.Text, "'", "''")) & "', '" & Trim(Replace(txtAdminBAITypeDesc.Text, "'", "''")) & _
    ...
    Private Sub gvBAITypeCodes_RowCancelingEdit(sender As Object, e As GridViewCancelEditEventArgs) Handles gvBAITypeCodes.RowCancelingEdit
        Try
            ...
            PopulateBAITypeCodes()
            ' Dim SQL As String = "select * froM DWH.AR_GECC.ActivityReport_BAITypeCodes_LU " & _
    ...
    Private Sub gvBAITypeCodes_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvBAITypeCodes.RowCommand
        ...
        Try
            ...
            If Commander = "RemoveCode" Then
                ...
                'Dim PrepSQL As String = "Update DWH.AR_GECC.ActivityReport_BAITypeCodes_LU set Active = 0, ModifiedDate = getdate(), ModifiedBy = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & _
                '    "' where BAITypeCode = '" & Replace(Detail_ID, "'", "''") & "' and Active = 1"

                Dim PrepSQL As String = "with cte as ( " & _
                    "select top 1 * from DWH.AR_GECC.ActivityReport_BAITypeCodes_LU " & _
    ...
    Private Sub gvBAITypeCodes_RowUpdating(sender As Object, e As GridViewUpdateEventArgs) Handles gvBAITypeCodes.RowUpdating
        ...
        Dim PrepSQL As String = "Update DWH.AR_GECC.ActivityReport_BAITypeCodes_LU set Active = 0, ModifiedDate = getdate(), ModifiedBy = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & _
                ...
                "Insert into DWH.AR_GECC.ActivityReport_BAITypeCodes_LU " & _
    ...
    Private Sub btnAdminAddBAN_Click(sender As Object, e As EventArgs) Handles btnAdminAddBAN.Click
        ...
        If Trim(ddlAdminBANFacilities.SelectedValue) = "(Select Facility)" Then
            ...
            Exit Sub
        End If
        If GetScalar("Select count(*) from DWH.AR_GECC.ActivityReport_BankAccountNumber_LU where AccountNumber = '" & Trim(Replace(txtAdminBankAccountNumber.Text, "'", "''")) & "' and Active = 1") > 0 Then
            explanationlabelAdmin.Text = "This Bank Account Number is already assigned"
            mpeAdminPage.Show()
            Exit Sub
        End If
        Dim SQL As String
        SQL = "Insert into DWH.AR_GECC.ActivityReport_BankAccountNumber_LU values ('" & Trim(Replace(txtAdminBankAccountNumber.Text, "'", "''")) & "', '" & Trim(Replace(txtBankDescription.Text, "'", "''")) & _
    ...
    Private Sub gvAdminBankAcctNos_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvAdminBankAcctNos.RowCommand
        Try
            ...
            If Commander = "RemoveBAN" Then
                ...
                'Dim PrepSQL As String = "Update DWH.AR_GECC.ActivityReport_BankAccountNumber_LU set Active = 0, ModifiedDate = getdate(), ModifiedBy = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & _
                '    "' where AccountNumber = '" & Replace(Detail_ID, "'", "''") & "' and Active = 1"

                Dim PrepSQL As String = "with cte as ( " & _
                "select top 1 * from DWH.AR_GECC.ActivityReport_BankAccountNumber_LU " & _
    ...
    Private Sub gvAdminBankAcctNos_RowEditing(sender As Object, e As GridViewEditEventArgs) Handles gvAdminBankAcctNos.RowEditing
        Try
            ...
            '" froM DWH.AR_GECC.ActivityReport_BankAccountNumber_LU " & _
            '"where Active = 1 "
    ...
    Private Sub gvAdminBankAcctNos_RowUpdating(sender As Object, e As GridViewUpdateEventArgs) Handles gvAdminBankAcctNos.RowUpdating
        ...
        Dim PrepSQL As String = "Update DWH.AR_GECC.ActivityReport_BankAccountNumber_LU set Active = 0, ModifiedDate = getdate(), ModifiedBy = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & _
                ...
                "Insert into DWH.AR_GECC.ActivityReport_BankAccountNumber_LU " & _
    ...
    Private Sub btnAddAdminCJType_Click(sender As Object, e As EventArgs) Handles btnAddAdminCJType.Click
        ...
        If GetScalar("Select count(*) from DWH.AR_GECC.ActivityReport_CashCategory_Type_LU where Type = '" & Trim(Replace(txtAdminCJType.Text, "'", "''")) & "' and Active = 1") > 0 Then
        ...
        Dim SQL As String
        SQL = "Insert into DWH.AR_GECC.ActivityReport_CashCategory_Type_LU values ('Cash Journal', '" & Trim(Replace(txtAdminCJType.Text, "'", "''")) & "', " & x & y & _
            " '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & _
                 "', getdate(), 1, 1)"
    ...
    Private Sub gvAdminCJTypes_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvAdminCJTypes.RowCommand
        Try
            ...
            If Commander = "RemoveType" Then
                ...
                'Dim PrepSQL As String = "Update DWH.AR_GECC.ActivityReport_CashCategory_Type_LU set Active = 0, ModifiedDate = getdate(), ModifiedBy = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & _
                '    "' where Type = '" & Replace(Detail_ID, "'", "''") & "' and Active = 1"

                Dim PrepSQL As String = "with cte as ( " & _
                "select top 1 * from DWH.AR_GECC.ActivityReport_CashCategory_Type_LU " & _
                "where TYPE = '" & Replace(Detail_ID, "'", "''") & "' " & _
                "order by Active desc,  ModifiedDate desc) " & _
                "update cte set Active = -Active + 1, ModifiedDate = getdate(), ModifiedBy =  '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & "'"
            ...
            ElseIf Commander = "ReverseSubmittal" Then
                ...
                'Dim PrepSQL As String = "Update DWH.AR_GECC.ActivityReport_CashCategory_Type_LU set Submittable = -Submittable + 1, ModifiedDate = getdate(), ModifiedBy = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & _
                '    "' where Type = '" & Replace(Detail_ID, "'", "''") & "' and Active = 1"
    ...
    Private Sub gvAdminCJTypes_RowUpdating(sender As Object, e As GridViewUpdateEventArgs) Handles gvAdminCJTypes.RowUpdating
        ...
        Dim PrepSQL As String = "Update DWH.AR_GECC.ActivityReport_CashCategory_Type_LU set Active = 0, ModifiedDate = getdate(), ModifiedBy = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & _
                ...
                "Insert into DWH.AR_GECC.ActivityReport_CashCategory_Type_LU " & _
    ...
    Private Sub btnAdminAddLB_Click(sender As Object, e As EventArgs) Handles btnAdminAddLB.Click
        ...
        If GetScalar("Select count(*) from DWH.AR_GECC.ActivityReport_LockboxCodes_LU where LockboxCode = '" & Trim(Replace(txtAdminLockboxCode.Text, "'", "''")) & "' and Active = 1") > 0 Then
            explanationlabelAdmin.Text = "This Lockbox Number is already assigned"
            mpeAdminPage.Show()
            Exit Sub
        End If
        Dim SQL As String
        SQL = "Insert into DWH.AR_GECC.ActivityReport_LockboxCodes_LU values ('" & Trim(Replace(txtAdminLockboxCode.Text, "'", "''")) & "', '" & Trim(Replace(txtAdminLockboxType.Text, "'", "''")) & _
    ...
    Private Sub gvAdminLBTypes_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvAdminLBTypes.RowCommand
        Try
            ...
            If Commander = "RemoveType" Then
                ...
                Dim PrepSQL As String = "Update DWH.AR_GECC.ActivityReport_LockboxCodes_LU set Active = 0, ModifiedDate = getdate(), ModifiedBy = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & _
            ...
            ElseIf Commander = "FlipRollup" Then
                ...
                Dim PrepSQL As String = "update DWH.AR_GECC.ActivityReport_LockboxCodes_LU set LDRollup = -isnull(LDRollup, 0) + 1, ModifiedDate = getdate(), ModifiedBy =  '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & _
                     "' where LockboxCode = '" & Replace(Detail_ID, "'", "''") & "' and Active = 1"
    ...
    Private Sub gvAdminLBTypes_RowUpdating(sender As Object, e As GridViewUpdateEventArgs) Handles gvAdminLBTypes.RowUpdating
        ...
        Dim PrepSQL As String = "Update DWH.AR_GECC.ActivityReport_LockboxCodes_LU set Active = 0, ModifiedDate = getdate(), ModifiedBy = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & _
                ...
                "Insert into DWH.AR_GECC.ActivityReport_LockboxCodes_LU " & _
    ...
    Private Sub btnAddAdminMiscGLCode_Click(sender As Object, e As EventArgs) Handles btnAddAdminMiscGLCode.Click
        ...
        'If GetScalar("Select count(*) from DWH.AR_GECC.ActivityReport_LockboxCodes_LU where FullDisplay = '" & Trim(Replace(txtAdminLockboxCode.Text, "'", "''")) & "' and Active = 1") > 0 Then
        '    explanationlabelAdmin.Text = "This Lockbox Number is already assigned"
        '    mpeAdminPage.Show()
        '    Exit Sub
        'End If
        ...
        Dim SQL As String
        SQL = "Insert into DWH.AR_GECC.ActivityReport_MiscGLCodes_LU values (" & x & y & "'" & Trim(Replace(txtAdminMiscGL_FullDesc.Text, "'", "''")) & "', '" & Trim(Replace(txtAdminMiscGL_ShortDesc.Text, "'", "''")) & _
            "', '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & _
                 "', getdate(), 1)"
    ...
    Private Sub gvAdminMiscGL_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvAdminMiscGL.RowCommand
        Try
            ...
            If Commander = "RemoveType" Then
                ...
                Dim PrepSQL As String = "Update DWH.AR_GECC.ActivityReport_MiscGLCodes_LU set Active = -Active + 1, ModifiedDate = getdate(), ModifiedBy = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & _
    ...
    Private Sub gvAdminMiscGL_RowUpdating(sender As Object, e As GridViewUpdateEventArgs) Handles gvAdminMiscGL.RowUpdating
        ...
        'Dim PrepSQL As String = "Update DWH.AR_GECC.ActivityReport_MiscGLCodes_LU set Active = 0, ModifiedDate = getdate(), ModifiedBy = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & _
        '            "' where Misc_Identity = '" & Replace(depid, "'", "''") & "' and Active = 1 " & _
        '        "Insert into DWH.AR_GECC.ActivityReport_MiscGLCodes_LU " & _
        '    "values (" & x & y & "'" & Replace(txtDescription.Text, "'", "''") & "', '" & Replace(txtShortDesc.Text, "'", "''") & _
        '    "', '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "', getdate(), 1) "



        Dim PrepSQL As String = "Insert into DWH.AR_GECC.ActivityReport_MiscGLCodes_LU_History select * from DWH.AR_GECC.ActivityReport_MiscGLCodes_Lu where Misc_Identity = '" & Replace(depid, "'", "''") & "' " & _
            "Update DWH.AR_GECC.ActivityReport_MiscGLCodes_LU set Department = " & x & " SubAcct = " & y & " FullDisplay = '" & Replace(txtDescription.Text, "'", "''") & "', DropDownDisplay = '" & _
    ...
    Private Sub btnAddUnresolved_Click(sender As Object, e As EventArgs) Handles btnAddUnresolved.Click
        ...
        If Trim(txtAdminUnresolvedFull.Text) = "" Then
            ...
            Exit Sub
        End If
        If GetScalar("Select count(*) from DWH.AR_GECC.ActivityReport_Unresolved_LU where UnresolvedFullName = '" & Trim(Replace(txtAdminUnresolvedFull.Text, "'", "''")) & "' and Active = 1") > 0 Then
            explanationlabelAdmin.Text = "This reason already exists"
            mpeAdminPage.Show()
            Exit Sub
        ElseIf GetScalar("Select count(*) from DWH.AR_GECC.ActivityReport_Unresolved_LU where UnresolvedFullName = '" & Trim(Replace(txtAdminUnresolvedFull.Text, "'", "''")) & "' and Active = 0") > 0 Then
        ...
        SQL = "Insert into DWH.AR_GECC.ActivityReport_Unresolved_LU values ('" & Replace(txtAdminUnresolvedFull.Text, "'", "''") & "', " & z & " getdate(), '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & "', 1)"
    ...
    Private Sub gvAdminUnresolvedReasons_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvAdminUnresolvedReasons.RowCommand
        Try
            ...
            If Commander = "RemoveType" Then
                ...
                Dim PrepSQL As String = "Update DWH.AR_GECC.ActivityReport_Unresolved_LU set Active = -Active + 1, ModifiedDate = getdate(), ModifiedUser = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & _
    ...
    Private Sub gvAdminUnresolvedReasons_RowUpdating(sender As Object, e As GridViewUpdateEventArgs) Handles gvAdminUnresolvedReasons.RowUpdating
        ...
        'Dim PrepSQL As String = "Insert into DWH.AR_GECC.ActivityReport_Unresolved_LU  select UnresolvedFullName, UnresolvedDropDownList, ModifiedDate, ModifiedUser + ' -- ' + convert(varchar, UnresolvedId), 0 " & _
        '        " from DWH.AR_GECC.ActivityReport_Unresolved_LU where UnresolvedId = '" & Replace(depid, "'", "''") & "' " & _
        '        "Update DWH.AR_GECC.ActivityReport_Unresolved_LU set ModifiedDate = getdate(), ModifiedUser = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & _
        ...
        Dim PrepSQL As String = "Insert into DWH.AR_GECC.ActivityReport_Unresolved_LU_History select * from DWH.AR_GECC.ActivityReport_Unresolved_LU where UnresolvedId = '" & Replace(depid, "'", "''") & "' " & _
            "Update DWH.AR_GECC.ActivityReport_Unresolved_LU set ModifiedDate = getdate(), ModifiedUser = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") & _
            "', UnresolvedFullName = '" & Replace(txtDescription.Text, "'", "''") & "', UnresolvedDropDownList = '" & Replace(txtShortDesc.Text, "'", "''") & _
    ...
    Private Sub SynchWebFDPermissions()
        ...
        Dim SQL As String = "insert into WebFD.dbo.aspnet_Users " & _
            ...
            "from DWH.AR_GECC.ActivityReport_Users dar " & _
            ...
            "select UserId, '773BC8D1-F524-2B0E-C41F-609112690ED3' from " & _
            "DWH.AR_GECC.ActivityReport_Users dar " & _
            ...
            "DWH.AR_GECC.ActivityReport_Users dar " & _
            "join WebFD.dbo.aspnet_Users au on  au.UserName = dar.UserLogin " & _
            "where dar.Active = 0) and RoleID = '773BC8D1-F524-2B0E-C41F-609112690ED3'"
    ...
    Private Sub btnAdminAddUser_Click(sender As Object, e As EventArgs) Handles btnAdminAddUser.Click
        ...
        If Trim(txtAdminADUserLogin.Text) = "" Then
            ...
            Exit Sub
        End If
        If GetScalar("Select count(*) from DWH.AR_GECC.ActivityReport_Users where UserLogin = '" & Trim(Replace(txtAdminADUserLogin.Text, "'", "''")) & "' and Active = 1") > 0 Then
        ...
        SQL = "Insert into DWH.AR_GECC.ActivityReport_Users values ('" & Replace(txtAdminADUserLogin.Text, "'", "''") & "', '" & _
    ...
    Private Sub gvAdminUsers_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvAdminUsers.RowCommand
        Try
            ...
            If Left(varname, 4) = "Flip" Then
                If GetScalar("select count(*) from DWH.AR_GECC.ActivityReport_Users where Active = 1 and AssignRoles = 1 and UserLogin = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "'") = 0 And CInt(Developer.Text) = 0 Then
            ...
            If varname = "RevokeAccess" Then
                Dim Sql As String = "update DWH.AR_GECC.ActivityReport_Users set Active = 1 - Active, ModifiedBy = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & _
                    "', DateModified = getdate() where UserLogin = '" & Replace(UserLogin, "'", "''") & "'"
                ExecuteSql(Sql)
                PopulateAdmiUsers()
                SynchWebFDPermissions()
            ElseIf varname = "FlipAdmin" Then
                Dim Sql As String = "update DWH.AR_GECC.ActivityReport_Users set Administrator = 1 - isnull(Administrator, 0), ModifiedBy = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & _
                    "', DateModified = getdate() where UserLogin = '" & Replace(UserLogin, "'", "''") & "'"
                ExecuteSql(Sql)
                PopulateAdmiUsers()
            ElseIf varname = "FlipNetting" Then
                Dim Sql As String = "update DWH.AR_GECC.ActivityReport_Users set Netting = 1 - isnull(Netting, 0), ModifiedBy = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & _
                    "', DateModified = getdate() where UserLogin = '" & Replace(UserLogin, "'", "''") & "'"
                ExecuteSql(Sql)
                PopulateAdmiUsers()
            ElseIf varname = "FlipResearch" Then
                Dim Sql As String = "update DWH.AR_GECC.ActivityReport_Users set Research = 1 - isnull(Research, 0), ModifiedBy = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & _
                    "', DateModified = getdate() where UserLogin = '" & Replace(UserLogin, "'", "''") & "'"
                ExecuteSql(Sql)
                PopulateAdmiUsers()
            ElseIf varname = "FlipAssignments" Then
                Dim Sql As String = "update DWH.AR_GECC.ActivityReport_Users set AssignRights = 1 - isnull(AssignRights, 0), ModifiedBy = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & _
                    "', DateModified = getdate() where UserLogin = '" & Replace(UserLogin, "'", "''") & "'"
                ExecuteSql(Sql)
                PopulateAdmiUsers()
            ElseIf varname = "FlipRoles" Then
                Dim Sql As String = "update DWH.AR_GECC.ActivityReport_Users set AssignRoles = 1 - isnull(AssignRoles, 0), ModifiedBy = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & _
                    "', DateModified = getdate() where UserLogin = '" & Replace(UserLogin, "'", "''") & "'"
                ExecuteSql(Sql)
                PopulateAdmiUsers()
            ElseIf varname = "FlipDP" Then
                Dim Sql As String = "update DWH.AR_GECC.ActivityReport_Users set RunProcessing = 1 - isnull(RunProcessing, 0), ModifiedBy = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & _
                    "', DateModified = getdate() where UserLogin = '" & Replace(UserLogin, "'", "''") & "'"
                ExecuteSql(Sql)
                PopulateAdmiUsers()
            ElseIf varname = "FlipReset" Then
                Dim Sql As String = "update DWH.AR_GECC.ActivityReport_Users set ResetRights = 1 - isnull(ResetRights, 0), ModifiedBy = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & _
                    "', DateModified = getdate() where UserLogin = '" & Replace(UserLogin, "'", "''") & "'"
                ExecuteSql(Sql)
                PopulateAdmiUsers()
            ElseIf varname = "FlipHoliday" Then
                Dim Sql As String = "update DWH.AR_GECC.ActivityReport_Users set HolidayRights = 1 - isnull(HolidayRights, 0), ModifiedBy = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & _
                    "', DateModified = getdate() where UserLogin = '" & Replace(UserLogin, "'", "''") & "'"
                ExecuteSql(Sql)
                PopulateAdmiUsers()
            End If
    ...
    Private Sub gvAdminUsers_RowEditing(sender As Object, e As GridViewEditEventArgs) Handles gvAdminUsers.RowEditing
        Try
            ...
            If GetScalar("select count(*) from DWH.AR_GECC.ActivityReport_Users where Active = 1 and AssignRoles = 1 and UserLogin = '" & Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "'") > 0 Then
    ...
    Private Sub gvAdminUsers_RowUpdating(sender As Object, e As GridViewUpdateEventArgs) Handles gvAdminUsers.RowUpdating
        Try
            ...
            If Len(Trim(txtLocking.Text)) = 0 Then
                y = " null, "
            Else
            ...
            If Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") = "e210635" Then
                ...
                Exit Sub
            End If
            Dim Sql As String = "update DWH.AR_GECC.ActivityReport_Users " & _
                "set UserDropDownListName = '" & Replace(txtShortName.Text, "'", "''") & "', MultipleLocks = " & y & " EmailAddress = " & z & _
                " DateModified = getdate(), ModifiedBy = '" & _
                Replace(Request.ServerVariables("AUTH_USER"), "NS\", "") & "' where UserLogin = '" & Replace(depid, "'", "''") & "'"
    ...
    Private Sub btnConfirmSrs2_Click(sender As Object, e As EventArgs) Handles btnConfirmSrs2.Click
        Try
            Dim x As String = "update DWH.AR_GECC.ActivityReport " & _
                ...
                "	where Active = 1 and ActivityDate = '" & Replace(Trim(txtARDate.Text), "'", "''") & "' " & _
                "	 " & _
                "update DWH.AR_GECC.ActivityReport_Splitting " & _
                "set Active = 0 " & _
                "	where Active = 1 and ActivityDate = '" & Replace(Trim(txtARDate.Text), "'", "''") & "' " & _
                "	 " & _
                "update DWH.AR_GECC.ActivityReport_Transfers " & _
                "set Active = 0 " & _
