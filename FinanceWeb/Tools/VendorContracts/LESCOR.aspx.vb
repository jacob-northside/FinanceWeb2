...
Public Class LESCOR1
    Inherits System.Web.UI.Page


    Dim LegalPendingApprovalColor As String = "#d5eaff"
    Dim LegalqueueColor As String = "#ffffaa"
    Dim LegalInProgressColor As String = "#e4f7ae"
    Dim LegalOnHoldColor As String = "#c6BCe7"
    ...
    Dim LegalApprovedColor As String = "#bbffbb"
    Dim LegalClosedColor As String = "#ffd7ff"
    Dim LegalLoadedColor As String = "#c4ffff"
    ...
    'Dim PendingLegalApprovalColor As String = "#ffffaa"

    'Dim ClosedColor As String = "#c6BCe7"

    'Dim ApprovedColor As String = "#bbffbb"
    'Dim ApprovedColor As String = "#e4f7ae"
    ...
    Protected Sub x() Handles Me.LoadComplete
        If IsPostBack Then
        Else
            If GetScalar("select count(*) from WebFD.vendorContracts_Testing.Department_2_User " & _
         "where InactivatedDate is null and roleID in (10, 11)  and UserLogin =  '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "'") > 0 Then
                'Try
                '    ddlLegalAssignedTo.SelectedValue = Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''")
            ...
            Else
                ...
                'Try
                '    'ddlLegalRequestor.SelectedValue = Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''")
                'Catch ex As Exception
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            Else
                ...
                Session("TempTableName") = Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "") & "_" & (New Random()).Next().ToString()
                ...
                If Legal.Text = "1" Then
                    ...
                    cbLegalShowApproved.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalApprovedColor)
                    ...
                    cbLegalInProgress.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalInProgressColor)
                    cbLegalOnHold.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalOnHoldColor)
                    cbLegalShowQueue.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalQueueColor)
        ...
        Catch ex As Exception
            LogWebFinError(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), Request.ServerVariables("HTTP_X_FORWARDED_FOR"), Request.ServerVariables("REMOTE_ADDR"), System.Reflection.MethodBase.GetCurrentMethod().ReflectedType.Name, System.Reflection.MethodBase.GetCurrentMethod.Name(), ex.ToString())
        End Try
    End Sub
    ...
    Private Sub CheckPermissions()
        Try
            ...
            Dim x As String = Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''")
            If Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") = "" Then
                lblOpenUserLogin.Text = "You are not logged in.  Please enter your credentials at the top to proceed."
            Else
                lblOpenUserLogin.Text = "Welcome, " & GetString("select  isnull(UserDisplayName, UserFullName) from WebFD.vendorContracts_Testing.Users where InactivatedDate is null and UserLogin = '" & _
            ...
            '    "where r.RoleName = 'Developer' and UserName = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "'") > 0 Then
            '    Developer.Text = "1"
            ...
            If GetScalar("select count(*) from WebFD.vendorContracts_Testing.Users " & _
                    "where InactivatedDate is null and Admin = 1 and UserLogin =  '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "'") > 0 Then
                Admin.Text = "1"
                tpAdministrative.Visible = True
                PopulateDeptGV()
                'PopulateGrantAccess()
            ...
            '"    where u.Active = 1 and u.UserLogin = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "') order by ord, UserName"

            '7/24/2019 CRW
            Dim sql As String = " select '(View All)' as UserName, '-1' as UserLogin, 0 as ord union all " & _
" select distinct isnull(isnull(u.UserDisplayName, u.UserFullName), ch.RequesterUserLogin), ch.RequesterUserLogin, 1 " & _
"from WebFD.VendorContracts_Testing.ContractHeader ch " & _
"left join (select *, ROW_NUMBER() over (partition by UserLogin order by Isnull(InactivatedDate, '12/31/9999') desc) rn from WebFD.vendorContracts_Testing.Users) u on ch.RequesterUserLogin = u.UserLogin and u.rn = 1 " & _
"where ch.InactivatedDate is null " & _
" and ch.DepartmentID in (select isnull(d2u.DepartmentID, ch.DepartmentID) " & _
"	from WebFD.vendorContracts_Testing.Users u  " & _
"    left join WebFD.vendorContracts_Testing.Department_2_User d2u on u.UserLogin = d2u.UserLogin and d2u.InactivatedDate is null  " & _
"         and getdate() between isnull(d2u.EffectiveFrom, '1/1/1800') and isnull(d2u.EffectiveTo, '12/31/9999')  " & _
"    left join WebFD.VendorContracts.Department_LU d on isnull(d2u.DepartmentID, d.DepartmentID) = d.DepartmentID and d.Active = 1  " & _
"    where u.InactivatedDate is null and u.UserLogin = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "') order by ord, UserName"
            ...
            If GetScalar("select count(*) from WebFD.vendorContracts_Testing.Department_2_User " & _
            "where InactivatedDate is null and RoleID in (10, 11, 13) and UserLogin =  '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "'") > 0 Then
                Legal.Text = "1"
                'tpPendingContracts.Visible = True
                tpLegalTab.Visible = True
                txtMeditractNumber.Enabled = True
                cbAgreements.Checked = True
                LoadWaitingYourApprovalGrid()
                LoadRecentlyReviewedGrid()
                If gvWaitingonYou.Rows.Count() > 0 Then
                    tpPendingContracts.Visible = True
                ElseIf gvRecentlyReceivedContracts.Rows.Count() > 0 Then
                    tpPendingContracts.Visible = True
                End If
            ...
            ' Defunct 7/24/2019; Legals now in D_2_users
            'If Legal.Text = "1" Then
            ...
            '     "					left join WebFD.VendorContracts.Department_2_User d2u on u.UserLogin = d2u.UserLogin and d2u.Active = 1 " & _
            '     "					left join WebFD.VendorContracts.Department_LU d on isnull(d2u.DepartmentID, d.DepartmentID) = d.DepartmentID and d.Active = 1  " & _
            '     "					where u.Active = 1 and u.UserLogin = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & _
            ...
            sql2 = " select '(View All Available)' as CC, -1 as DepartmentID, 0 as ord union all " & _
             ...
             "                      and getdate() between isnull(d2u.EffectiveFrom, '1/1/1800') and isnull(d2u.EffectiveTo, '12/31/9999') " & _
             "					left join WebFD.VendorContracts.Department_LU d on isnull(d2u.DepartmentID, d.DepartmentID) = d.DepartmentID and d.Active = 1  " & _
             "					where u.InactivatedDate is null and u.UserLogin = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & _
             "'  and (d2u.DepartmentID is null or d.DepartmentID is not null) order by 3, 1"
            ...
            ' Defunct 7/24/2019; wasn't actually using anything, unnecessarily complicated
            ...
            '    "   ) Main " & _
            '    "	where UserLogin = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' "
            ...
            Dim s As String = "select  u.UserLogin, isnulL(u.UserDisplayName  , u.UserFullName) as UserDisplayName  " & _
                "    from WebFD.vendorContracts_Testing.Users u  " & _
                "    left join WebFD.vendorContracts_Testing.Department_2_User d2u on u.UserLogin = d2u.UserLogin  and d2u.InactivatedDate is null " & _
                "       and getdate() between isnull(d2u.EffectiveFrom, '1/1/1800') and isnull(d2u.EffectiveTo, '12/31/9999') " & _
                "    left join WebFD.VendorContracts.Department_LU d on d2u.DepartmentID = d.DepartmentID and d.Active = 1  " & _
                "    where u.InactivatedDate is null  and (d2u.DepartmentID is null or d.DepartmentID is not null)  " & _
                "     and u.UserLogin =  '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' "
    ...
    Private Sub PopulateHierarchiesGrid()
        ...
        Dim y As String = "	select RoleMatch, left(Reqs, len(Reqs) - 1) as Users, Hierarchy from (  " & _
            ...
            "                                    (   " & _
            "                                        Select UserName + ', ' AS [text()]   " & _
            "                                        from (select isnull(isnull(u.UserDisplayName, u.UserFullName), d2u.UserLogin) as UserName, r.RoleFull, r.RoleID   " & _
            "            				from WebFD.vendorContracts_Testing.Role_LU r  " & _
            "            				join WebFD.vendorContracts_Testing.Department_2_User d2u on r.RoleID = d2u.RoleID and d2u.InactivatedDate is null " & _
                            "       and getdate() between isnull(d2u.EffectiveFrom, '1/1/1800') and isnull(d2u.EffectiveTo, '12/31/9999') " & _
            "            				join WebFD.VendorContracts.Department_LU d on d2u.DepartmentID = d.DepartmentID and d.Active = 1  " & _
            "            				join WebFD.vendorContracts_Testing.Users u on d2u.UserLogin = u.UserLogin and u.InactivatedDate is null  " & _
            "            				where r.InactivatedDate is null and d.DepartmentID = '" & Replace(ddlDepartmentNo.SelectedValue, "'", "''") & "') ST1     " & _
            "            				where (ST1.RoleID = r.RoleID)  " & _
            "                                          " & _
            "                                        For XML PATH ('')   " & _
            "                                   ) Reqs   " & _
            "                                From WebFD.vendorContracts_Testing.Role_LU r    " & _
            "                        		where r.InactivatedDate is null and DisplayRole = 1 ) x  " & _
            "            					where Reqs is not null  " & _
            "            					order by Hierarchy "
    ...
    Private Sub PopulateSignaturesGrid()
        ...
        'Currently hard-coded; anticipate a change
        Dim x As String = "		select null as LowerBound, 50000.00 as UpperBound, 'Less than or equal to $50,000' as ImpactRange, 'Requestor and Direct Supervisor' as ApprovalRequired " & _
"		union " & _
"		select 50000.00, 250000.00, 'Greater than $50,000 but less than or equal to $250,000', 'Requestor, Direct Supervisor, and Northside System VP or Campus CEO' " & _
"		union " & _
"		select 250000.01, 500000.00, 'Greater than $250,000 but less than or equal to $500,000', 'Requestor, Direct Supervisor, Northside System VP or Campus CEO, Financial Planning and Analysis, Northside System Controller' " & _
"		union " & _
"		select 500000.01, null, 'Greater than $500,000', 'Requestor, Direct Supervisor, Northside System VP or Campus CEO, Financial Planning and Analysis, Northside System CFO' " & _
"		order by LowerBound "
    ...
    Private Sub PopulateDepartmentDDL()
        ...
        '    "					left join WebFD.VendorContracts.Department_2_User d2u on u.UserLogin = d2u.UserLogin and d2u.Active = 1 " & _
        '    "					left join WebFD.VendorContracts.Department_LU d on isnull(d2u.DepartmentID, d.DepartmentID) = d.DepartmentID and d.Active = 1 " & _
        '    "					where u.Active = 1 and u.UserLogin = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "'  and (d2u.DepartmentID is null or d.DepartmentID is not null) order by 3, 1"
        ...
        Dim sql As String = " select '(Select Cost Center)' as DepartmentSelected, -1 as DepartmentID, 0 as ord union all " & _
            ...
            "                       and getdate() between isnull(d2u.EffectiveFrom, '1/1/1800') and isnull(d2u.EffectiveTo, '12/31/9999') " & _
            "					left join WebFD.VendorContracts.Department_LU d on isnull(d2u.DepartmentID, d.DepartmentID) = d.DepartmentID and d.Active = 1 " & _
            "					where u.InactivatedDate is null and u.UserLogin = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "'  and (d2u.DepartmentID is null or d.DepartmentID is not null) order by 3, 1"
    ...
    Private Function SubmissionView()
        ...
        '        Return GetData("	select ROW_NUMBER() over (order by qo.[Order], qo.QuestionID) as RN, Question, q.QuestionID, q.Method,   " & _
        '"		convert(varchar, ROW_NUMBER() over (order by qo.[Order], qo.QuestionID)) + '. ' as DisplayingNo " & _
        '"		from WebFD.VendorContracts_Testing.Question_LU q " & _
        '"		join WebFD.vendorContracts_Testing.question_Order qo on q.QuestionID = qo.questionID " & _
        '"			and Qo.INactivatedDate is null and GETDATE() between isnull(qo.EffectiveFrom, '1/1/1800') and isnull(qo.EffectiveTo, '12/31/9999') " & _
        '"			where q.InactivatedDate is null and isnull(SummaryQuestion, 0) = 0 order by 1").DefaultView

        Return GetData("select ROW_NUMBER() over (order by qo.[Order], qo.QuestionID) as RN, Question, q.QuestionID, q.Method,    " & _
"		convert(varchar, ROW_NUMBER() over (order by qo.[Order], qo.QuestionID)) + '. ' as DisplayingNo  " & _
"		, isnull(ResponseQuestion, 'Please Explain') as ResponseQuestion " & _
"		from WebFD.VendorContracts_Testing.Question_LU q  " & _
"		join WebFD.vendorContracts_Testing.question_Order qo on q.QuestionID = qo.questionID  " & _
"			and Qo.INactivatedDate is null and GETDATE() between isnull(qo.EffectiveFrom, '1/1/1800') and isnull(qo.EffectiveTo, '12/31/9999')  " & _
"		left join (select QuestionID, responseQuestion from WebFD.vendorContracts_Testing.Question_Answer_LU where InactivatedDate is null and responseQuestion is not null) qr " & _
"				on qr.QuestionID = q.QuestionID " & _
"			where q.InactivatedDate is null and isnull(SummaryQuestion, 0) = 0 order by 1 ").DefaultView
    ...
    Private Sub loadgrid()
        ...
        Dim gridsql2 As String = "	select ROW_NUMBER() over (order by qo.[Order], qo.QuestionID) as RN, Question, q.QuestionID, q.Method,   " & _
            "		convert(varchar, ROW_NUMBER() over (order by qo.[Order], qo.QuestionID)) + '. ' as DisplayingNo " & _
            "		from WebFD.VendorContracts_Testing.Question_LU q " & _
            "		join WebFD.vendorContracts_Testing.question_Order qo on q.QuestionID = qo.questionID " & _
            "			and Qo.INactivatedDate is null and GETDATE() between isnull(qo.EffectiveFrom, '1/1/1800') and isnull(qo.EffectiveTo, '12/31/9999') " & _
            "			where q.InactivatedDate is null and isnull(SummaryQuestion, 0) = 1 order by 1"
    ...
    Private Sub loadprintgrid()
        ...
        '"			where q.Active = 1 and pc.ModifiedBy = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & _
        ...
        '7/25/2019
        Dim gridsql As String = "select ROW_NUMBER() over (order by qo.[Order], qo.QuestionID) as RN " & _
"           	, Question " & _
"           	,q.QuestionID " & _
"           	, q.Method " & _
"			, pc.Response, pc.AnswerComment, pc.AnswerDate, case when pc.AnswerComment is null then 'false' else 'true' end ViewComment " & _
...
"           from WebFD.VendorContracts_Testing.Question_LU q " & _
        "		join WebFD.vendorContracts_Testing.question_Order qo on q.QuestionID = qo.questionID " & _
        "			and Qo.INactivatedDate is null and GETDATE() between isnull(qo.EffectiveFrom, '1/1/1800') and isnull(qo.EffectiveTo, '12/31/9999') " & _
"			left join WebFD.VendorContracts_Testing.PendingContract_Answers pc on q.QuestionID = pc.QuestionID and pc.InactivatedDate is null " & _
"			where q.InactivatedDate is null and pc.AddedBy = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & _
    "' and pc.ContractID = '" & Replace(AttachIndex.Text.ToString, "'", "''") & "' and isnull(SummaryQuestion, 0) = 0 " & _
"          order by RN "

        Dim gridsql2 As String = "select ROW_NUMBER() over (order by qo.[Order], qo.QuestionID) as RN " & _
"           	, Question " & _
"           	,q.QuestionID " & _
"           	, q.Method " & _
"			, pc.Response, pc.AnswerComment, pc.AnswerDate, case when pc.AnswerComment is null then 'false' else 'true' end ViewComment " & _
...
"			left join WebFD.VendorContracts_Testing.PendingContract_Answers pc on q.QuestionID = pc.QuestionID and pc.InactivatedDate is null " & _
"			where q.InactivatedDate is null and pc.AddedBy = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & _
    "' and pc.ContractID = '" & Replace(AttachIndex.Text.ToString, "'", "''") & "' and isnull(SummaryQuestion, 0) = 1 " & _
"          order by RN "
    ...
    Private Sub loadApprovalgrid(x As Integer)
        ...
        '7/25/2019 CRW
        Dim gridsql As String = "select ROW_NUMBER() over (order by qo.[Order], qo.QuestionID) as RN , convert(varchar,ROW_NUMBER() over (order by qo.[Order], qo.QuestionID))  + '. ' as DisplayingNo " & _
"           	, Question " & _
"           	,q.QuestionID " & _
"           	, q.Method " & _
"			, pc.Response, pc.AnswerComment, pc.AnswerDate, case when pc.AnswerComment is null then 'false' else 'true' end ViewComment " & _
"           from WebFD.VendorContracts_Testing.Question_LU q " & _
"			join WebFD.VendorContracts_Testing.Contract_Answers pc on q.QuestionID = pc.QuestionID and pc.InactivatedDate is null " & _
        "		join WebFD.vendorContracts_Testing.question_Order qo on q.QuestionID = qo.questionID " & _
        "			and Qo.INactivatedDate is null and pc.AddedDate between isnull(qo.EffectiveFrom, '1/1/1800') and isnull(qo.EffectiveTo, '12/31/9999') " & _
"			where q.InactivatedDate is null and pc.ContractID = '" & x.ToString & "' and isnull(SummaryQuestion, 0) = 0 " & _
"          order by RN "
        ' 10/12/2017 CRW Revised join instead of left join and to remove the q.Active = 1 so that all contracts can be viewed as submitted

        Dim gridsql2 As String = "select ROW_NUMBER() over (order by qo.[Order], qo.QuestionID) as RN, convert(varchar,ROW_NUMBER() over (order by qo.[Order], qo.QuestionID))  + '. ' as DisplayingNo " & _
"           	, Question " & _
"           	,q.QuestionID " & _
"           	, q.Method " & _
"			, pc.Response, pc.AnswerComment, pc.AnswerDate, case when pc.AnswerComment is null then 'false' else 'true' end ViewComment  " & _
...
"			join WebFD.VendorContracts_Testing.Contract_Answers pc on q.QuestionID = pc.QuestionID and pc.InactivatedDate is null " & _
        "		join WebFD.vendorContracts_Testing.question_Order qo on q.QuestionID = qo.questionID " & _
        "			and Qo.INactivatedDate is null and pc.AddedDate between isnull(qo.EffectiveFrom, '1/1/1800') and isnull(qo.EffectiveTo, '12/31/9999') " & _
"			where q.InactivatedDate is null and pc.ContractID = '" & x.ToString & "' and isnull(SummaryQuestion, 0) = 1 " & _
"          order by RN "

        '9/15/2020 CRW fixed bug where it was using Gedate() instead of PC.AddedDate
    ...
    Private Sub loadLegalgrid(x As Integer)
        ...
        Dim gridsql As String = "select *, convert(varchar, RN) + '. ' as DisplayingNo	 " & _
"           	   from ( " & _
"           select Dense_Rank() over (order by qo.[Order], q.QuestionID asc " & _
"           	)   as RN " & _
"           	, Question " & _
"           	,q.QuestionID " & _
"           	, q.Method " & _
"			, pc.Response, pc.AnswerComment, pc.AnswerDate, case when pc.AnswerComment is null then 'false' else 'true' end ViewComment " & _
"           from WebFD.VendorContracts_Testing.Question_LU q " & _
"			left join WebFD.VendorContracts_Testing.Contract_Answers pc on q.QuestionID = pc.QuestionID and pc.InactivatedDate is null " & _
    "		join WebFD.vendorContracts_Testing.question_Order qo on q.QuestionID = qo.questionID " & _
        "			and Qo.INactivatedDate is null and pc.AddedDate between isnull(qo.EffectiveFrom, '1/1/1800') and isnull(qo.EffectiveTo, '12/31/9999') " & _
"			where (q.InactivatedDate is null or pc.InactivatedDate is null) and pc.ContractID = '" & x.ToString & "' and isnull(SummaryQuestion, 0) = 0) N " & _
"          order by RN "
...
"			left join WebFD.VendorContracts_Testing.Contract_Answers pc on q.QuestionID = pc.QuestionID and pc.InactivatedDate is null " & _
    "		join WebFD.vendorContracts_Testing.question_Order qo on q.QuestionID = qo.questionID " & _
        "			and Qo.INactivatedDate is null and pc.AddedDate between isnull(qo.EffectiveFrom, '1/1/1800') and isnull(qo.EffectiveTo, '12/31/9999') " & _
"			where (q.InactivatedDate is null or pc.InactivatedDate is null) and pc.ContractID = '" & x.ToString & "' and isnull(SummaryQuestion, 0) = 1) N " & _
"          order by RN "
    ...
    Private Sub loadprintattachmentgrid()
        ...
        Dim Str As String = "select * from WebFD.VendorContracts_Testing.PendingContractAttachments " & _
            "where ContractID = '" & AttachIndex.Text & "' and InactivatedDate is null and content is not null and AddedBy = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' "
    ...
    Private Sub ResetPage()
        Try
            ...
            '"select '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & "', 1 where not exists (select * from WebFD.VendorContracts.PendingContractHeader " & _
            '"	where Active = 1 and UserLogin = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & "') " & _
            '"" & _
            ...
            '"where UserLogin = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & "' and a.Active = 1 "
            ...
            Dim AttachInt As String = "insert  WebFD.VendorContracts_Testing.PendingContractAttachments " & _
                "output Inserted.ContractID " & _
                "select isnull(max(ContractID), 0) + 1, null, null, null, getdate(), '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "', null, null " & _
                "from WebFD.VendorContracts_Testing.PendingContractAttachments " & _
                "where AddedBy = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' "
            ...
            search.Filter = "(&(objectClass=user)(samaccountname=" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & "))"
            ...
            lblSubRequestorID.Text = Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "")
    ...
    Private Function ApprovalView()
        ...
        '            ") x  " & _
        '            "where RNK = 1 and ApprovalSeekLogin = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' ").DefaultView
        ...
        '    "                   where RNK = 1  and isnull(ApprovalSeekLogin, u.UserLogin) = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' ").DefaultView

        ' Revamp 7/31/2019 CRW

        Return GetData("select ch.ContractID, ch.AddedDate as DateAdded, ch.ContractName, ch.DepartmentID, d.DepartmentNo, d.DepartmentName, isnull(ru.UserDisplayName, ru.UserFullName)  as Requestor " & _
                    "from WebFD.vendorContracts_Testing.ContractHeader ch with (nolock) " & _
                    "join WebFD.vendorContracts_Testing.Contract_Approval_Pending cap with (nolock) on ch.ContractID = cap.ContractID and cap.InactivatedDate is null " & _
                    "left join WebFD.vendorContracts_Testing.Role_LU r with (nolock) on cap.Hierarchy = r.Hierarchy and r.InactivatedDate is null and ApprovalRole = 1 " & _
                    "left join WebFD.vendorContracts_Testing.Department_2_User d2u with (nolock) on d2u.RoleID = r.RoleID and isnull(d2u.DepartmentID, ch.DepartmentID) = ch.DepartmentID  " & _
                    "	and d2u.InactivatedDate is null and GETDATE() between isnull(d2u.EffectiveFrom, '1/1/1800') and isnull(d2u.EffectiveTo, '12/31/9999') " & _
                    "left join WebFD.vendorContracts.Department_LU d with (nolock) on d.DepartmentID = ch.DepartmentID and d.Active = 1 " & _
                    "left join WebFD.vendorContracts_Testing.Users ru with (nolock) on ch.RequesterUserLogin = ru.UserLogin and ru.InactivatedDate is null " & _
                    "where d2u.UserLogin = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' and ch.InactivatedDate is null " & _
                    "and not exists (select * from WebFD.vendorContracts_Testing.Contract_Status cs with (nolock) where cs.ContractID = ch.ContractID and (cs.StatusTypeID = 3 or cs.StatusID in ( 7)) and cs.InactivatedDate is null) " & _
                    "and not exists (select * from WebFD.vendorContracts_Testing.Contract_Approvals ca with (nolock) where ca.ContractID = ch.ContractID and d2u.UserLogin = ca.UserLogin and ca.InactivatedDate is null) ").DefaultView

        ' Only excludes deleted re Ryan 11/11/2020 CRW
    ...
    Private Function ReviewedView()
        ...
        '    Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "')	").DefaultView()

        ' Revamp 7/31/2019 CRW

        ' /* Switched to left join 3/4/2020  CRW */
        'Return GetData("select ch.ContractID, ch.AddedDate as DateAdded, ch.ContractName, ch.DepartmentID, d.DepartmentNo, d.DepartmentName as DepartmentFull " & _
        '    "		, isnull(ru.UserDisplayName, ru.UserFullName)  as Requestor " & _
        '    "	, csl.StatusShortDescription as UserStatus " & _
        '    "	from WebFD.vendorContracts_Testing.ContractHeader ch with (nolock) " & _
        '    "	JOIN (select *, ROW_NUMBER() over (partition by ContractID order by AddedDate desc) rn from WebFD.vendorContracts_Testing.Contract_Approvals with (nolock) where InactivatedDate is null " & _
        '    "   and AddedDate between dateadd(day, -1, getdate()) and getdate() ) ca on ch.ContractID = ca.ContractID and ca.rn = 1 " & _
        '    "	join WebFD.vendorContracts_Testing.Department_2_User d2u with (nolock) on d2u.UserLogin = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & _
        '    "' and isnull(d2u.DepartmentID, ch.DepartmentID) = ch.DepartmentID  " & _
        '    "		and d2u.InactivatedDate is null and GETDATE() between isnull(d2u.EffectiveFrom, '1/1/1800') and isnull(d2u.EffectiveTo, '12/31/9999') " & _
        '    "	left join WebFD.vendorContracts.Department_LU d with (nolock) on d.DepartmentID = ch.DepartmentID and d.Active = 1 " & _
        '    "	left join WebFD.vendorContracts_Testing.Users ru with (nolock) on ch.RequesterUserLogin = ru.UserLogin and ru.InactivatedDate is null " & _
        '    "	left join WebFD.vendorContracts_Testing.ContractStatus_LU csl with (nolock) on ca.StatusID = csl.StatusID and ca.StatusTypeID = csl.StatusTypeID " & _
        '    "		and csl.InactivatedDate is null and getdate() between isnulL(csl.EffectiveFrom, '1/1/1800') and isnull(csl.EffectiveTo, '12/31/9999') " & _
        '    "	where ch.InactivatedDate is null order by ContractID").DefaultView

        ' /*Switched again 10/12/2023 */
        Return GetData("select ch.ContractID, ch.AddedDate as DateAdded, ch.ContractName, ch.DepartmentID, d.DepartmentNo, d.DepartmentName as DepartmentFull " & _
    "		, isnull(ru.UserDisplayName, ru.UserFullName)  as Requestor " & _
    "	, csl.StatusShortDescription as UserStatus " & _
    "	from WebFD.vendorContracts_Testing.ContractHeader ch with (nolock) " & _
    ...
    "		and csl.InactivatedDate is null and getdate() between isnulL(csl.EffectiveFrom, '1/1/1800') and isnull(csl.EffectiveTo, '12/31/9999') " & _
    "	where ch.InactivatedDate is null " & _
    " and exists (Select * from  WebFD.vendorContracts_Testing.Department_2_User d2u with (nolock) where d2u.UserLogin = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' " & _
    " and  (d2u.DepartmentID = ch.DepartmentID  	 or 	d2u.DepartmentID is null) " & _
    " and d2u.InactivatedDate is null and GETDATE() between isnull(d2u.EffectiveFrom, '1/1/1800') and isnull(d2u.EffectiveTo, '12/31/9999') " & _
    " ) order by ContractID").DefaultView
    ...
    Private Sub LoadExpenseChoices()
        ...
        ' 7/31/2019 CRW
        Dim expenseString As String = "select ContractExpenseID, LongDescription, MoneyValue as ord  " & _
            "From WebFD.vendorContracts_Testing.Contract_Expense_LU where InactivatedDate is null  " & _
            "	and getdate() between isnull(EffectiveFrom, '1/1/1800') and isnull(EffectiveTo, '12/31/9999') " & _
            "union select -1, '(Select Annual Expense)', 0 " & _
            "order by ord  "

        ddlSubmitContractCost.DataSource = GetData(ExpenseString)
        ddlSubmitContractCost.DataTextField = "LongDescription"
        ddlSubmitContractCost.DataValueField = "ContractExpenseID"
    ...
    Private Sub gvSubmissionQuestions_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles gvSubmissionQuestions.RowDataBound
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            Dim answerString As String = "Select * from WebFD.VendorContracts_Testing.Question_Answer_LU where InactivatedDate is null and QuestionID = '" & e.Row.DataItem("QuestionID") & "' "
    ...
    '            "where UserLogin = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & "' and Active = 1"
    ...
    '        Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & "' where not exists (select * from WebFD.VendorContracts.Vendor_LU where VendorTin = '" & _
    '        Replace(Trim(txtNewVendorTIN.Text), "'", "''") & "' and Active = 1)"
    ...
    '                "1, getdate(), getdate(), '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' " & _
    ...
    Private Sub btnSubmitContract_Click(sender As Object, e As EventArgs) Handles btnSubmitContract.Click
        Try
            ...
            Dim AnswerSQL As String = "Delete from WebFD.VendorContracts_Testing.PendingContract_Answers where AddedBy = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & _
            ...
            For i As Integer = 0 To gvSubmissionQuestions.Rows.Count - 1
                ...
                Dim NewRow As Integer = 0
                AnswerSQL += "Insert into WebFD.VendorContracts_Testing.PendingContract_Answers values ('" & Replace(AttachIndex.Text.ToString, "'", "''") & "', '" & QuestionID & "', '"
                ...
                AnswerSQL += ", getdate(), '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "', null, null )"
            ...
            If GetScalar("select count(*) from WebFD.VendorContracts_Testing.PendingContractAttachments " & _
                "where ContractID = '" & AttachIndex.Text & "' and Content is not null and InactivatedDate is null and AddedBy = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "'") < 1 Then
            ...
            '    "Output Inserted.ContractID select '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & _
            ...
            'SubmitSQL += Replace(ddlSubContractExpense.SelectedValue, "'", "''") & ", 1, getdate(), getdate(), '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & _
            '    "', 1 "
            ...
            '    AnswerSql += "', 1, getdate(), getdate(), '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "') "
            ...
            '    Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' and Active = 1 " & _
            '    " Delete from WebFD.VendorContracts.PendingContractAttachments where UserLogin = '" & _
            '    Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' and Active = 1 "

            ''"insert into WebFD.VendorContracts.Users " & _
            ''"select '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "', '" & Replace(lblSubRequestor.Text, "'", "''") & "', null, 1, " & _
            ''" getdate(), getdate(), 'Automatic', 0, 0, 0, 0 " & _
            ''"where not exists (select * from WebFD.VendorContracts.Users where UserLogin = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "') "
...
#Region "OldGrids"
    ...
    '        "left join WebFD.VendorContracts.Users uPulled on uPulled.UserLogin = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' " & _
    ...
    '          " or (UserLogin = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' and 1 = " & AllMine.ToString & " )"
    ...
    '"and d2u.Active = 1 and d2u.UserLogin = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' " & _
    ...
    '"join WebFD.VendorContracts.Users u on UserLogin = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' and u.Active = 1 " & _
...
#End Region
        ...
        Try
            ...
            For i As Integer = 0 To q.Rows.Count - 1
                ...
                If cbResponse.Visible And cbResponse.Checked Then
                    s = "Select isnull(count(*), 0) from WebFD.VendorContracts_Testing.Question_Answer_LU where QuestionID = '" & Replace(q.DataKeys(i).Value.ToString, "'", "''") & _
                                        "' and InactivatedDate is null and CommentRequired = 1 and AnswerValue = '" & Replace(cbResponse.Text, "'", "''") & "'  "
                    y = "Select isnull(count(*), 0) from WebFD.VendorContracts_Testing.Question_Answer_LU where QuestionID = '" & Replace(q.DataKeys(i).Value.ToString, "'", "''") & _
                                        "' and InactivatedDate is null and DateRequired = 1 and AnswerValue = '" & Replace(cbResponse.Text, "'", "''") & "'  "
                ...
                ElseIf rblResponse.Visible Then
                    s = "Select isnull(count(*), 0) from WebFD.VendorContracts_Testing.Question_Answer_LU where QuestionID = '" & Replace(q.DataKeys(i).Value.ToString, "'", "''") & _
                                        "' and InactivatedDate is null and CommentRequired = 1 and AnswerValue = '" & Replace(rblResponse.SelectedValue, "'", "''") & "' "
                    y = "Select isnull(count(*), 0) from WebFD.VendorContracts_Testing.Question_Answer_LU where QuestionID = '" & Replace(q.DataKeys(i).Value.ToString, "'", "''") & _
                                       "' and InactivatedDate is null and DateRequired = 1 and AnswerValue = '" & Replace(rblResponse.SelectedValue, "'", "''") & "'  "
                    ' x = "Select isnull(count(*), 0) from WebFD.VendorContracts.Question_Answer_LU where QuestionID = '" & Replace(q.DataKeys(i).Value.ToString, "'", "''") & _
                    '"' and Active = 1 and RequiredAnswer = 0 and AnswerValue = '" & Replace(rblResponse.SelectedValue, "'", "''") & "' "
                Else
    ...
    Private Sub btnConfirmContract_Click(sender As Object, e As EventArgs) Handles btnConfirmContract.Click
        Try
            ...
            Dim insertsql As String = "Insert WebFD.VendorContracts_Testing.ContractHeader " & _
                " output inserted.ContractID " & _
                " select (select isnull(max(ContractID), 0) + 1 from WebFD.VendorContracts_Testing.ContractHeader), '" & _
                 Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & _
                 "', '" & Replace(txtSubmitContractName.Text, "'", "''") & "', '" & Replace(ddlDepartmentNo.SelectedValue.ToString, "'", "''") & _
            ...
            insertsql += " '" & Replace(ddlSubmitContractBudgetAcct.SelectedValue, "'", "''") & "',  getdate(), '" & _
                Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "', null, null "

            Dim x As Integer = GetScalar(insertsql)

            'Dim Furthersql As String = "insert into WebFD.VendorContracts_Testing.ContractAttachments " & _
            '   " select (select isnull(max(AttachmentID),0) + 1 from WebFD.VendorContracts_Testing.ContractAttachments), '" & x & "', FileName, ContentType, Content, Dateadded, UserID, null, null " & _
            '   "from WebFD.VendorContracts_Testing.PendingContractAttachments where ContractID = '" & AttachIndex.Text & "' and InactivatedDate is null and UserID = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "'  " & _
            '   " Insert into WebFD.VendorContracts_Testing.Contract_Answers select '" & x & "', QuestionID, Response, AnswerComment, AnswerDate, AddedDate, AddedBy, InactivatedDate, InactivatedBy " & _
            '   "from WebFD.VendorContracts_Testing.PendingContract_Answers where ContractID = '" & AttachIndex.Text & "' and ModifiedBy = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' and InactivatedDate is null " & _
            '   "Insert into WebFD.VendorContracts_Testing.Contract_Comments " & _
            '   " select (select isnull(max(CommentID),0) + 1 from WebFD.VendorContracts_Testing.Contract_Comments), '" & x & "', 'Submitter', '" & Replace(txtSubmissionComments.Text, "'", "''") & "', 'Submission', 0, getdate(), '" & _
            '   Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "', null, null " & _
            '           "insert into WebFD.VendorContracts_Testing.Contract_Approvals " & _
            ...
            '   "where d2u.UserLogin = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & " ' and ch.Active = 1 and d2u.Position = 'VP' and ContractID = '" & x & "' " & _
            ...
            '   "where d2u.UserLogin = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' and ch.Active = 1 and ContractID = '" & x & "' " & _
            '   "and not exists (select * from WebFD.VendorContracts.Contract_Approvals ca  " & _
            '   "		where ca.ContractID = ch.ContractID and ca.Active = 1 and ca.UserLogin <> 'Default Insert' ) "

            Dim Furthersql As String = "insert into WebFD.VendorContracts_Testing.ContractAttachments " & _
                " select (select isnull(max(AttachmentID),0) from WebFD.VendorContracts_Testing.ContractAttachments) + row_Number() over (order by AddedDate), '" & x & "', FileName, ContentType, Content, AddedDate, AddedBy, null, null " & _
                "from WebFD.VendorContracts_Testing.PendingContractAttachments where ContractID = '" & AttachIndex.Text & "' and InactivatedDate is null and Content is not null and AddedBy = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "'  " & _
                " Insert into WebFD.VendorContracts_Testing.Contract_Answers select '" & x & "', QuestionID, Response, AnswerComment, AnswerDate, AddedDate, AddedBy, InactivatedDate, InactivatedBy " & _
                "from WebFD.VendorContracts_Testing.PendingContract_Answers where ContractID = '" & AttachIndex.Text & "' and AddedBy = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' and InactivatedDate is null " & _
                "Insert into WebFD.VendorContracts_Testing.Contract_Comments " & _
                " select (select isnull(max(CommentID),0) + 1 from WebFD.VendorContracts_Testing.Contract_Comments), '" & x & "', 'Submitter', '" & Replace(txtSubmissionComments.Text, "'", "''") & "', 'Submission', 0, getdate(), '" & _
                Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "', null, null " & _
                "insert into WebFD.vendorContracts_Testing.Contract_Approvals (ContractID, RoleID, UserLogin, StatusTypeID, StatusID, Comment, AddedDate, AddedBy) " & _
                "select ch.ContractID, d2u.RoleID, RequesterUserLogin, 3, 13, 'Requestor', getdate(), 'Default Insert' " & _
                "from WebFD.vendorContracts_Testing.ContractHeader ch " & _
                "join WebFD.vendorContracts_Testing.Department_2_User d2u on ch.DepartmentID = isnull(d2u.DepartmentID, ch.DepartmentID) and  d2u.InactivatedDate is null   " & _
                "	and getdate() between isnull(d2u.EffectiveFrom, '1/1/1800') and isnull(d2u.EffectiveTo, '12/31/9999') " & _
                "where d2u.UserLogin = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' and ch.InactivatedDate is null and ContractID = '" & x & "' " & _
                "and not exists (select * from WebFD.VendorContracts_Testing.Contract_Approvals ca  " & _
                "		where ca.ContractID = ch.ContractID and ca.InactivatedDate is null and ca.UserLogin <> 'Default Insert' ) "



            ExecuteSql(Furthersql)

            Dim BonusSQL1 As String = "exec WebFD.vendorContracts_Testing.Update_Contract_Status_New '" & x & "'"
            ExecuteSql(BonusSQL1)


            Dim BonusSql As String = "Insert into WebFD.VendorContracts_Testing.EmailStatus values (" & x & ", 'PendingSubmission', 1, null) "
            ExecuteSql(BonusSql)
    ...
    Private Sub gvWaitingonYou_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gvWaitingonYou.SelectedIndexChanged
        ...
        gvApprovalCurrent.DataSource = GetData("select distinct isnull(isnull(u.UserDisplayName, u.UserFullName), ca.UserLogin) as UserName " & _
                ", csl.StatusLongDescription as Status, Comment, ca.AddedDate as ModifyDate " & _
                "from WebFD.VendorContracts_Testing.Contract_Approvals ca with (nolock)  " & _
                " left join WebFD.vendorContracts_Testing.ContractStatus_LU csl with (nolock)  on csl.StatusID = ca.StatusID  and csl.InactivatedDate is null " & _
                "   and getdate() between isnull(csl.EffectiveFrom, '1/1/1800') and isnull(csl.EffectiveTo, '12/31/9999') " & _
                "left join WebFD.vendorContracts_Testing.Users u with (nolock)  on ca.UserLogin = u.UserLogin and u.InactivatedDate is null " & _
                "where ca.ContractID = '" & x & "' and ca.InactivatedDate is null and ca.UserLogin <> 'Default Insert' and ca.StatusTypeID = 3 order by ModifyDate")

        gvApprovalCurrent.DataBind()

        Dim y As DataTable = GetData("select isnull(isnull(u.UserDisplayName, u.UserFullName), ch.RequesterUserLogin) as Requestor , ch.*, cel.LongDescription as AnnualContractExpense " & _
             ", convert(varchar, DepartmentNo) + ' - ' + DepartmentName as Department, ContractType, cp.ContractParty,  convert(varchar, Acct) + ' - ' + Description as ExpenseDescrip, Comment " & _
            "from WebFD.VendorContracts_Testing.ContractHeader ch with (nolock)  " & _
            "left join WebFD.VendorContracts.Department_LU d with (nolock)  on ch.DepartmentID = d.DepartmentID and d.Active = 1 " & _
            "left join WebFD.vendorContracts_Testing.Users u with (nolock)  on ch.RequesterUserLogin = u.UserLogin and u.InactivatedDate is null " & _
            "left join WebFD.VendorContracts.ContractType_LU cl with (nolock)  on cl.ContractTypeID = ch.ContractTypeID and cl.Active = 1 " & _
            "left join WebFD.VendorContracts.ContractParty_LU cp with (nolock)  on ch.ContractingParty = cp.ContractPartyID and cp.Active = 1 " & _
            "left join WebFD.vendorContracts_Testing.Contract_Expense_LU cel with (nolock)  on ch.ContractExpenseID = cel.ContractExpenseID and cel.InactivatedDate is null " & _
            "   and ch.AddedDate between isnull(cel.EffectiveFrom, '1/1/1800') and isnull(cel.EffectiveTo, '12/31/9999') " & _
            "left join DWH.Axiom.Acct a with (nolock)  on a.Acct = ch.ExpenseAccount and ISMAP  in ('SUP', 'FEE', 'OTH') " & _
            "left join WebFD.VendorContracts_Testing.Contract_Comments  cc with (nolock)  on cc.ContractID = ch.ContractID and cc.CommentType = 'Submission' and cc.InactivatedDate is null and LegalOnly = 0 " & _
            " where ch.InactivatedDate is null and ch.ContractID = '" & x & "' ")
        ...
        If IsDBNull(y(0)("AddedDate")) Then
            lblApproveDate.Text = ""
        Else
            lblApproveDate.Text = (y(0)("AddedDate"))
        End If
    ...
    Private Sub btnApproveContract_Click(sender As Object, e As EventArgs) Handles btnApproveContract.Click
        Try
            ExplanationLabelApproval.Text = "You are trying to approve Request ID " & Replace(lblApproveContractID.Text, "'", "''") & ", titled '" & _
                GetString("select ContractName from WebFD.VendorContracts_Testing.ContractHeader where InactivatedDate is null and ContractID = '" & Replace(lblApproveContractID.Text, "'", "''") & "'") & "' <br>  Please confirm."
    ...
    Private Function ContractSearch()
        ...
        ' Rebuild 8/1/2019 CRW
        Dim a As String = "select distinct ch.ContractID, isnull(u.UserDisplayName, u.UserFullName) as Approver " & _
        "	into #TempContractPull_" & Session("TempTableName") & "  " & _
        "	From WebFD.vendorContracts_Testing.ContractHeader ch with (nolock)  " & _
        "	join WebFD.vendorContracts_Testing.Contract_Approval_Pending cap with (nolock)  on ch.ContractID = cap.ContractID and cap.InactivatedDate is null							 " & _
        "	join WebFD.vendorContracts_Testing.Role_LU r with (nolock)  on r.Hierarchy = cap.Hierarchy and r.InactivatedDate is null and ApprovalRole = 1 " & _
        "	join WebFD.vendorContracts_Testing.Department_2_User d2u with (nolock)  on isnull(d2u.DepartmentID, ch.DepartmentID) = ch.DepartmentID  " & _
        "		and d2u.RoleID = r.RoleID and d2u.InactivatedDate is null " & _
        "		and getdate() between isnull(d2u.EffectiveFrom, '1/1/1800') and isnull(d2u.EffectiveTo, '12/31/9999') " & _
        "	join WebFD.vendorContracts_Testing.Users u with (nolock)  on d2u.UserLogin = u.UserLogin and u.InactivatedDate is null " & _
        "	where ch.InactivatedDate is null " & _
        "   and not exists (select * from WebFD.vendorContracts_Testing.Contract_Approvals ca with (nolock)  where ca.ContractID = ch.ContractID and d2u.UserLogin = ca.UserLogin and ca.InactivatedDate is null)"

        Dim x As String =
  "	select * from (  " & _
                      "	select ch.ContractID, ch.RequesterUserLogin, ch.ContractName, ca.Alias as LegalContractAlias " & _
"	, cas.UserAssigned as LegalUserAssigned, casu.UserAssigned as ReferralAssignment, ch.DepartmentID, ch.VendorName, ch.ContractTypeID, ch.ContractLength " & _
"	, ch.ContractExpenseID , cel.LongDescription as AnnualContractExpense, ch.AutoRenewal, ch.RenewalTerm, ch.ContractPurpose, ch.ContractingParty, ch.ContractingPartyDetail " & _
"	, ch.ExpenseAccount, ch.AddedDate as DateAdded, isnull(ca.Alias, ContractName) as LegalName , isnull(csl.StatusShortDescription, 'Pending') as UserStatus " & _
"	, pl.LegalPriorityShortDescription as LegalPriority, cp.PriorityID as LegalPriorityID, lcsl.StatusLongDescription as LegalFullStatus, lcsl.StatusShortDescription as LegalStatus " & _
"	, lcs.StatusID as LegalStatusID, d.DepartmentNo, convert(varchar, DepartmentNo) + ' - ' + DepartmentName as DepartmentFull   " & _
"	, isnull(ru.UserDisplayName, ru.UserFullName) as Requestor, isnull(isnull(lu.UserDisplayName, lu.UserFullName), cas.UserAssigned) as LegalUserName " & _
"	 , isnull(LegalDeadline, dead.AnswerDate) as Deadline, LegalDeadline, dead.AnswerDate as UserRequestedDeadline   " & _
"     , case when LegalDeadline is null and dead.AnswerDate is not null then 'True' else 'False' end as DeadlineColor   " & _
"	 , convert(varchar, DepartmentNo) + ' - ' + DepartmentName as Department, ContractType, cpl.ContractParty " & _
"	 ,  convert(varchar, Acct) + ' - ' + Description as ExpenseDescrip, cc.Comment, pnding.Approvers " & _
"	 , case when pnding.Approvers is null then appdate else null end as DateApproved " & _
"   ,  cslo.StatusShortDescription as OverallStatus  " & _
" " & _
"	From WebFD.vendorContracts_Testing.ContractHeader ch with (nolock)  " & _
"	left join WebFD.vendorContracts_Testing.Contract_Alias ca with (nolock)  on ch.ContractID = ca.ContractID and ca.AliasTypeID = 2 and ca.InactivatedDate is null " & _
"	left join WebFD.vendorContracts_Testing.Contract_Assignment cas with (nolock)  on cas.ContractID = ch.ContractID and cas.TypeID = 2 and cas.InactivatedDate is null " & _
"	left join WebFD.vendorContracts_Testing.Contract_Assignment casu with (nolock)  on casu.ContractID = ch.ContractID and casu.TypeID = 1 and casu.InactivatedDate is null " & _
"	left join WebFD.vendorContracts_Testing.Contract_Expense_LU cel with (nolock)  on ch.ContractExpenseID = cel.ContractExpenseID and cel.InactivatedDate is null " & _
"		and ch.AddedDate between isnull(cel.EffectiveFrom, '1/1/1800') and isnull(cel.EffectiveTo, '12/31/9999') " & _
"	left join WebFD.vendorContracts_Testing.Contract_Status cs with (nolock)  on ch.ContractID = cs.ContractID and cs.InactivatedDate is null and cs.StatusTypeID = 3 " & _
"	left join WebFD.vendorContracts_Testing.ContractStatus_LU csl with (nolock)  on cs.StatusID =csl.StatusID and csl.InactivatedDate is null " & _
"		and getdate() between isnull(csl.effectiveFrom, '1/1/1800') and isnull(csl.EffectiveTo, '12/31/9999') " & _
"   left join (select max(AddedDate) appdate, ContractID " & _
"	from WebFD.vendorContracts_Testing.Contract_Approvals where StatusID = 13 and InactivatedDate is null " & _
"	group by ContractID) mxapp on mxapp.ContractID = ch.ContractID " & _
"	left join WebFD.vendorContracts_Testing.Contract_Status cso with (nolock)  on ch.ContractID = cso.ContractID and cso.InactivatedDate is null and cso.StatusTypeID = 1 " & _
"	left join WebFD.vendorContracts_Testing.ContractStatus_LU cslo with (nolock)  on cso.StatusID =cslo.StatusID and cslo.InactivatedDate is null " & _
"		and getdate() between isnull(cslo.effectiveFrom, '1/1/1800') and isnull(cslo.EffectiveTo, '12/31/9999') " & _
"	left join WebFD.vendorContracts_Testing.Contract_Priority cp with (nolock)  on ch.ContractID = cp.ContractID and cp.InactivatedDate is null " & _
"	left join WebFD.VendorContracts.ContractPriority_LU pl with (nolock)  on cp.PriorityID = pl.LegalPriorityID and pl.Active = 1 " & _
"	left join WebFD.vendorContracts_Testing.Contract_Status lcs with (nolock)  on ch.ContractID = lcs.ContractID and lcs.InactivatedDate is null and lcs.StatusTypeID = 2 " & _
"	left join WebFD.vendorContracts_Testing.ContractStatus_LU lcsl with (nolock)  on lcs.StatusID =lcsl.StatusID and lcsl.InactivatedDate is null " & _
"		and getdate() between isnull(lcsl.effectiveFrom, '1/1/1800') and isnull(lcsl.EffectiveTo, '12/31/9999') " & _
"	left join WebFD.vendorContracts.Department_LU d with (nolock)  on ch.DepartmentID = d.DepartmentID and d.Active = 1 " & _
"	left join (select *, ROW_NUMBER() over (partition by UserLogin order by isnull(InactivatedDate, '12/31/9999') desc, AddedDate desc) rn from WebFD.vendorContracts_Testing.Users with (nolock) ) ru on ch.RequesterUserLogin = ru.UserLogin and ru.rn = 1		 " & _
"	left join (select *, ROW_NUMBER() over (partition by UserLogin order by isnull(InactivatedDate, '12/31/9999') desc, AddedDate desc) rn from WebFD.vendorContracts_Testing.Users with (nolock) ) lu on cas.UserAssigned = lu.UserLogin and lu.rn = 1 " & _
"	left join WebFD.vendorContracts_Testing.Contract_Answers dead with (nolock)  on dead.QuestionID = 17 and dead.InactivatedDate is null and dead.ContractID = ch.ContractID  		 " & _
"	left join WebFD.VendorContracts.ContractType_LU cl with (nolock)  on cl.ContractTypeID = ch.ContractTypeID and cl.Active = 1   " & _
"	 left join WebFD.VendorContracts.ContractParty_LU cpl with (nolock)  on ch.ContractingParty = cpl.ContractPartyID and cpl.Active = 1  " & _
"	 left join DWH.Axiom.Acct a on a.Acct = ch.ExpenseAccount and ISMAP  in ('SUP', 'FEE', 'OTH')    " & _
"	 left join WebFD.vendorContracts_Testing.Contract_Comments cc with (nolock)  on cc.ContractID = ch.ContractID and cc.CommentType = 'Submission' and cc.LegalOnly = 0 and cc.InactivatedDate is null " & _
"	 left join (  select distinct ST2.ContractID, substring( ( Select ' or ' + Approver as [text()]    " & _
"  						from #TempContractPull_" & Session("TempTableName") & " ST1 where ST1.ContractID = ST2.ContractID  " & _
"  						Order by Approver  " & _
"  						for XML PATH ('') ), 5, 1000) [Approvers]  " & _
"  						from #TempContractPull_" & Session("TempTableName") & " ST2  " & _
"                           ) Pnding on Pnding.ContractID = ch.ContractID  " & _
"	where ch.InactivatedDate is null  " & _
  "                		 ) x  where 1  = 1  "
        ...
        If Legal.Text = "1" Then
        Else
            x += " and x.DepartmentID in ( select distinct isnull(DepartmentID, x.DepartmentID) from WebFD.VendorContracts_Testing.Department_2_User t2u with (nolock)  " & _
                "join WebFD.vendorContracts_Testing.Users u with (nolock)  on t2u.UserLogin = u.UserLogin and u.InactivatedDate is null " & _
                "where t2u.InactivatedDate is null and  getdate() between isnull(t2u.effectiveFrom, '1/1/1800') and isnull(t2u.EffectiveTo, '12/31/9999') " & _
                " and u.UserLogin = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "')"
        End If
    ...
    Private Function AdvancedSearch()
        ...
        If Trim(txtLegalSearchKeywords.Text) <> "" Then
            ...
            "	declare @Mini varchar(max) " & _
            "	 " & _
            "	if OBJECT_ID('tempdb..#VendorTempy" & Session("TempTableName") & "') is not null  " & _
            "           begin  " & _
            "               drop table #VendorTempy" & Session("TempTableName") & "  " & _
            "            End  " & _
            "	Select * " & _
            "	, 0 as NameScore, 0 as HeaderScore, 0 as CommentScore, 0 as AttachmentScore, 0 as QuestionScore  " & _
            "	into #VendorTempy" & Session("TempTableName") & " " & _
            "	from ( "
        End If

        x += test.Item2

        If ddlLegalAssignedTo.SelectedValue = "--Unassigned crw--" Then
            x += "and LegalUserAssigned is null "
        ElseIf ddlLegalAssignedTo.SelectedValue <> "-- ALL crw--" Then
            x += "and LegalUserAssigned = '" & Replace(ddlLegalAssignedTo.SelectedValue, "'", "''") & "' "
        End If

        If cbLegalShowPending.Checked Then
        Else
            x += "and (OverallStatus <> 'Dept') "
        End If

        If cbLegalShowQueue.Checked Then
        ElseIf cbLegalShowQueue.Visible = False Then
        Else
            x += "and (Isnull(LegalStatus, 'Open') <> 'Open' or OverallStatus <> 'Legal') "
        ...
        ElseIf cbLegalShowQueueH.Visible = False Then
        Else
            x += "and (Isnull(LegalStatus, 'Open') <> 'Open' or OverallStatus <> 'Legal' or isnull(LegalPriority, 'Medium') <> 'High' ) "
        End If

        If cbLegalShowQueueM.Checked Then
        ElseIf cbLegalShowQueueM.Visible = False Then
        Else
            x += "and (Isnull(LegalStatus, 'Open') <> 'Open' or OverallStatus <> 'Legal' or isnull(LegalPriority, 'Medium') <> 'Medium' ) "
        End If

        If cbLegalShowQueueL.Checked Then
        ElseIf cbLegalShowQueueL.Visible = False Then
        Else
            x += "and (Isnull(LegalStatus, 'Open') <> 'Open' or OverallStatus <> 'Legal' or isnull(LegalPriority, 'Medium') <> 'Low') "
        End If

        If cbLegalShowApproved.Checked Then
        Else
            x += "and (isnull(LegalStatus, 'Deleted') <> 'Approved')"
        End If

        If cbLegalShowRejected.Checked Then
        Else
            x += "and (isnull(UserStatus, '') <> 'Deleted' and isnull(LegalStatus, '') <> 'Deleted')"
        End If

        If cbLegalShowClosed.Checked Then
        Else
            x += "and isnull(LegalStatus, '') <> 'Closed'"
            'x += "and (OverallStatus <> 'Processing' or isnull(LegalStatus, 'Deleted') <> 'Closed')"
            ' Uncertain why the <> Processing clause was here?  Holdover from previous iteration?  Removed 1/18/2023 CRW
        End If

        If cbLegalLoadedinMeditract.Checked Then
            'ElseIf cbLegalLoadedinMeditract.Visible = False Then
        Else
            x += "and (isnull(LegalStatus, 'Deleted') <> 'Loaded')"
        End If

        If cbLegalNoResponse.Checked Then
            'ElseIf cbLegalNoResponse.Visible = False Then
        Else
            x += "and (isnull(LegalStatus, 'Deleted') <> 'No Response')"
        End If

        If cbLegalOnHold.Checked Then
            'ElseIf cbLegalOnHold.Visible = False Then
        Else
            x += "and  (isnull(LegalStatus, 'Deleted') <> 'On Hold') "
        End If

        If cbLegalInProgress.Checked Then
            'ElseIf cbLegalInProgress.Visible = False Then
        Else
            x += "and  (isnull(LegalStatus, 'Deleted') <> 'In Progress') "
        End If

        'If cbLegalNegotiation.Checked Then
        'ElseIf cbLegalNegotiation.Visible = False Then
        'Else
        '    x += "and  (OverallStatus <> 'Processing' or isnull(LegalStatus, 'Deleted') <> 'Negotiation') "
        'End If

        'If cbLegalLegal.Checked Then
        'ElseIf cbLegalLegal.Visible = False Then
        'Else
        '    x += "and  (OverallStatus <> 'Processing' or isnull(LegalStatus, 'Deleted') <> 'Legal Review') "
        'End If

        'If cbLegalPreliminary.Checked Then
        'ElseIf cbLegalPreliminary.Visible = False Then
        'Else
        '    x += "and  (OverallStatus <> 'Processing' or isnull(LegalStatus, 'Deleted') <> 'Preliminary Review') "
        'End If
        ...
        If Trim(txtLegalSearchKeywords.Text) <> "" Then
            ...
            "End  " & _
            " " & _
            "Update #VendorTempy" & Session("TempTableName") & " " & _
            "set NameScore = Namescore + 3  " & _
            "where LegalName like '% ' + @Mini + ' %' " & _
            "	or LegalName like @Mini + ' %' " & _
            "	or LegalName like '% ' + @Mini " & _
            ...
            "update vt  " & _
            "set CommentScore = CommentScore + 3 " & _
            "from #VendorTempy" & Session("TempTableName") & " vt " & _
            "join WebFD.VendorContracts_Testing.Contract_Comments cc on vt.ContractID = cc.ContractID and cc.InactivatedDate is null " & _
            "where cc.Comment like '% ' + @Mini + ' %' " & _
            "	or cc.Comment like @Mini + ' %' " & _
            "	or cc.Comment like '% ' + @Mini " & _
            "	or cc.Comment = @Mini " & _
            ...
            "set AttachmentScore = AttachmentScore + 3 " & _
            "from #VendorTempy" & Session("TempTableName") & " vt " & _
            "join WebFD.VendorContracts_Testing.ContractAttachments cc on vt.ContractID = cc.ContractID and cc.InactivatedDate is null " & _
            "where cc.FileName like '% ' + @Mini + ' %' " & _
            "	or FileName like @Mini + ' %' " & _
            "	or FileName like '% ' + @Mini " & _
            "	or FileName = @Mini " & _
            ...
            "set QuestionScore = QuestionScore + 3 " & _
            "from #VendorTempy" & Session("TempTableName") & " vt " & _
            "join WebFD.VendorContracts_Testing.Contract_Answers cc on vt.ContractID = cc.ContractID and cc.InactivatedDate is null " & _
            "where cc.AnswerComment like '% ' + @Mini + ' %' " & _
            "	or AnswerComment like @Mini + ' %' " & _
            "	or AnswerComment like '% ' + @Mini " & _
            "	or AnswerComment = @Mini " & _
            ...
            "Select NameScore * 10 + CommentScore * 6 + AttachmentScore + HeaderScore * 4 + AttachmentScore * 2 + QuestionScore * 6 as ord " & _
            ", * " & _
            "from #VendorTempy" & Session("TempTableName") & " where NameScore * 10 + CommentScore * 6 + AttachmentScore + HeaderScore * 4 + AttachmentScore * 2 + QuestionScore * 6 > 0" & _
            " " & _
            "order by 1 desc, DateAdded desc "
        Else
            x += " order by DateAdded desc, LegalName"
    ...
    Private Sub gvLegalResults_RowCreated(sender As Object, e As GridViewRowEventArgs) Handles gvLegalResults.RowCreated
        Try
            If e.Row.RowType = DataControlRowType.DataRow Then
                ...
                If Legal.Text = "1" Then
                    If Not IsDBNull(e.Row.DataItem("OverallStatus")) Then
                        If e.Row.DataItem("OverallStatus") = "Dept" Then
                            e.Row.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalPendingApprovalColor)
                        ElseIf e.Row.DataItem("UserStatus") = "Deleted" Then
                            e.Row.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalRejectedColor)
                        ElseIf IsDBNull(e.Row.DataItem("LegalStatus")) Then
                            If Not IsDBNull(e.Row.DataItem("LegalPriority")) Then
                                If e.Row.DataItem("LegalPriority") = "High" Then
                                    e.Row.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalHighPriorityColor)
                                ElseIf e.Row.DataItem("LegalPriority") = "Medium" Then
                        ...
                        ElseIf e.Row.DataItem("LegalStatus") = "Closed" Then
                            e.Row.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalClosedColor)
                        ElseIf e.Row.DataItem("LegalStatus") = "Deleted" Then
                            e.Row.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalRejectedColor)
                        ElseIf e.Row.DataItem("LegalStatus") = "Approved" Then
                            e.Row.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalApprovedColor)
                        ElseIf e.Row.DataItem("LegalStatus") = "Loaded" Then
                            e.Row.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalLoadedColor)
                        ElseIf e.Row.DataItem("LegalStatus") = "No Response" Then
                            e.Row.BackColor = System.Drawing.ColorTranslator.FromHtml(NoResponseColor)
                        ElseIf e.Row.DataItem("LegalStatus") = "On Hold" Then
                            e.Row.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalOnHoldColor)
                        ElseIf e.Row.DataItem("LegalStatus") = "In Progress" Then
                            e.Row.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalInProgressColor)
                            'ElseIf e.Row.DataItem("OverallStatus") = "Legal" Then
                        Else
                            If Not IsDBNull(e.Row.DataItem("LegalPriority")) Then
                                If e.Row.DataItem("LegalPriority") = "High" Then
                                    e.Row.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalHighPriorityColor)
                ...
                Else
                    If Not IsDBNull(e.Row.DataItem("OverallStatus")) Then
                        If e.Row.DataItem("OverallStatus") = "Dept" Then
                            e.Row.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalPendingApprovalColor)
                            'ElseIf e.Row.DataItem("OverallStatus") = "Legal" Then
                            '    e.Row.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalqueueColor)
                        ElseIf e.Row.DataItem("UserStatus") = "Deleted" Then
                            e.Row.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalRejectedColor)
                        ElseIf IsDBNull(e.Row.DataItem("LegalStatus")) Then
                            e.Row.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalqueueColor)
    ...
    Private Sub LegalRowing()
        ...
        For Each canoe As GridViewRow In gvLegalResults.Rows
            ...
            If Legal.Text = "1" Then
                If canoe.Cells(15).Text <> "&nbsp;" Then
                    If canoe.Cells(15).Text = "Dept" Then
                        canoe.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalPendingApprovalColor)
                    ElseIf canoe.Cells(10).Text = "Deleted" Then
                        canoe.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalRejectedColor)
                    ...
                    ElseIf canoe.Cells(11).Text = "Approved" Then
                        canoe.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalApprovedColor)
                    ElseIf canoe.Cells(11).Text = "Loaded" Then
                        canoe.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalLoadedColor)
                    ElseIf canoe.Cells(11).Text = "No Response" Then
                        canoe.BackColor = System.Drawing.ColorTranslator.FromHtml(NoResponseColor)
                    ElseIf canoe.Cells(11).Text = "In Progress" Then
                        canoe.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalInProgressColor)
                    ElseIf canoe.Cells(11).Text = "On Hold" Then
                        canoe.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalOnHoldColor)
                        'ElseIf canoe.Cells(15).Text = "Legal" Then
                    Else
                        If canoe.Cells(12).Text <> "&nbsp;" Then
                            If canoe.Cells(12).Text = "High" Then
                        ...
                        'ElseIf canoe.Cells(11).Text = "Preliminary Review" Then
                        '    canoe.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalPreliminaryColor)
                        'ElseIf canoe.Cells(11).Text = "Legal Review" Then
                        '    canoe.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalLegalColor)
                        'ElseIf canoe.Cells(11).Text = "Negotiation" Then
                        '    canoe.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalNegotiationColor)
            ...
            Else
                If canoe.Cells(15).Text <> "" Then
                    If canoe.Cells(15).Text = "Dept" Then
                        canoe.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalPendingApprovalColor)
                        'ElseIf canoe.Cells(15).Text = "Legal" Then
                        '    canoe.BackColor = System.Drawing.ColorTranslator.FromHtml(LegalqueueColor)
                    Else
    ...
    Private Sub ResetgvLegalApproval()
        ...
        gvLegalApproval.DataSource = GetData("select isnull(isnull(u.UserDisplayName, u.UserFullName), ca.UserLogin) as UserName " & _
                ", StatusShortDescription as Status, Comment, ca.AddedDate as ModifyDate, RoleFull as Role, ca.UserLogin, case when ca.StatusID = 14 then 'ButtonGeneral' else 'hidden' end as BtnCss  " & _
                "from WebFD.VendorContracts_Testing.Contract_Approvals ca " & _
                "left join webfd.VendorContracts_Testing.Role_LU r on ca.RoleID = r.RoleID and r.InactivatedDate is null " & _
                "left join WebFD.vendorContracts_Testing.ContractStatus_LU csl on ca.StatusID = csl.StatusID and csl.InactivatedDate is null " & _
                "   and getdate() between isnull(csl.EffectiveFrom, '1/1/1800') and isnulL(csl.EffectiveTo, '12/31/9999') " & _
                "left join WebFD.vendorContracts_Testing.Users u on ca.UserLogin = u.UserLogin and u.InactivatedDate is null " & _
                "where ca.ContractID = '" & x & "' and ca.InactivatedDate is null and ca.UserLogin <> 'Default Insert'")
    ...
    Private Sub SelectLegalRow()
        Try
            ...
            ddlLegalResultAssignedLegal.DataSource = GetData("select isnull(UserDisplayName, UserFullName) as UserName, UserLogin, 2 as Ord From WebFD.vendorContracts_Testing.Users u " & _
            "where exists (select * from WebFD.VendorContracts_Testing.Department_2_User d2u where d2u.UserLogin = u.UserLogin and d2u.RoleID in (11, 13) and d2u.InactivatedDate is null) " & _
            "and InactivatedDate is null " & _
            "union " & _
            "select isnull(UserDisplayName, UserFullName) + ' (INACTIVE)' as UserName, cas.UserAssigned as LegalUserAssigned, 3  " & _
            " from WebFD.VendorContracts_Testing.ContractHeader ch " & _
            " left join WebFD.VendorContracts_Testing.Contract_Assignment cas on ch.ContractID = cas.ContractID and cas.InactivatedDate is null and cas.TypeID = 2 " & _
            "left join WebFD.vendorContracts_Testing.Users u on cas.UserAssigned = u.UserLogin and u.InactivatedDate is null   " & _
            "where ch.ContractID = '" & x & "' and cas.UserAssigned is not null and not exists (select * from WebFD.VendorContracts_Testing.Department_2_User d2u where d2u.UserLogin = u.UserLogin and d2u.RoleID in (10, 11, 13) and d2u.InactivatedDate is null) " & _
            ...
            ddlLegalResultContractStatus.DataSource = GetData("select StatusID, StatusLongDescription, DefaultInd from WebFD.VendorContracts_Testing.ContractStatus_LU" & _
                " where InactivatedDate is null and StatusTypeID = 2 and getdate() between isnull(EffectiveFrom, '1/1/1800') and isnull(EffectiveTo, '12/31/9999') " & _
                "union select * from ( select top 1 StatusID, StatusLongDescription + ' (Retired)' as StatusLongDescription, 0 as DefaultInd from WebFD.VendorContracts_Testing.ContractStatus_LU a " & _
                " where InactivatedDate is not null and StatusID = " & Replace(z, "'", "''") & _
                " and not exists (select * from WebFD.VendorContracts_Testing.ContractStatus_LU b where a.StatusID = b.StatusID and b.InactivatedDate is null and b.StatusTypeID = 2 " & _
                "   and getdate() between isnull(b.EffectiveFrom, '1/1/1800') and isnull(b.EffectiveTo, '12/31/9999') ) order by EffectiveTo desc, InactivatedDate desc) x " & _
                " order by DefaultInd desc, StatusLongDescription")
            ddlLegalResultContractStatus.DataValueField = "StatusID"
            ddlLegalResultContractStatus.DataTextField = "StatusLongDescription"
            ...
            ddlLegalResultContractPriority.DataSource = GetData("select LegalPriorityID, LegalPriorityShortDescription, DefaultInd from WebFD.VendorContracts.ContractPriority_LU where Active = 1 " & _
                ...
                " order by DefaultInd desc, LegalPriorityShortDescription")
            ...
            If IsDBNull(y(0)("LegalPriorityID")) Then
                ddlLegalResultContractPriority.SelectedIndex = -1
            Else
            ...
            txtMeditractNumber.Text = GetString("Select MeditractNumber from WebFD.VendorContracts_Testing.MeditractNumber where InactivatedDate is null and ContractID = '" & x & "'")
    ...
    Private Sub PopulateDDLLegalAssignedTo()
        ...
        If GetScalar("select count(*) from WebFD.VendorContracts_Testing.Department_2_User where InactivatedDate is null and RoleID = 13 and UserLogin = '" & _
                    Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "'") > 0 Then
            ddlLegalAssignedTo.DataSource = GetData("select isnull(UserDisplayName, UserFullName) as UserName, UserLogin, 2 as Ord From WebFD.vendorContracts_Testing.Users u " & _
            "where UserLogin = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "'" & _
        ...
        Else
           ...
           "where exists (select * from WebFD.VendorContracts_Testing.Department_2_User d2u where d2u.UserLogin = u.UserLogin and d2u.RoleID in (11, 13) and d2u.InactivatedDate is null) " & _
                       "and InactivatedDate is null " & _
           "union " & _
           "select isnull(UserDisplayName, UserFullName) + ' (INACTIVE)' as UserName, UserAssigned, 3  " & _
           " from WebFD.VendorContracts_Testing.ContractHeader ch " & _
           " left join WebFD.VendorContracts_Testing.Contract_Assignment cas on ch.ContractID = cas.ContractID and cas.InactivatedDate is null and cas.TypeID = 2 " & _
           "left join WebFD.vendorContracts_Testing.Users u on cas.UserAssigned = u.UserLogin and u.InactivatedDate is null   " & _
           "where cas.UserAssigned is not null and not exists (select * from WebFD.VendorContracts_Testing.Department_2_User d2u where d2u.UserLogin = u.UserLogin and d2u.RoleID in (10, 11, 13) and d2u.InactivatedDate is null) " & _
           "union Select '(View All)', '-- ALL crw--', 0 " & _
           "union Select '(Unassigned)', '--Unassigned crw--', 1 " & _
           "order by Ord, UserName, UserLogin ")
            ddlLegalAssignedTo.DataValueField = "User"
        End If
    ...
    Private Sub LoadLegalComments(x As Integer)
        Try
            ...
            Dim y As DataTable = GetData("select " & _
                " '<br>' + Comment + '<br>' " & _
                "+ '<i> - ' + isnull(isnull(UserDisplayName, UserFullName), cc.AddedBy) " & _
                "+ '   (' + convert(varchar, cc.AddedDate) + ')</i><br><br>' " & _
                "from WebFD.VendorContracts_Testing.Contract_Comments cc " & _
                "left join WebFD.vendorContracts_Testing.Users u on cc.AddedBy = u.UserLogin and u.InactivatedDate is null " & _
                "where cc.InactivatedDate is null and Relation = 'Legal' and ContractID = " & x & " " & _
                "order by cc.AddedDate ")
    ...
    Private Sub btnLegalUpdateContract_Click(sender As Object, e As EventArgs) Handles btnLegalUpdateContract.Click
        Try
            ...
            Dim NewUpdateSQL As String = ""

            NewUpdateSQL += "/* Deactivate Assign if necessary */ " & _
                " update WebFD.VendorContracts_Testing.Contract_Assignment set InactivatedDate = getdate(), InactivatedBy = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' " & _
                " where ContractID = '" & x & "' and TypeID = 2 and InactivatedDate is null and UserAssigned <> '" & Replace(ddlLegalResultAssignedLegal.SelectedValue, "'", "''") & "' "

            If ddlLegalResultAssignedLegal.SelectedIndex <> 0 Then
                NewUpdateSQL += " /* Insert Assign if necessary */ " & _
                    "  Insert into WebFD.VendorContracts_Testing.Contract_Assignment (ContractID, TypeID, UserAssigned, AddedDate, AddedBy) " & _
                    " Select '" & x & "', 2, '" & Replace(ddlLegalResultAssignedLegal.SelectedValue, "'", "''") & "', getdate(), '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "'  " & _
                    " where not exists (select * from WebFD.VendorContracts_Testing.Contract_Assignment where InactivatedDate is null and TypeID = 2 and ContractID = '" & x & _
                    "' and UserAssigned = '" & Replace(ddlLegalResultAssignedLegal.SelectedValue, "'", "''") & "') "
            End If
            ...
            NewUpdateSQL += "/* Deactivate Assign if necessary */ " & _
            " update WebFD.VendorContracts_Testing.Contract_Alias set InactivatedDate = getdate(), InactivatedBy = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' " & _
            " where ContractID = '" & x & "' and AliasTypeID = 2 and InactivatedDate is null and Alias <> '" & Replace(Trim(txtLegalResultLegalContractName.Text), "'", "''") & "' "

            If Trim(txtLegalResultLegalContractName.Text) <> "" Then
                NewUpdateSQL += " /* Insert Assign if necessary */ " & _
                    "  Insert into WebFD.VendorContracts_Testing.Contract_Alias (ContractID, AliasTypeID, Alias, AddedDate, AddedBy) " & _
                    " Select '" & x & "', 2, '" & Replace(Trim(txtLegalResultLegalContractName.Text), "'", "''") & "', getdate(), '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "'  " & _
                    " where not exists (select * from WebFD.VendorContracts_Testing.Contract_Alias where InactivatedDate is null and AliasTypeID = 2 and ContractID = '" & x & _
                    "' and Alias = '" & Replace(Trim(txtLegalResultLegalContractName.Text), "'", "''") & "') "
            End If
            ...
            NewUpdateSQL += "/* Deactivate Referral Assign if necessary */ " & _
            " update WebFD.VendorContracts_Testing.Contract_Assignment set InactivatedDate = getdate(), InactivatedBy = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' " & _
            " where ContractID = '" & x & "' and TypeID = 1 and InactivatedDate is null and UserAssigned <> '" & Replace(Trim(txtLegalResultReferralAssignment.Text), "'", "''") & "' "

            If Trim(txtLegalResultReferralAssignment.Text) <> "" Then
                NewUpdateSQL += " /* Insert Assign if necessary */ " & _
                    "  Insert into WebFD.VendorContracts_Testing.Contract_Assignment (ContractID, TypeID, UserAssigned, AddedDate, AddedBy) " & _
                    " Select '" & x & "', 1, '" & Replace(Trim(txtLegalResultReferralAssignment.Text), "'", "''") & "', getdate(), '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "'  " & _
                    " where not exists (select * from WebFD.VendorContracts_Testing.Contract_Assignment where InactivatedDate is null and TypeID = 1 and ContractID = '" & x & _
                    "' and UserAssigned = '" & Replace(Trim(txtLegalResultReferralAssignment.Text), "'", "''") & "') "
            End If
            ...
            NewUpdateSQL += "  Update WebFD.VendorContracts_Testing.Contract_Status set InactivatedDate = getdate(), InactivatedBy = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' " & _
                " where ContractID = '" & x & "' and StatusTypeID= 2 and InactivatedDate is null and StatusID <> '" & Replace(ddlLegalResultContractStatus.SelectedValue, "'", "''") & "' " & _
                "Insert into WebFD.VendorContracts_Testing.Contract_Status (ContractID, StatusTypeID, StatusID, AddedDate, AddedBy) " & _
                "Select '" & x & "', 2, '" & Replace(ddlLegalResultContractStatus.SelectedValue, "'", "''") & "', getdate(), '" & _
                Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' where not exists (select * from WebFD.VendorContracts_Testing.Contract_Status " & _
                " where InactivatedDate is null and ContractID = '" & x & "' and StatusTypeID = 2 )"

            Dim dead As Date
            Dim deadstring As String = ""
            If Date.TryParse(txtLegalResultDeadline.Text, dead) = False Then
                dead = "1/1/1800"
                deadstring = "null "
            Else
                deadstring = "'" & dead.ToString & "' "
            End If

            NewUpdateSQL += "  Update WebFD.VendorContracts_Testing.Contract_Priority set InactivatedDate = getdate(), InactivatedBy = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' " & _
            " where ContractID = '" & x & "' and (PriorityID <> '" & Replace(ddlLegalResultContractPriority.SelectedValue, "'", "''") & "' or isnull(LegalDeadline, '1/1/1800') <> '" & dead & "') " & _
            "Insert into WebFD.VendorContracts_Testing.Contract_Priority (ContractID, PriorityID, LegalDeadline, AddedDate, AddedBy) " & _
            "Select '" & x & "', '" & Replace(ddlLegalResultContractPriority.SelectedValue, "'", "''") & "', " & deadstring & ", getdate(), '" & _
            Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' where not exists (select * from WebFD.VendorContracts_Testing.Contract_Priority " & _
            " where InactivatedDate is null and ContractID = '" & x & "' ) "
            ...
            '    Replace(ddlLegalResultContractPriority.SelectedValue, "'", "''") & "', '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "', getdate(), 1 "
            ...
            If Trim(txtLegalResultComments.Text) <> "" Then
                NewUpdateSQL += "Insert into WebFD.VendorContracts_Testing.Contract_Comments (CommentID, ContractID, Relation, Comment, CommentType, LegalOnly, AddedDate, AddedBy) " & _
                    " select isnull(max(CommentID), 0) + 1, '" & x & "', 'Legal', '" & Replace(txtLegalResultComments.Text, "'", "''") & "', 'Legal', 1, getdate(), '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' " & _
                    " from WebFD.VendorContracts_Testing.Contract_Comments "
                ...
                'UpdateSQL += ", getdate(), '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "', 1 )"

            End If

            NewUpdateSQL += "Update WebFD.VendorContracts_Testing.MeditractNumber set InactivatedDate = getdate(), InactivatedBy = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' " & _
                "where InactivatedDate is null and ContractID = '" & x & "' and MeditractNumber <> '" & Replace(Trim(txtMeditractNumber.Text), "'", "''") & "' "

            If Trim(txtMeditractNumber.Text) <> "" Then
                NewUpdateSQL += "Insert into WebFD.VendorContracts_Testing.MeditractNumber (ContractID, MeditractNumber, AddedDate, AddedBy) " & _
                    "select '" & x & "', '" & Replace(Trim(txtMeditractNumber.Text), "'", "''") & "', getdate(), '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' " & _
                    "where not exists (select * from WebFD.VendorContracts_Testing.MeditractNumber where ContractID = '" & x & "' and InactivatedDate is null) "
            End If


            ExecuteSql(NewUpdateSQL)
            'ExecuteSql("exec WebFD.VendorContracts_Testing.Update_Contract_Status_New '" & x & "'")
            ' Removed Trigger 8/8/2022 Unsure why Exists??? CRW

            'ExecuteSql(UpdateSQL)

            'Dim BonusSql As String = "Insert into WebFD.VendorContracts_Testing.EmailStatus select " & x & ", 'PendingLegalRejection', 1, null where not exists (select * from WebFD.VendorContracts_Testing.EmailStatus " & _
    ...
    Private Function DeptView()
        ...
        If Trim(txtAdminDepFilter.Text) <> "" Then
            ...
            "	declare @Mini varchar(max) " & _
            "	 " & _
            "	if OBJECT_ID('tempdb..#DeptUsersTempy" & Session("TempTableName") & "') is not null  " & _
            "           begin  " & _
            "               drop table #DeptUsersTempy" & Session("TempTableName") & "  " & _
            "            End  " & _
            "	Select * " & _
            "	, 0 as NameScore, 0 as UserLoginScore  " & _
            "	into #DeptUsersTempy" & Session("TempTableName") & " " & _
            "	from ( "
        End If
        ...
        SQL += "select * " & _
                    ...
                    "from WebFD.VendorContracts.Department_LU d with (nolock) where not exists (select * from WebFD.VendorContracts.Department_LU  d2 with (nolock) where " & _
        ...
        If Trim(txtAdminDepFilter.Text) <> "" Then
          ...
          "End  " & _
          " " & _
          "Update #DeptUsersTempy" & Session("TempTableName") & " " & _
          "set NameScore = Namescore + 3  " & _
          ...
          ", * " & _
          "from #DeptUsersTempy" & Session("TempTableName") & " where  NameScore * 8 + UserLoginScore * 10  > 0" & _
          " " & _
        ...
        Try
            ...
            If Commander = "RemoveDept" Then
                ...
                Dim PrepSQL As String = "with cte as ( " & _
                    "select top 1 * from WebFD.VendorContracts.Department_LU with (nolock) " & _
                    "where DepartmentID = '" & Replace(Detail_ID, "'", "''") & "' " & _
                    "order by Active desc,  DateModified desc) " & _
                    "update cte set Active = -Active + 1, DateModified = getdate(), ModifiedBy =  '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "'"
    ...
    Private Sub gvAdminDepartments_RowEditing(sender As Object, e As GridViewEditEventArgs) Handles gvAdminDepartments.RowEditing
        Try
            ...
            gvDeptUserAccess.DataSource = GetData("select UserLogin, UserName, RoleID, RoleFull as Position, " & _
                "case when DepartmentID is null then 'Granted All Departments'	else 'Inactivate' end as Clicky  " & _
                "from (select ROW_NUMBER() over (partition by d2u.UserLogin order by case when Roleshort like '%PROXY%' then 2 else 1 end, Hierarchy desc) as RN , " & _
                "d2u.UserLogin, d2u.RoleID, RoleFull, isnull(isnull(u.UserDisplayName, u.UserFullName), d2u.UserLogin) as UserName, d2u.DepartmentID " & _
                ", r.Hierarchy, r.ApprovalRole, r.DisplayRole " & _
                "from WebFD.VendorContracts_Testing.Department_2_User d2u with (nolock) " & _
                "left join WebFD.VendorContracts_Testing.Role_LU r with (nolock) on d2u.RoleID = r.RoleID and r.InactivatedDate is null  " & _
                "join WebFD.vendorContracts_Testing.Users u with (nolock) on d2u.UserLogin = u.UserLogin and u.InactivatedDate is null " & _
                "where d2u.InactivatedDate is null and (DepartmentID = '" & x & "' or DepartmentID is null) and r.RoleID not in (10, 11, 13) " & _
                ") x where RN = 1 order by ApprovalRole desc, DisplayRole desc, Hierarchy")
    ...
    Private Sub gvAdminDepartments_RowUpdating(sender As Object, e As GridViewUpdateEventArgs) Handles gvAdminDepartments.RowUpdating
        Try
            ...
            '    Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & "' where DepartmentID = '" & Replace(depid, "'", "''") & "'"
            ...
            Dim SQL As String = "update d set Active = 0, DateModified = getdate() " & _
                ...
                Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & "' " & _
                "from WebFD.VendorContracts.Department_LU d  " & _
                "where DepartmentID = '" & Replace(depid, "'", "''") & "' and not exists (select * from WebFD.VendorContracts.Department_LU d2 where d.DepartmentID = d2.DepartmentID " & _
    ...
    Private Sub gvDeptUserAccess_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvDeptUserAccess.RowCommand
        ...
        Try
            ...
            If Commander = "RemoveAccess" Then
                ...
                'Dim PrepSQL As String = "with cte as ( " & _
                '    "select top 1 * from WebFD.VendorContracts_Testing.Department_2_User " & _
                ...
                '    "order by isnull(InactivatedDate, '12/31/9999') desc) " & _
                '    "update cte set Active = -Active + 1, DateModified = getdate(), ModifiedBy =  '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "'"

                Dim PrepSQL As String = "update WebFD.VendorContracts_Testing.Department_2_User set InactivatedDate = getdate(), InactivatedBy = '" & _
                    Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "'  where InactivatedDate is null and DepartmentID = '" & _
                    Replace(gvAdminDepartments.DataKeys(gvAdminDepartments.EditIndex).Value, "'", "''") & "' and UserLogin = '" & _
                     Replace(Detail_ID, "'", "''") & "' "
                ...
                gvDeptUserAccess.DataSource = GetData("select UserLogin, UserName, RoleID, RoleFull as Position, " & _
                                ...
                                "left join WebFD.VendorContracts_Testing.Role_LU r with (nolock) on d2u.RoleID = r.RoleID and r.InactivatedDate is null  " & _
                                "join WebFD.vendorContracts_Testing.Users u with (nolock) on d2u.UserLogin = u.UserLogin and u.InactivatedDate is null " & _
                                "where d2u.InactivatedDate is null and (DepartmentID = '" & Replace(gvAdminDepartments.DataKeys(gvAdminDepartments.EditIndex).Value, "'", "''") & _
                                "' or DepartmentID is null) and r.RoleID not in (10, 11, 13) " & _
                                ") x where RN = 1 order by ApprovalRole desc, DisplayRole desc, Hierarchy")
                gvDeptUserAccess.DataBind()
    ...
    Private Function UserView()
        ...
        If Trim(txtFilterUsers.Text) <> "" Then
            ...
            "	declare @Mini varchar(max) " & _
            "	 " & _
            "	if OBJECT_ID('tempdb..#VendorUsersTempy" & Session("TempTableName") & "') is not null  " & _
            "           begin  " & _
            "               drop table #VendorUsersTempy" & Session("TempTableName") & "  " & _
            "            End  " & _
            "	Select * " & _
            "	, 0 as NameScore, 0 as UserLoginScore  " & _
            "	into #VendorUsersTempy" & Session("TempTableName") & " " & _
            "	from ( "
        End If

        SQL += "select u2.*, case when Admin = 0 then 'Grant Admin Rights' else 'Revoke Admin Rights' end as AdminClicky " & _
                ", case when  d2u.UserLogin is null then 'Grant Legal Rights' else 'Revoke Legal Rights' end as Legal  " & _
                ", case when isnull(WebToolAccess, 0) = 0 then 'Add User' else 'Remove User' end as UserClicky  " & _
                "from WebFD.vendorContracts_Testing.Users u2 with (nolock) " & _
                "left join WebFD.vendorContracts_Testing.Department_2_User d2u with (nolock) on u2.UserLogin = d2u.UserLogin and d2u.InactivatedDate is null and d2u.RoleID in (10, 11, 13) " & _
                "where 1 = 1 and not exists (select * from WebFD.vendorContracts_Testing.Users u with (nolock) 	" & _
                "where u.UserLogin = u2.UserLogin and  (isnull(u.InactivatedDate, '12/31/9999') > isnull(u2.InactivatedDate, '12/31/9999') " & _
                " or (isnull(u.InactivatedDate, '12/31/9999')  = isnull(u2.InactivatedDate, '12/31/9999') and u.AddedDate > u2.AddedDate))) "

        If cbAdminUserActives.Checked = False Then
            SQL += " and u2.InactivatedDate is null "
        End If
        ...
        If Trim(txtFilterUsers.Text) <> "" Then
          ...
          "End  " & _
          " " & _
          "Update #VendorUsersTempy" & Session("TempTableName") & " " & _
          "set NameScore = Namescore + 3  " & _
          ...
          ", * " & _
          "from #VendorUsersTempy" & Session("TempTableName") & " where  NameScore * 10 + UserLoginScore * 6  > 0" & _
          " " & _
          "order by 1 desc, AddedDate desc "
        Else
    ...
    Private Sub btnAdminAddUser_Click(sender As Object, e As EventArgs) Handles btnAdminAddUser.Click
        If Trim(txtAdminADUserLogin.Text) = "" Then
            ...
            Exit Sub
        End If
        If GetScalar("Select count(*) from WebFD.vendorContracts_Testing.Users with (nolock) where UserLogin = '" & Trim(Replace(txtAdminADUserLogin.Text, "'", "''")) & "' and InactivatedDate is null ") > 0 Then
        ...
        SQL = "Insert into WebFD.vendorContracts_Testing.Users (UserLogin, UserFullName, UserDisplayName, WebToolAccess, AddedDate, AddedBy, Admin, UserEmailAddress) values ('" & _
                Replace(txtAdminADUserLogin.Text, "'", "''") & "', '" & _
                Trim(Replace(lblAdminADUserName.Text, "'", "''")) & "', " & x & ", 1, getdate(), '" & _
                Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "', 0, null)"
    ...
    Private Sub SynchWebFDPermissions()
        ...
        "join WebFD.dbo.aspnet_Users au on  au.UserName = dar.UserLogin " & _
        "where uir.UserId = au.UserId and dar.InactivatedDate is null and WebToolAccess= 1)"
    ...
    Private Sub gvAdminUsers_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvAdminUsers.RowCommand
        Try
            ...
            If Developer.Text = 0 And UserLogin = Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") Then
            ...
            If varname = "FlipAdmin" Then
                'Dim Sql As String = "update WebFD.vendorContracts_Testing.Users set Admin = 1 - isnull(Admin, 0), ModifiedBy = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & _
                '    "', DateModified = getdate() where UserLogin = '" & Replace(UserLogin, "'", "''") & "'"

                Dim Sql As String = "Insert into WebFD.vendorContracts_Testing.Users (UserLogin, UserFullName, UserDisplayName, UserEmailAddress, WebToolAccess, Admin, AddedDate, AddedBy ) " & _
        "select UserLogin, UserFullName, UserDisplayName, UserEmailAddress, WebToolAccess, -Admin + 1, getdate(), '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & "' " & _
        "from  WebFD.vendorContracts_Testing.Users where InactivatedDate is null and UserLogin = '" & Replace(UserLogin, "'", "''") & "' " & _
        "Update u set InactivatedDate = getdate(), InactivatedBy = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & "'" & _
        "from WebFD.vendorContracts_Testing.Users u where InactivatedDate is null and exists " & _
        "(select * from  WebFD.vendorContracts_Testing.Users u2 where u2.InactivatedDate is null and u.UserLogin = u2.UserLogin and u2.AddedDate > u.AddedDate) "

                ExecuteSql(Sql)
                PopulateAdminUsers()

            ElseIf varname = "FlipLegal" Then
                'Dim Sql As String = "update WebFD.vendorContracts_Testing.Users set LegalTeam = 1 - isnull(LegalTeam, 0), ModifiedBy = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & _
                '    "', DateModified = getdate() where UserLogin = '" & Replace(UserLogin, "'", "''") & "'"
                'ExecuteSql(Sql)

                Dim Sql As String = "Insert into WebFD.vendorContracts_Testing.Department_2_User (UserLogin, RoleID, AddedDate, AddedBy ) " & _
      "select '" & Replace(UserLogin, "'", "''") & "', 11, getdate(), '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & "' " & _
      "where not exists (select * from WebFD.vendorContracts_Testing.Department_2_User where RoleID = 11 and InactivatedDate is null and UserLogin = '" & Replace(UserLogin, "'", "''") & "') " & _
      "Update u set InactivatedDate = getdate(), InactivatedBy = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & "'" & _
      "from WebFD.vendorContracts_Testing.Users u where InactivatedDate is null and RoleID = 11 and UserLOgin = '" & Replace(UserLogin, "'", "''") & "' and getdate() - AddedDate > '1900-01-01 00:00:03.000'"

                ExecuteSql(Sql)
                PopulateAdminUsers()
                'explanationlabelAdmin.Text = "Contact Website Admin"
                'mpeAdminPage.Show()
            ElseIf varname = "FlipActive" Then
                'Dim Sql As String = "update u2 set Active = 1 - isnull(Active, 0), ModifiedBy = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & _
                '    "', DateModified = getdate() from WebFD.vendorContracts_Testing.Users u2 where UserLogin = '" & Replace(UserLogin, "'", "''") & "' and not exists (select * from WebFD.vendorContracts_Testing.Users u " & _
                '    "	where u.UserLogin = u2.UserLogin and (u.Active > u2.Active or (u.Active = u2.Active and u.DateModified > u2.DateModified)))"
                'ExecuteSql(Sql)

                Dim Sql As String = "Insert into WebFD.vendorContracts_Testing.Users (UserLogin, UserFullName, UserDisplayName, UserEmailAddress, WebToolAccess, Admin, AddedDate, AddedBy ) " & _
                    "select top 1 UserLogin, UserFullName, UserDisplayName, UserEmailAddress, -WebToolAccess + 1, Admin, getdate(), '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & "' " & _
                    "from  WebFD.vendorContracts_Testing.Users where UserLogin = '" & Replace(UserLogin, "'", "''") & "' order by isnull(InactivatedDate, '12/31/9999') desc " & _
                    "Update u set InactivatedDate = getdate(), InactivatedBy = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & "'" & _
                    "from WebFD.vendorContracts_Testing.Users u where InactivatedDate is null and UserLogin = '" & Replace(UserLogin, "'", "''") & "' and exists " & _
                    "(select * from  WebFD.vendorContracts_Testing.Users u2 where u2.InactivatedDate is null and u.UserLogin = u2.UserLogin and u2.AddedDate > u.AddedDate) "
    ...
    Private Sub gvAdminUsers_RowUpdating(sender As Object, e As GridViewUpdateEventArgs) Handles gvAdminUsers.RowUpdating
        Try
            ...
            '    "set UserDisplayName = " & x & " UserEmailAddress = " & y & " DateModified = getdate(), ModifiedBy = '" & _
            '    Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & "' from WebFD.vendorContracts_Testing.Users u2 where UserLogin = '" & Replace(depid, "'", "''") & _
            '    "' and not exists (select * from WebFD.vendorContracts_Testing.Users u " & _
            '        "	where u.UserLogin = u2.UserLogin and (u.Active > u2.Active or (u.Active = u2.Active and u.DateModified > u2.DateModified)))"

            Dim Sql As String = "Insert into WebFD.vendorContracts_Testing.Users (UserLogin, UserFullName, UserDisplayName, UserEmailAddress, WebToolAccess, Admin, AddedDate, AddedBy ) " & _
          "select UserLogin, UserFullName, " & x & " " & y & " WebToolAccess , Admin, getdate(), '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & "' " & _
          "from  WebFD.vendorContracts_Testing.Users where InactivatedDate is null and UserLogin = '" & Replace(depid, "'", "''") & "' " & _
          "Update u set InactivatedDate = getdate(), InactivatedBy = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & "'" & _
          "from WebFD.vendorContracts_Testing.Users u where InactivatedDate is null and UserLogin = '" & Replace(depid, "'", "''") & "' and exists " & _
          "(select * from  WebFD.vendorContracts_Testing.Users u2 where u2.InactivatedDate is null and u.UserLogin = u2.UserLogin and u2.AddedDate > u.AddedDate) "
    ...
    '            Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' from WebFD.VendorContracts.Roles where Active = 1 and RoleFull = '" & Replace(ddlGrantPosition.SelectedValue, "'", "''") & "' " & _
    ...
    '            Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' from WebFD.VendorContracts.Roles r " & _
    ...
    Private Sub AttachmentGrid(q As GridView, x As Integer)
        q.DataSource = GetData("select * from WebFD.VendorContracts_Testing.ContractAttachments " & _
                "where ContractID = '" & x & "' and InactivatedDate is null ")
        q.DataBind()
    End Sub
    ...
    Private Sub BindSubmissionAttachmentGrid()
        ...
        Using conn As New System.Data.SqlClient.SqlConnection(ConfigurationManager.ConnectionStrings("PRDconn").ConnectionString)
            ...
            Dim Str As String = "select AttachmentID, ContractID, FileName, ContentType, Content, AddedBy, 'Download' as downloadtext, 'Remove' as removetext from WebFD.VendorContracts_Testing.PendingContractAttachments " & _
                "where ContractID = '" & AttachIndex.Text & "' and Content is not null and InactivatedDate is null and AddedBy = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & "' union " & _
            "select null, null, 'None', null, null, null, null, null where not exists (select * from WebFD.VendorContracts_Testing.PendingContractAttachments " & _
                "where ContractID = '" & AttachIndex.Text & "' and content is not null and InactivatedDate is null and AddedBy = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & "')"
    ...
    Protected Sub UploadSubmissionAttachmentFile(sender As Object, e As EventArgs)
        ...
        Try
            ...
            Try
                filename = Path.GetFileName(fileSubmissionAttachmentInput.FileName)
            Catch
                Exit Sub
            End Try
            ...
            ElseIf filename.EndsWith(".wav", StringComparison.CurrentCultureIgnoreCase) Then
                explantionlabel.Text = "Please do not submit wav files"
                ModalPopupExtender1.Show()
                Exit Sub
            ElseIf filename.EndsWith(".dll", StringComparison.CurrentCultureIgnoreCase) Then
                explantionlabel.Text = "Please do not submit dll files"
                ModalPopupExtender1.Show()
                Exit Sub
            End If
            ...
            If x > 52600000 Then
                explantionlabel.Text = "Your file is too large to submit; Please either compress your attachment, or submit it directly to Legal"
                ModalPopupExtender1.Show()
                Exit Sub
            ElseIf x = 0 Then
                explantionlabel.Text = "The submitted file contains no content; please submit a valid file"
                ModalPopupExtender1.Show()
                Exit Sub
            End If
            ...
            Using fs As Stream = fileSubmissionAttachmentInput.PostedFile.InputStream
                ...
                Using br As New BinaryReader(fs)
                    ...
                    Using conn As New System.Data.SqlClient.SqlConnection(ConfigurationManager.ConnectionStrings("PRDconn").ConnectionString)
                        ...
                        query = "INSERT INTO WebFD.VendorContracts_Testing.PendingContractAttachments ( ContractID, FileName, ContentType, Content, AddedDate, AddedBy) " & _
                            "values ('" & AttachIndex.Text & "', @FileName, @ContentType, @Content, getdate(), '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", "") & "') "
        ...
        Catch ex As Exception
            explantionlabel.Text = "Error uploading file"
            ModalPopupExtender1.Show()
            LogWebFinError(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), Request.ServerVariables("HTTP_X_FORWARDED_FOR"), Request.ServerVariables("REMOTE_ADDR"), System.Reflection.MethodBase.GetCurrentMethod().ReflectedType.Name, System.Reflection.MethodBase.GetCurrentMethod.Name(), ex.ToString())
        End Try
    End Sub
    ...
    Protected Sub DownloadSubmissionAttachmentFile(sender As Object, e As EventArgs)
        Try
            ...
            Using conn As New System.Data.SqlClient.SqlConnection(ConfigurationManager.ConnectionStrings("PRDconn").ConnectionString)
                ...
                cmd.CommandText = "select FileName, Content, ContentType from WebFD.VendorContracts_Testing.PendingContractAttachments where AttachmentID=@Id"
                ...
                Using sdr As SqlDataReader = cmd.ExecuteReader()
                    ...
                    fileName = Replace(sdr("FileName").ToString(), ",", "_")
        ...
        Catch ex As Exception
            explantionlabel.Text = "Error downloading file"
            ModalPopupExtender1.Show()
            LogWebFinError(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), Request.ServerVariables("HTTP_X_FORWARDED_FOR"), Request.ServerVariables("REMOTE_ADDR"), System.Reflection.MethodBase.GetCurrentMethod().ReflectedType.Name, System.Reflection.MethodBase.GetCurrentMethod.Name(), ex.ToString())
        End Try
    ...
    Protected Sub DownloadApprovalAttachmentFile(sender As Object, e As EventArgs)
        ...
        Using conn As New System.Data.SqlClient.SqlConnection(ConfigurationManager.ConnectionStrings("PRDconn").ConnectionString)
            ...
            cmd.CommandText = "select FileName, Content, ContentType from WebFD.VendorContracts_Testing.ContractAttachments where AttachmentID=@Id"
    ...
    Protected Sub RemoveSubmissionAttachmentFile(sender As Object, e As EventArgs)
        Try
            ...
            ExecuteSql("Update WebFD.VendorContracts_Testing.PendingContractAttachments set InactivatedDate = getdate(), InactivatedBy = '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & _
                       "' where AttachmentID = '" & Replace(FileID, "'", "''") & "'")
            BindSubmissionAttachmentGrid()
    ...
    Private Sub gvLegalAttachments_PageIndexChanging(sender As Object, e As GridViewPageEventArgs) Handles gvLegalAttachments.PageIndexChanging
        Try

            Dim n As String = Replace(gvLegalResults.DataKeys(gvLegalResults.SelectedIndex).Value.ToString(), "'", "''")

            gvLegalAttachments.PageIndex = e.NewPageIndex
            gvLegalAttachments.DataSource = GetData("select * from WebFD.VendorContracts_Testing.ContractAttachments " & _
        "where ContractID = '" & n & "' and InactivatedDate is null ")
            gvLegalAttachments.DataBind()
    ...
    '        "where Active = 1 and LegalTeam = 1 and UserLogin =  '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "'") > 0 Then
    '        Try
    '            ddlLegalAssignedTo.SelectedValue = Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''")
    '        Catch ex As Exception
    '        End Try
    ...
    Private Sub btnConfirmApprovalRejection_Click(sender As Object, e As EventArgs) Handles btnConfirmApprovalRejection.Click
        ...
        If hiddenRejectorApprove.Text = "Approve" Then
            ...
            '              Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "', 'Approved', '" & Replace(Trim(txtApproveComments.Text), "'", "''") & "', getdate(), '" & _
            '              Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "', 1, r.RoleFull from WebFD.VendorContracts.ContractHeader ch " & _
            ...
            '              Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' ")
            '
            ' rework 8/2/2019 CRW
            ExecuteSql("Insert into WebFD.VendorContracts_Testing.Contract_Approvals (ContractID, RoleID, UserLogin, StatusTypeID, StatusID, Comment, AddedDate, AddedBy) " & _
           " Select ch.ContractID, d2u.RoleID, '" & _
              Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "', 3, 13, '" & Replace(Trim(txtApproveComments.Text), "'", "''") & "', getdate(), '" & _
              Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' " & _
              " from WebFD.VendorContracts_Testing.ContractHeader ch " & _
              "join WebFD.VendorContracts_Testing.Department_2_User d2u on ch.DepartmentID = isnull(d2u.DepartmentID, ch.DepartmentID) " & _
              "     and getdate() between isnull(d2u.EffectiveFrom, '1/1/1800') and isnull(d2u.EffectiveTo, '12/31/9999') and d2u.InactivatedDate is null " & _
              "join WebFD.VendorContracts_Testing.Role_LU r on d2u.RoleID = r.RoleID and r.InactivatedDate is null " & _
              "where ch.InactivatedDate is null " & _
              "and ch.ContractID = '" & Replace(lblApproveContractID.Text, "'", "''") & "' and UserLogin = '" & _
              Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' ")

            Dim BonusSQL1 As String = "exec WebFD.vendorContracts_Testing.Update_Contract_Status_New '" & Replace(lblApproveContractID.Text, "'", "''") & "'"
            ExecuteSql(BonusSQL1)
        ...
        Else
            'ExecuteSql("Insert into WebFD.VendorContracts.Contract_Approvals Select ch.ContractID, '" & _
            '   Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "', 'Deleted', '" & Replace(Trim(txtApproveComments.Text), "'", "''") & "', getdate(), '" & _
            '   Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "', 1, r.RoleFull from WebFD.VendorContracts.ContractHeader ch " & _
            '   "join WebFD.VendorContracts.Department_2_User d2u on ch.DepartmentID = isnull(d2u.DepartmentID, ch.DepartmentID) " & _
            '   "join WebFD.VendorContracts.Roles r on d2u.Position = r.RoleShort and r.Active = 1 " & _
            '   "where d2u.Active = 1 " & _
            ...
            ExecuteSql("Insert into WebFD.VendorContracts_Testing.Contract_Approvals (ContractID, RoleID, UserLogin, StatusTypeID, StatusID, Comment, AddedDate, AddedBy) " & _
           " Select ch.ContractID, d2u.RoleID, '" & _
              Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "', 3, 14, '" & Replace(Trim(txtApproveComments.Text), "'", "''") & "', getdate(), '" & _
              Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' " & _
              " from WebFD.VendorContracts_Testing.ContractHeader ch " & _
              "join WebFD.VendorContracts_Testing.Department_2_User d2u on ch.DepartmentID = isnull(d2u.DepartmentID, ch.DepartmentID) " & _
              "     and getdate() between isnull(d2u.EffectiveFrom, '1/1/1800') and isnull(d2u.EffectiveTo, '12/31/9999') and d2u.InactivatedDate is null " & _
            ...
            Dim BonusSql As String = "Insert into WebFD.VendorContracts_Testing.EmailStatus select " & Replace(lblApproveContractID.Text, "'", "''") & ", 'PendingDeptRejection', 1, null where not exists ( " & _
                    "select * from  WebFD.VendorContracts_Testing.EmailStatus where Active = 1 and ContractID = '" & Replace(lblApproveContractID.Text, "'", "''") & "' and LatestEmailStatus = 'PendingDeptRejection') "
            ExecuteSql(BonusSql)
    ...
    Private Sub btnResetSearch_Click(sender As Object, e As EventArgs) Handles btnResetSearch.Click
        ...
        If Legal.Text = "1" Then
            ...
            cbLegalLoadedinMeditract.Checked = False
            cbLegalNoResponse.Checked = False
            cbLegalOnHold.Checked = False
            cbLegalInProgress.Checked = True
            'cbLegalLegal.Checked = True
            'cbLegalNegotiation.Checked = True
            'cbLegalPreliminary.Checked = True
        Else
    ...
    Private Sub gvRecentlyReceivedContracts_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gvRecentlyReceivedContracts.SelectedIndexChanged
        ...
        gvApprovalCurrent.DataSource = GetData("select distinct isnull(isnull(u.UserDisplayName, u.UserFullName), ca.UserLogin) as UserName " & _
                ", sl.StatusLongDescription as Status, Comment, ca.AddedDate as ModifyDate " & _
                "from WebFD.VendorContracts_Testing.Contract_Approvals ca " & _
                "left join WebFD.vendorContracts_Testing.Users u on ca.UserLogin = u.UserLogin and u.InactivatedDate is null " & _
                "left join WebFD.vendorContracts_Testing.ContractStatus_LU sl on ca.StatusID = sl.StatusID and sl.InactivatedDate is null " & _
                "   and getdate() between isnull(sl.EffectiveFrom, '1/1/1800') and isnull(sl.EffectiveTo, '12/31/9999') " & _
                "where ca.ContractID = '" & x & "' and ca.InactivatedDate is null and ca.UserLogin <> 'Default Insert'  order by ModifyDate")
    ...
    Private Sub gvLegalApproval_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvLegalApproval.RowCommand
        Try
            Dim CommandArg As String = e.CommandArgument
            Dim Commander As String = e.CommandName
            Dim PrepSQL As String

            If Commander = "ResetRejection" Then


                PrepSQL = " update WebFD.vendorContracts_Testing.Contract_Approvals set InactivatedBy = '" & Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "'", "''") &
                    "', InactivatedDate = getdate() where Userlogin = '" & Replace(CommandArg, "'", "''") & "' and InactivatedDate is null and ContractID = '" & gvLegalResults.DataKeys(gvLegalResults.SelectedIndex).Value.ToString & "' "

                PrepSQL += " update WebFD.vendorContracts_Testing.Contract_Status  " & _
                    " set InactivatedDate = getdate(), InactivatedBy = 'Removed Rejection' " & _
                    " where ContractID = '" & gvLegalResults.DataKeys(gvLegalResults.SelectedIndex).Value.ToString & "'  and StatusTypeID = 3 and InactivatedDate is null " & _
                    " " & _
                    " update WebFD.vendorContracts_Testing.Contract_Status  " & _
                    " set InactivatedDate = getdate(), InactivatedBy = 'Removed Rejection' " & _
                    " where ContractID = '" & gvLegalResults.DataKeys(gvLegalResults.SelectedIndex).Value.ToString & "'  and StatusTypeID = 1 and InactivatedDate is null and StatusID = 17 "

                ExecuteSql(PrepSQL)

                Dim BonusSQL1 As String = "exec WebFD.vendorContracts_Testing.Update_Contract_Status_New '" & gvLegalResults.DataKeys(gvLegalResults.SelectedIndex).Value.ToString & "'"
                ExecuteSql(BonusSQL1)
    ...
    Private Sub btn_LegalAddComment_Click(sender As Object, e As EventArgs) Handles btn_LegalAddComment.Click

        Dim x As String = Replace(lblLegalContractID.Text, "'", "''")

        If Trim(txtLegalResultComments.Text) <> "" Then
            Dim UpdateSQL As String = "Insert into WebFD.VendorContracts_Testing.Contract_Comments (CommentID, ContractID, Relation, Comment, CommentType, LegalOnly, AddedDate, AddedBy) " & _
                " select isnull(max(CommentID), 0) + 1, '" & x & "', 'Legal', '" & Replace(txtLegalResultComments.Text, "'", "''") & "', 'Legal', 1, getdate(), '" & Replace(Replace(Replace(Request.ServerVariables("AUTH_USER"), "NS\", ""), "NS\", ""), "'", "''") & "' " & _
                " from WebFD.VendorContracts_Testing.Contract_Comments "
    ...
    Private Sub cb_SelectAll_CheckedChanged(sender As Object, e As EventArgs) Handles cb_SelectAll.CheckedChanged
        ...
        If cb_SelectAll.Checked Then
            ...
            cbLegalShowQueue.Checked = True
            cbLegalInProgress.Checked = True
            cbLegalOnHold.Checked = True
            cbLegalNoResponse.Checked = True
            cbLegalShowRejected.Checked = True
          
            cbLegalShowApproved.Checked = True
            cbLegalShowClosed.Checked = True
            cbLegalLoadedinMeditract.Checked = True
        Else
            If Legal.Text = "1" Then
                cbLegalShowQueueH.Checked = False
                cbLegalShowQueueM.Checked = False
                cbLegalShowQueueL.Checked = False
            Else
                cbLegalShowQueue.Checked = False
            End If
            cbLegalInProgress.Checked = False
            cbLegalShowPending.Checked = False
            cbLegalShowQueue.Checked = False
