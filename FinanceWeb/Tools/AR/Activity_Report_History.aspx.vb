...
Public Class Activity_Report_History
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            Else
                ...
                Dim adminsql As String = "SELECT count(*) FROM DWH.AR.ActivityReport_Users with (nolock) where UserLogin = '" & _
            ...
            If Request.QueryString("Detail_ID") IsNot Nothing Then
                ...
                '    "from DWH.AR.ActivityReport ar join DWH.AR.ActivityReport_Detail ad on ar.ActivityID = ad.ActivityID " & _
...
"	select * " & _
" , case when FinalActivityDate is not null then 'Closed' else (select top 1 ar.DetailStatus from DWH.AR.ActivityReport_Detail ar with (nolock) " & _
...
"	join (select NetID as ActivityID from DWH.AR.ActivityReport_Netting with (nolock)  " & _
"	where InitialID = @SearchID and Active = 1  " & _
"	union   " & _
"	select InitialID from DWH.AR.ActivityReport_Netting with (nolock)  " & _
"	where NetID = @SearchID  and Active = 1  " & _
"	union  " & _
"	select TransferFrom from  (select * from DWH.AR.ActivityReport_Transfers with (nolock) union all select * from DWH.AR.ActivityReport_TransfersHistorical with (nolock)) x  " & _
"	where isnull(IntermediateRow, 0) = @SearchID or TransferTo = @SearchID and Active = 1  " & _
"	union   " & _
"	select TransferTo from  (select * from DWH.AR.ActivityReport_Transfers with (nolock) union all select * from DWH.AR.ActivityReport_TransfersHistorical with (nolock)) x  " & _
...
"left join DWH.AR.ActivityReport_2_Source a2s with (nolock) on ar.ActivityID = a2s.ActivityID and a2s.Active = 1 " & _
"left join DWh.AR.ActivityReport_Source s with (nolock) on a2s.Base_ID = s.Base_ID and s.Active = 1 " & _
"left join DWH.AR.ActivityReport_Users u with (nolock) on ar.CreatedBy = u.UserLogin and u.Active = 1 " & _
"where ar.Active = 1  ) N "
    ...
    Private Sub BindFirstGrid(detailID As Integer)
        ...
        Dim s As String = "declare @SearchID int =  " & detailID & _
        " " & _
"If OBJECT_ID('tempdb.dbo.#TmpBases', 'U') IS NOT NULL   " & _
"              DROP TABLE #TmpBases;  " & _
 " " & _
...
"	from DWH.AR.ActivityReport ar with (nolock) " & _
"	left join DWH.AR.ActivityReport_2_Source two with (nolock) on ar.ActivityID = two.ActivityID and two.Active = 1  " & _
"	join (select NetID as ActivityID from DWH.AR.ActivityReport_Netting with (nolock)  " & _
"	where InitialID = @SearchID and Active = 1  " & _
"	union   " & _
"	select InitialID from DWH.AR.ActivityReport_Netting  with (nolock) " & _
...
"	where isnull(IntermediateRow, 0) = @SearchID or TransferFrom = @SearchID and Active = 1  " & _
"	union select @SearchID) find on ar.ActivityID = find.ActivityID  " & _
"	left join DWH.AR.ActivityReport_Source s with (nolock) on two.Base_ID = s.Base_ID and s.Active = 1  " & _
"	) N  " & _
     " " & _
...
"            From (select art.*, t.*   " & _
"	from #TmpBases t with (nolock) " & _
"	left join DWH.AR.ActivityReport_Source s with (nolock) on t.Base_ID = s.Base_ID and s.Active = 1 " & _
"	left join DWH.AR.ActivityReport_2_Source ars with (nolock) on ars.Base_ID = s.Base_ID and ars.Active = 1  " & _
"	join DWH.AR.ActivityReport ar  with (nolock) on ar.Active = 1 and (ar.ActivityID = ars.Active or -ar.ActivityID = t.Base_ID) " & _
    " " & _
"	join (select * from DWH.AR.ActivityReport_Transfers with (nolock)  " & _
"		union all select * from DWH.AR.ActivityReport_TransfersHistorical with (nolock)) art on art.Active = 1 " & _
"			and  ar.ActivityID  in (art.IntermediateRow, art.TransferFrom, art.TransferTo)  " & _
"	) ST1  " & _
...
"			and  ar.ActivityID  in (art.IntermediateRow, art.TransferFrom, art.TransferTo)  " & _
"	) ST2  " & _
"	left join (select *, ROW_NUMBER() over (partition by UserLogin order by Active desc, DateModified desc) rn From DWH.ar.ActivityReport_Users with (nolock)) u on u.rn = 1 and ST2.ModifyUser = u.UserLogin  " & _
     " " & _
"	union all  " & _
" " & _
...
"	left join DWH.AR.ActivityReport_2_Source ars with (nolock) on ars.Base_ID = s.Base_ID and ars.Active = 1  " & _
"	join DWH.AR.ActivityReport ar  with (nolock) on ar.Active = 1 and (ar.ActivityID = ars.Active or -ar.ActivityID = t.Base_ID) " & _
"	join DWH.AR.ActivityReport_Netting art  with (nolock) on art.Active = 1 and ar.ActivityID  in (art.InitialID, art.NetID) " & _
"		) ST1  " & _
...
"	join DWH.AR.ActivityReport_Netting art  with (nolock) on art.Active = 1 and ar.ActivityID  in (art.InitialID, art.NetID) " & _
"	) ST2  " & _
"	left join (select *, ROW_NUMBER() over (partition by UserLogin order by Active desc, DateModified desc) rn " & _
"		 From DWH.ar.ActivityReport_Users with (nolock)) u on u.rn = 1 and ST2.ModifyUser = u.UserLogin  " & _
     " " & _
"	union all  " & _
     " " & _
...
"	left join DWH.AR.ActivityReport_2_Source ars with (nolock) on ars.Base_ID = s.Base_ID and ars.Active = 1  " & _
"	join DWH.AR.ActivityReport ar  with (nolock) on ar.Active = 1 and (ar.ActivityID = ars.Active or -ar.ActivityID = t.Base_ID) " & _
"	join DWH.AR.ActivityReport_Splitting art on art.Active = 1 and ar.ActivityID  = art.InitialID " & _
"	) ST2  " & _
"	left join (select *, ROW_NUMBER() over (partition by UserLogin order by Active desc, DateModified desc) rn  " & _
"			From DWH.ar.ActivityReport_Users with (nolock)) u on u.rn = 1 and ST2.ModifyUser = u.UserLogin  " & _
"	left join DWH.AR.ActivityReport_Detail ad with (nolock) on ST2.InitialID = ad.ActivityID  " & _
"		and convert(date, ad.ActivityDate) <= ST2.ActivityDate  " & _
"		and not exists (select * from DWH.AR.ActivityReport_Detail adfake with (nolock)  " & _
"			where adfake.ActivityID = ad.ActivityID and adfake.ActivityDate > ad.ActivityDate  " & _
...
"	left join DWH.AR.ActivityReport_2_Source ars with (nolock) on ars.Base_ID = s.Base_ID and ars.Active = 1  " & _
"	join DWH.AR.ActivityReport ar  with (nolock) on ar.Active = 1 and (ar.ActivityID = ars.Active or -ar.ActivityID = t.Base_ID) " & _
"	join DWH.AR.ActivityReport_Detail ard with (nolock) on ar.ActivityID = ard.ActivityID and ard.Active = 1 " & _
"	left join (select *, ROW_NUMBER() over (partition by UserLogin order by Active desc, DateModified desc) rn From DWH.ar.ActivityReport_Users with (nolock)) u on u.rn = 1 and ard.ModifiedBy = u.UserLogin  " & _
" " & _
"	order by 3, 5 "
        ...
        '"	left join DWH.AR.ActivityReport_2_Source two on ar.ActivityID = two.ActivityID and two.Active = 1 " & _
        '"	join (select NetID as ActivityID from DWH.AR.ActivityReport_Netting " & _
        '"	where InitialID = @SearchID and Active = 1 " & _
        '"	union  " & _
        '"	select InitialID from DWH.AR.ActivityReport_Netting " & _
        '"	where NetID = @SearchID  and Active = 1 " & _
        '"	union " & _
        '"	select TransferFrom from  (select * from DWH.AR.ActivityReport_Transfers union all select * from DWH.AR.ActivityReport_TransfersHistorical) x " & _
        '"	where isnull(IntermediateRow, 0) = @SearchID or TransferTo = @SearchID and Active = 1 " & _
        '"	union  " & _
        '"	select TransferTo from  (select * from DWH.AR.ActivityReport_Transfers union all select * from DWH.AR.ActivityReport_TransfersHistorical) x " & _
        '"	where isnull(IntermediateRow, 0) = @SearchID or TransferFrom = @SearchID and Active = 1 " & _
        '"	union select @SearchID) find on ar.ActivityID = find.ActivityID " & _
        '"	left join DWH.AR.ActivityReport_Source s on two.Base_ID = s.Base_ID and s.Active = 1 " & _
        '"	) N " & _
        '"	 " & _
        '"	Select distinct ST2.TFID, 'Transfer' as ChangeType, " & _
        '"	 ST2.TransferDate as ActivityDate, ST2.Amount, ST2.ModifyDate " & _
        ...
        '"            Select ','+ST1.LetterCode  AS [text()] " & _
        '"            From (select art.*, t.*  " & _
        '"	from  (select * from DWH.AR.ActivityReport_Transfers union all select * from DWH.AR.ActivityReport_TransfersHistorical) art " & _
        '"	join DWH.AR.ActivityReport ar on ar.ActivityID  in (art.IntermediateRow, art.TransferFrom, art.TransferTo) and ar.Active = 1 " & _
        '"	left join DWH.AR.ActivityReport_2_Source ars on ars.ActivityID = ar.ActivityID and ars.Active = 1 " & _
        '"	left join DWH.AR.ActivityReport_Source s on ars.Base_ID = s.Base_ID and s.Active = 1 " & _
        ...
        '"	left join (select *, ROW_NUMBER() over (partition by UserLogin order by Active desc, DateModified desc) rn From DWH.ar.ActivityReport_Users) u on u.rn = 1 and ST2.ModifyUser = u.UserLogin " & _
        '"	 " & _
        '"	union all " & _
        '"" & _
        '"		Select distinct ST2.NetID, 'Net Rows', " & _
        ...
        '"            Select ','+ST1.LetterCode  AS [text()] " & _
        '"            From (select art.*, t.*  " & _
        '"			from DWH.AR.ActivityReport_Netting art " & _
        '"	join DWH.AR.ActivityReport ar on ar.ActivityID  in (art.InitialID, art.NetID) and ar.Active = 1 " & _
        '"	left join DWH.AR.ActivityReport_2_Source ars on ars.ActivityID = ar.ActivityID and ars.Active = 1 " & _
        '"	left join DWH.AR.ActivityReport_Source s on ars.Base_ID = s.Base_ID and s.Active = 1 " & _
        '"	join #TmpBases t on s.Base_ID = t.Base_ID or t.Base_ID = -ar.ActivityID " & _
        '"	where art.Active = 1) ST1 " & _
        ...
        '"   ST2.LetterCode as LetterCodes " & _
        '"From (select art.*, t.*  " & _
        '"	from DWH.AR.ActivityReport_Splitting art " & _
        '"	join DWH.AR.ActivityReport ar on ar.ActivityID  in (art.InitialID) and ar.Active = 1 " & _
        '"	left join DWH.AR.ActivityReport_2_Source ars on ars.ActivityID = ar.ActivityID and ars.Active = 1 " & _
        '"	left join DWH.AR.ActivityReport_Source s on ars.Base_ID = s.Base_ID and s.Active = 1 " & _
        '"	join #TmpBases t on s.Base_ID = t.Base_ID or t.Base_ID = -ar.ActivityID " & _
        '"	where art.Active = 1) ST2 " & _
        '"	left join (select *, ROW_NUMBER() over (partition by UserLogin order by Active desc, DateModified desc) rn From DWH.ar.ActivityReport_Users) u on u.rn = 1 and ST2.ModifyUser = u.UserLogin " & _
        '"	left join DWH.AR.ActivityReport_Detail ad on ST2.InitialID = ad.ActivityID and convert(date, ad.ActivityDate) <= ST2.ActivityDate " & _
        '"		and not exists (select * from DWH.AR.ActivityReport_Detail adfake where adfake.ActivityID = ad.ActivityID and adfake.ActivityDate > ad.ActivityDate " & _
        '"							and adfake.ActivityDate <= ST2.ActivityDate) " & _
        '"" & _
        '"union all " & _
        '" " & _
        ...
        '"	, t.LetterCode " & _
        '"	from DWH.AR.ActivityReport ar " & _
        '"	join DWH.AR.ActivityReport_Detail ard on ar.ActivityID = ard.ActivityID " & _
        '"	left join DWH.AR.ActivityReport_2_Source ars on ars.ActivityID = ar.ActivityID and ars.Active = 1 " & _
        '"	left join DWH.AR.ActivityReport_Source s on ars.Base_ID = s.Base_ID and s.Active = 1 " & _
        '"	join #TmpBases t on s.Base_ID = t.Base_ID or t.Base_ID = -ar.ActivityID " & _
        '"	left join (select *, ROW_NUMBER() over (partition by UserLogin order by Active desc, DateModified desc) rn From DWH.ar.ActivityReport_Users) u on u.rn = 1 and ard.ModifiedBy = u.UserLogin " & _
        '"" & _
        '"	order by 3, 5 "
    ...
    Private Function BindGrid(x As Integer)
        ...
        Dim s As String = "declare @SearchID int =  " & x & _
            " " & _
            "If OBJECT_ID('tempdb.dbo.#TmpHistory', 'U') IS NOT NULL   " & _
"              DROP TABLE #TmpHistory;  " & _
" " & _
...
"	from (select * from DWH.AR.ActivityReport_Transfers union all select * from DWH.AR.ActivityReport_TransfersHistorical) art  " & _
"	join DWH.AR.ActivityReport ar on ar.ActivityID  in (art.IntermediateRow, art.TransferFrom, art.TransferTo) and ar.Active = 1  " & _
"	left join (select *, ROW_NUMBER() over (partition by UserLogin order by Active desc, DateModified desc) rn From DWH.ar.ActivityReport_Users) u on u.rn = 1 and art.ModifyUser = u.UserLogin " & _
"	left join DWH.AR.ActivityReport_Detail ard on ard.Active = 1 and ar.ActivityID = ard.ActivityID and ard.ActivityDate <= art.TransferDate and ard.ModifiedDate <= art.ModifyDate " & _
"		and not exists (select * from DWH.AR.ActivityReport_Detail badard where badard.activityID = ar.ActivityID and badard.Active = 1  " & _
...
"	left join DWH.AR.ActivityReport_Detail ttard on ttard.Active = 1 and art.TransferTo = ttard.ActivityID and ttard.ActivityDate <= art.TransferDate  " & _
"		and not exists (select * from DWH.AR.ActivityReport_Detail badard where badard.activityID = ttard.ActivityID and badard.Active = 1  " & _
...
"	left join DWH.AR.ActivityReport_Detail tfard on tfard.Active = 1 and art.TransferFrom = tfard.ActivityID and tfard.ActivityDate <= art.TransferDate and tfard.ModifiedDate <= art.ModifyDate " & _
"		and not exists (select * from DWH.AR.ActivityReport_Detail badard where badard.activityID = tfard.ActivityID and badard.Active = 1  " & _
"			and badard.ActivityDate <= art.TransferDate and badard.ModifiedDate <= art.ModifyDate  " & _
...
"            		DWH.AR.ActivityReport_MiscGL m " & _
"					join (select distinct TFID, TransferDate, ModifyDate from DWH.AR.ActivityReport_Transfers where Active = 1) t on convert(date, m.ActivityDate) <= t.TransferDate " & _
...
"            left join (select t.TFID, TransferFrom, sum(Amount) TransferredOut, convert(Date, tt.TransferDate) as TFDate from DWH.AR.ActivityReport_Transfers tt " & _
"				join (select distinct TFID, TransferDate, ModifyDate from DWH.AR.ActivityReport_Transfers where Active = 1) t  " & _
...
"            left join (select t.TFID, TransferTo, sum(Amount) TransferredIn, convert(date, tf.TransferDate) as TFTDate from DWH.AR.ActivityReport_Transfers  tf " & _
"				join (select distinct TFID, TransferDate, ModifyDate from DWH.AR.ActivityReport_Transfers where Active = 1) t  " & _
...
"	) ST2  " & _
"	/*left join DWH.AR.ActivityReport_Detail ard on ard.Active = 1 and ChangeType <> 'Row Created by Netting Others' and ST2.ActivityID = ard.ActivityID and ard.ActivityDate <= ST2.ActivityDate  " & _
"		and ard.ModifiedDate <= ST2.ModifyDate " & _
"		and not exists (select * from DWH.AR.ActivityReport_Detail badard where badard.activityID = ST2.ActivityID and badard.Active = 1  " & _
"			and badard.ActivityDate <= ST2.ActivityDate and badard.ModifiedDate <= ST2.ModifyDate  " & _
"			and (badard.ActivityDate > ard.ActivityDate or (badard.ActivityDate = ard.ActivityDate and badard.ModifiedDate > ard.ModifiedDate))) */ " & _
"	left join DWH.AR.ActivityReport_Detail neward on neward.Active = 1 and ChangeType = 'Row Created by Netting Others' and ST2.ActivityID = neward.ActivityID  " & _
...
"            left join (select  TransferFrom, sum(Amount) TransferredOut, convert(Date, tt.TransferDate) as TFDate from DWH.AR.ActivityReport_Transfers tt " & _
"            		where Active = 1    " & _
...
"            left join (select  TransferTo, sum(Amount) TransferredIn, convert(date, tf.TransferDate) as TFTDate from DWH.AR.ActivityReport_Transfers  tf " & _
"            		where Active = 1 " & _
...
"left join DWH.AR.ActivityReport_2_Source a2s with (nolock) on ar.ActivityID = a2s.ActivityID and a2s.Active = 1 " & _
"left join DWh.AR.ActivityReport_Source s with (nolock) on a2s.Base_ID = s.Base_ID and s.Active = 1 " & _
"left join (select *, ROW_NUMBER() over (partition by UserLogin order by Active desc, DateModified desc) rn From DWH.ar.ActivityReport_Users with (nolock)) u on ar.CreatedBy = u.UserLogin and u.rn = 1 " & _
